...
>
> **Actualizado 2026-09-08:** alta de **#203 frida-tgrep** (grep indexado por trigramas, 3.4–52x sobre ripgrep en monorepos — research en [docs/research/2026-09-08-tgrep-trigram-grep.md](research/2026-09-08-tgrep-trigram-grep.md)). P2/P3 frontera, después de F10/#191: valor alto condicionado al contexto (monorepos cliente = sí; repos medianos = invisible, por eso `minFiles` + fallback rg). Complementa la dupla de facturación: búsqueda rápida → menos lecturas completas.
>
...
| **P2** | Autonomía, aislamiento y **facturación** (agente seguro, paralelo y barato) | ✅ #35, ✅ #26, ✅ #13; **#202 shunt↖, F10, #14, #2✅, #203 tgrep, F8, F9** | **#202 y F10 = dupla de facturación (cima por valor/esfuerzo)**; #203 tgrep (P2/P3 frontera — condicionado a monorepos cliente); sandboxes/detached/worktrees/plugins/abort listos; restan sesiones paralelas (#14) y las F de UX |
...
| **#203** `frida-tgrep` | **Grep indexado (trigramas)** para monorepos: override del tool grep con fallback ripgrep; server con watcher + ciclo de vida gestionado; binario pineado (patrón scc) | 3.4–52x sobre ripgrep en repos grandes (mismo perfil de los monorepos Java cliente). Condicionado al contexto: `minFiles` + fallback lo hacen gratis cuando no aplica. Complementa #202 (búsqueda rápida → menos reads completos) |
...
4. **Sprint P2** — **#202 frida-shunt (PRIMERO — dupla de facturación, mejor
   ...
   después **#203 frida-tgrep** (si el uso dominante son monorepos cliente
   — condicionado), #14 (sesiones paralelas, base #13 lista) + F8 (advisor,
   F7 listo) + F9 (web_search keyless). ~~#35, #26, M9, M2, M3, M10~~ ✅
   todos cerrados.
