 ...
 */
export function hasShellIndirection(raw: string | undefined | null): IndirectionCheck {
  ...
  if (CHAINING.test(command)) {
    return {
      detected: true,
      reason:
        "Comando compuesto (&&/||/;/|). Cada parte se ejecuta; confirma que todas son intencionales.",
    };
  }
