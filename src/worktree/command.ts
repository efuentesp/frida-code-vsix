	...
	return {
		...
		notify(message, level) {
			...
			if (level === "error") void vscode.window.showErrorMessage(clean);
				...
				void vscode.window.showWarningMessage(clean);
...
async function pickAction(
	...
	const actions: Array<{ label: string; action: Action }> = [
		...
		{
			label: "Remove worktree — eliminar (conserva la rama)",
			action: "remove",
		},
...
async function addFlow(
	...
	const requestedBranch = await ui.input(
	...
	const requestedPath = await ui.input(
...
async function removeFlow(
	...
	ui.notify(
		`Worktree removido: ${selected.path}. La rama se conservó.`,
		"info",
	);
}
