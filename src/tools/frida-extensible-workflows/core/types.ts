...
export const WORKFLOW_CALL_KINDS = [
	"agent",
	"parallel",
	"pipeline",
	"checkpoint",
...
export const WORKFLOW_AGENT_STATE_CHANGED_EVENT =
	"workflow:agent-state-changed";
export const WORKFLOW_PHASE_CHANGED_EVENT = "workflow:phase-changed";
export const WORKFLOW_CHECKPOINT_STATE_CHANGED_EVENT =
	"workflow:checkpoint-state-changed";
...
export interface AgentOptions<Schema extends TSchema = never> {
	...
	// Seam (ADR-0028): el index signature debe incluir `undefined` para que las
	// props opcionales de arriba (que son `T | undefined` bajo strictNullChecks,
	// activo en Frida) satisfagan el índice (TS2411). No cambia la semántica en
	// runtime: AgentOptions sigue siendo un objeto plano validado en la frontera
	// JSON. El original compila sin strictNullChecks; Frida usa strict:true.
	[key: string]: JsonValue | NoInfer<Schema> | undefined;
}
export type ParallelTasks = Record<
	string,
	() => JsonValue | Promise<JsonValue>
>;
export type ParallelResult<Tasks extends ParallelTasks> = {
...
export type WorkflowBudgetPatch = Partial<
	Record<
		BudgetDimension,
		BudgetLimits | { soft?: number | null; hard?: number | null } | null
	>
>;
...
export interface WorkflowModelAlias {
	resolve: (
		context: Readonly<WorkflowModelAliasResolverContext>,
...
export interface WorkflowExtensionSettings {
	herdr?: Readonly<HerdrExtensionSettings>;
}
export interface WorkflowSettings {
	concurrency: number;
	modelAliases?: Readonly<Record<string, string>>;
	disabledAgentResources?: Readonly<AgentResourceExclusions>;
	extensions?: Readonly<WorkflowExtensionSettings>;
}
...
export interface WorkflowOrchestrationContext {
	agent: {
		<Schema extends TSchema>(
			prompt: string,
			options: Readonly<AgentOptions<Schema> & { outputSchema: Schema }>,
		): Promise<Static<Schema>>;
		(prompt: string, options?: Readonly<AgentOptions>): Promise<JsonValue>;
	};
	...
	parallel: <Tasks extends ParallelTasks>(
		operationName: string,
		tasks: Tasks,
	) => Promise<ParallelResult<Tasks>>;
	pipeline: (...args: readonly unknown[]) => Promise<JsonValue>;
	withWorktree: <Result extends JsonValue>(
		name: string,
		callback: WorkflowWorktreeCallback<Result>,
	) => Promise<Result>;
...
export type WorkflowWorktreeCallback<Result extends JsonValue = JsonValue> = (
	reference: Readonly<WorkflowWorktreeReference>,
...
> = {
	...
	output: OutputSchema;
	run: (
		input: Readonly<Static<NoInfer<InputSchema>>>,
		context: Readonly<WorkflowFunctionContext>,
	) => Promise<Static<NoInfer<OutputSchema>>> | Static<NoInfer<OutputSchema>>;
};
type DefinedWorkflowFunction<
	InputSchema extends TSchema,
	OutputSchema extends TSchema,
	Run extends TypedWorkflowFunction<InputSchema, OutputSchema>["run"],
> = Omit<TypedWorkflowFunction<InputSchema, OutputSchema>, "run"> & {
	run: Run;
};
export function defineWorkflowFunction<
	...
	Run extends TypedWorkflowFunction<InputSchema, OutputSchema>["run"],
>(
	workflowFunction: DefinedWorkflowFunction<InputSchema, OutputSchema, Run>,
): DefinedWorkflowFunction<InputSchema, OutputSchema, Run> {
...
export type LiveSessionHandoffState =
	| "local-running"
	| "handoff-pending"
	| "herdr-running"
	| "returning-local"
...
export interface WorkflowAgentSession {
	...
	subscribeAsync?(
		listener: (event: WorkflowAgentSessionEvent) => void | Promise<void>,
	): () => void;
...
export interface AgentTransportContext {
	...
	readonly tuiIndex?: number;
	readonly tuiLabel?: string;
}
export interface AgentTransport {
	readonly id: string;
	createSession(
		prepared: Readonly<PreparedAgentSession>,
		context: Readonly<AgentTransportContext>,
...
export interface AgentSetupHook {
	...
	setup: (
		agent: AgentSetup,
		context: Readonly<AgentSetupContext>,
	) => void | Promise<void>;
}
...
export interface WorkflowBridge {
	...
	worktree?: (
		owner: string,
		signal: AbortSignal,
	) => Promise<Readonly<WorkflowWorktreeReference>>;
...
export interface WorkflowCatalogError {
	error: { code: "NOT_FOUND"; name: string; message: string };
}
