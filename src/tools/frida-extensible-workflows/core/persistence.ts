...
import type { OwnershipRecord } from "./agent-execution";
import { WorkflowError } from "./types";
import { loadLaunchSnapshot } from "./utils";
...
type Journal = {
	completed: Record<string, CompletedOperation>;
	awaiting?: Record<string, AwaitingCheckpoint>;
	decisions?: Record<string, PendingWorkflowDecision>;
};
const TERMINAL_SUMMARY_STATES = new Set(["completed", "failed", "stopped"]);
const EMPTY_USAGE: WorkflowBudgetUsage = {
	tokens: 0,
	costUsd: 0,
	durationMs: 0,
	agentLaunches: 0,
...
const SYSTEM_PROMPT_ARTIFACT = {
	version: 2 as const,
	format: "append-only" as const,
	storage: SYSTEM_PROMPT_STORAGE,
};
function summaryArtifacts(directory: string): RunSummaryArtifacts {
	return {
		runDirectory: directory,
		statePath: join(directory, "state.json"),
		journalPath: join(directory, "journal.json"),
		snapshotPath: join(directory, "snapshot.json"),
		workflowPath: join(directory, "workflow.js"),
...
function summaryFromRun(
	...
	directory: string,
	journal: Journal,
	previous: Partial<RunSummary> | undefined,
	...
	const replayablePaths = [
		...new Set([
			...(run.retry?.completedPaths ?? []),
			...Object.keys(journal.completed),
		]),
	];
	const incompletePaths = [
		...new Set([
			...(run.retry?.incompletePaths ?? []),
			...(failedAt ? [failedAt] : []),
		]),
	];
	return {
		...
		createdAt,
		updatedAt: now,
		...(previous?.terminalAt || TERMINAL_SUMMARY_STATES.has(run.state)
			? { terminalAt: previous?.terminalAt ?? now }
			: {}),
		usage: { ...EMPTY_USAGE, ...(run.usage ?? {}) },
		agents: run.agents.map(({ id, name, label, state, role, attempts }) => ({
			id,
			name,
			...(label ? { label } : {}),
			state,
		...
		replayablePaths,
		incompletePaths,
		artifacts: summaryArtifacts(directory),
	};
}
...
const gitIdentity = {
	GIT_AUTHOR_NAME: "pi-extensible-workflows",
	GIT_AUTHOR_EMAIL: "pi-extensible-workflows@localhost",
	GIT_COMMITTER_NAME: "pi-extensible-workflows",
	GIT_COMMITTER_EMAIL: "pi-extensible-workflows@localhost",
...
export function projectSessionsDirectory(
	cwd: string,
	home = homedir(),
): string {
	// Seam (ADR-0028 D5 / ADR-0010): layout de persistencia bajo ~/.frida (no
	// ~/.pi) para aislar las runs de Frida del CLI pi. Mismma estructura, distinta
	// raíz. consistency: runsDirectory/listPersistedSessionIds/hasLiveSessionLease
	// derivan todas de aquí.
	return join(
		home,
		".frida",
		"workflows",
...
export async function listPersistedSessionIds(
	cwd: string,
	home = homedir(),
): Promise<string[]> {
	try {
		const entries = await readdir(projectSessionsDirectory(cwd, home), {
			withFileTypes: true,
		});
		return entries
...
export async function hasLiveSessionLease(
	cwd: string,
	sessionId: string,
	home = homedir(),
): Promise<boolean> {
	...
	if (!owner || typeof owner !== "object" || Array.isArray(owner))
		throw new WorkflowError(
			"RUN_OWNED",
			`Pi session ${sessionId} has an invalid ownership lease`,
		);
...
async function cleanupRunTemps(
	directory: string,
	...
	await Promise.all(
		entries.map(async (entry) => {
			const match = entry.isDirectory()
				? RUN_CREATE_TEMP.exec(entry.name)
				: undefined;
		...
		const owner: SessionOwner = {
			...
			startedAt:
				process.platform === "linux"
					? (await stat(`/proc/${String(process.pid)}`)).ctimeMs
					: Date.now(),
		};
		try {
			...
			try {
				...
				if (
					...
					existing.pid > 0 &&
					(await processAlive(existing.pid, existing.startedAt))
				)
					throw new WorkflowError(
						"RUN_OWNED",
						`Pi session ${sessionId} is already owned by process ${String(existing.pid)}`,
					);
			} catch (readError) {
				...
				if (age < SESSION_OWNER_WRITE_GRACE_MS)
					throw new WorkflowError(
						"RUN_OWNED",
						`Pi session ${sessionId} has an active ownership lease`,
					);
				existing = {};
			}
...
export function structuralPath(...names: string[]): string {
	if (names.length === 0 || names.some((name) => name.trim() === ""))
		throw new WorkflowError(
			"INVALID_METADATA",
			"Structural paths require non-empty explicit names",
		);
...
export function atomicWriteFile(
	...
	return writeFile(temporary, content, { encoding: "utf8", mode: 0o600 })
...
function systemPromptStoragePath(directory: string): string {
	return join(directory, SYSTEM_PROMPT_STORAGE);
}
function systemPromptRecordsPath(directory: string): string {
	return join(systemPromptStoragePath(directory), SYSTEM_PROMPT_RECORDS);
}
function systemPromptBodiesPath(directory: string): string {
	return join(systemPromptStoragePath(directory), SYSTEM_PROMPT_BODIES);
}
function systemPromptSequencePath(directory: string): string {
	return join(systemPromptStoragePath(directory), SYSTEM_PROMPT_SEQUENCE);
}
function systemPromptRecordName(sequence: number): string {
	return `${String(sequence).padStart(20, "0")}.json`;
}
async function createSystemPromptStorage(
	directory: string,
	writeArtifact: boolean,
): Promise<void> {
	const storage = systemPromptStoragePath(directory);
	await mkdir(join(storage, SYSTEM_PROMPT_RECORDS), {
		recursive: true,
		mode: 0o700,
	});
	await mkdir(join(storage, SYSTEM_PROMPT_BODIES), {
		recursive: true,
		mode: 0o700,
	});
	...
	if (writeArtifact)
		await atomicJson(
			join(directory, "system-prompts.json"),
...
export class RunStore {
	...
	constructor(
		...
		this.directory = join(
			runsDirectory(this.cwd, sessionId, home),
			safePart(runId),
		);
	}
	...
	async create(
		...
		if (
			resolve(run.cwd) !== this.cwd ||
			run.sessionId !== this.sessionId ||
			run.id !== this.runId
		)
			throw new WorkflowError(
				"INTERNAL_ERROR",
				"Run identity does not match its session-scoped store",
			);
		const temporary = join(
			dirname(this.directory),
			`.${safePart(this.runId)}.${String(process.pid)}.${randomUUID()}.tmp`,
		);
		await mkdir(dirname(this.directory), { recursive: true, mode: 0o700 });
		await mkdir(temporary, { mode: 0o700 });
		try {
			await writeFile(join(temporary, "workflow.js"), snapshot.script, {
				encoding: "utf8",
				mode: 0o600,
			});
			await atomicJson(join(temporary, "snapshot.json"), snapshot);
			await atomicJson(join(temporary, "journal.json"), {
				completed: {},
				awaiting: {},
				decisions: {},
			});
	...
	private async refreshSummary(): Promise<void> {
		const write = this.summaryWrite.then(async () => {
			...
			const previous = await json<Partial<RunSummary>>(
				join(this.directory, "summary.json"),
			).catch(() => undefined);
			const fallbackCreatedAt = await stat(
				join(this.directory, "state.json"),
			).then((value) => new Date(value.mtimeMs).toISOString());
			await atomicJson(
				join(this.directory, "summary.json"),
				summaryFromRun(
					run,
	...
	private refreshSummaryBestEffort(): void {
	...
	async isComplete(): Promise<boolean> {
		try {
			await Promise.all([
				access(join(this.directory, "snapshot.json")),
				access(join(this.directory, "journal.json")),
				access(join(this.directory, "ownership.json")),
				access(join(this.directory, "state.json")),
			]);
			return true;
		} catch {
			return false;
	...
	async load(): Promise<{
		...
		if (
			...
			throw new WorkflowError(
				"RESUME_INCOMPATIBLE",
				"Persisted run belongs to another cwd or Pi session",
			);
		const persisted = run as unknown as Record<string, unknown>;
		if (
			!Array.isArray(persisted.agentSessions) ||
			Object.hasOwn(persisted, "nativeSessions")
		)
			throw new WorkflowError(
				"RESUME_INCOMPATIBLE",
				"Persisted run uses an unsupported agent session format",
			);
		return {
			run,
			snapshot: loadLaunchSnapshot(
				await json<LaunchSnapshot>(join(this.directory, "snapshot.json")),
			),
		};
	}
	async loadStatus(): Promise<PersistedRun> {
		...
		if (
			...
			throw new WorkflowError(
				"RUN_NOT_FOUND",
				"Persisted run does not belong to this project",
			);
		return run;
	}
	async loadSummary(): Promise<RunSummary> {
		...
		const run = await json<PersistedRun>(join(this.directory, "state.json"));
		const journal = await json<Journal>(join(this.directory, "journal.json"));
		const previous = await json<RunSummary>(
			join(this.directory, "summary.json"),
		).catch(() => undefined);
		const [stateStat, journalStat] = await Promise.all([
			stat(join(this.directory, "state.json")),
			stat(join(this.directory, "journal.json")),
		]);
		...
		const updatedAt = new Date(
			Math.max(
				stateStat.mtimeMs,
				journalStat.mtimeMs,
				Number.isNaN(previousUpdatedAt) ? 0 : previousUpdatedAt,
			),
		).toISOString();
	...
	async updateState(
		update: (run: PersistedRun) => PersistedRun | Promise<PersistedRun>,
	): Promise<PersistedRun> {
		let result!: PersistedRun;
		const write = this.stateWrite.then(async () => {
			const current = await json<PersistedRun>(
				join(this.directory, "state.json"),
			);
			if (
				resolve(current.cwd) !== this.cwd ||
				current.sessionId !== this.sessionId ||
				current.id !== this.runId
			)
				throw new WorkflowError(
					"RESUME_INCOMPATIBLE",
					"Persisted run belongs to another cwd or Pi session",
			...
			if (
				resolve(result.cwd) !== this.cwd ||
				result.sessionId !== this.sessionId ||
				result.id !== this.runId
			)
				throw new WorkflowError(
					"INTERNAL_ERROR",
					"Run identity does not match its session-scoped store",
	...
	async saveSnapshot(snapshot: Readonly<LaunchSnapshot>): Promise<void> {
		const write = this.launchSnapshotWrite.then(() =>
			atomicJson(join(this.directory, "snapshot.json"), snapshot),
		);
	...
	async appendEvent(event: WorkflowRunEvent): Promise<void> {
		await this.updateState((run) => ({
			...
			events: [
				...
				run.events?.some(
					(current) =>
						current.type === event.type && current.message === event.message,
				)
					? []
					: [event]),
			],
	...
	async loadOwnership(): Promise<readonly PersistedOwnershipNode[]> {
		return json<readonly PersistedOwnershipNode[]>(
			join(this.directory, "ownership.json"),
		);
	}

	systemPromptPath(): string {
		return join(this.directory, "system-prompts.json");
	}
	private async readSystemPromptArtifact(): Promise<
		| {
				version: number;
				format?: unknown;
				storage?: unknown;
				entries?: EffectiveSystemPrompt[];
		  }
		| undefined
	> {
		return json<{
			...
			storage?: unknown;
			entries?: EffectiveSystemPrompt[];
		}>(this.systemPromptPath()).catch((error: unknown) => {
			if ((error as NodeJS.ErrnoException).code === "ENOENT") return undefined;
			throw error;
		});
	}
	private async appendSystemPromptV2(
		entry: Omit<EffectiveSystemPrompt, "sha256">,
	): Promise<void> {
		...
		try {
			...
			if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
			await atomicWriteFile(bodyPath, entry.prompt);
		}
		const previous = Number(
			await readFile(systemPromptSequencePath(this.directory), "utf8"),
		);
		if (
			...
			throw new Error("Persisted system-prompt sequence is invalid");
		const sequence = previous + 1;
		await atomicWriteFile(
			systemPromptSequencePath(this.directory),
			`${String(sequence)}\n`,
		);
		await atomicJson(
			join(
				systemPromptRecordsPath(this.directory),
				systemPromptRecordName(sequence),
			),
			{
	...
	private async migrateSystemPrompts(legacy: {
		version: number;
		entries?: EffectiveSystemPrompt[];
	}): Promise<void> {
		await rm(systemPromptStoragePath(this.directory), {
			recursive: true,
			force: true,
		});
	...
	private async prepareSystemPromptStorage(): Promise<void> {
		...
		if (artifact === undefined) {
			...
			artifact.version !== SYSTEM_PROMPT_ARTIFACT.version ||
			artifact.format !== SYSTEM_PROMPT_ARTIFACT.format ||
			artifact.storage !== SYSTEM_PROMPT_ARTIFACT.storage
		)
			throw new Error("Persisted system prompts are invalid");
	}
	private async readSystemPromptsV2(): Promise<
		readonly EffectiveSystemPrompt[]
	> {
		const artifact = await this.readSystemPromptArtifact();
		if (
			!artifact ||
		...
		if (!Number.isSafeInteger(sequence) || sequence < 0)
			throw new Error("Persisted system-prompt sequence is invalid");
		const names = (await readdir(systemPromptRecordsPath(this.directory)))
		...
		for (const name of names) {
			...
			if (!match)
				throw new Error("Persisted system-prompt records are invalid");
			...
			const record = await json<Record<string, unknown>>(
				join(systemPromptRecordsPath(this.directory), name),
			);
			...
			if (
				...
				throw new Error("Persisted system-prompt record is invalid");
			const prompt = await readFile(
				join(systemPromptBodiesPath(this.directory), sha256),
				"utf8",
			);
			if (createHash("sha256").update(prompt).digest("hex") !== sha256)
				throw new Error("Persisted system-prompt body is invalid");
	...
	async systemPrompts(): Promise<readonly EffectiveSystemPrompt[]> {
		...
		if (artifact.version === SYSTEM_PROMPT_ARTIFACT.version)
			return this.readSystemPromptsV2();
		throw new Error("Persisted system prompts are invalid");
	}

	private async updateJournal<T>(
		update: (journal: Journal) => T | Promise<T>,
	): Promise<T> {
		let result!: T;
	...
	async complete(path: string, value: JsonValue): Promise<void> {
		await this.updateJournal((journal) => {
			if (journal.completed[path])
				throw new WorkflowError(
					"DUPLICATE_NAME",
					`Completed structural path already exists: ${path}`,
				);
	...
	private async replayableOperationsFrom(
		seen: Set<string>,
	): Promise<readonly CompletedOperation[]> {
		if (seen.has(this.runId))
			throw new WorkflowError(
				"RESUME_INCOMPATIBLE",
				"Retry provenance contains a cycle",
			);
	...
	async awaitCheckpoint(
		...
		return this.updateJournal((journal) => {
			...
			(journal.awaiting as Record<string, AwaitingCheckpoint>)[
				checkpoint.path
			] = checkpoint;
			return undefined;
		});
	...
	async requestWorkflowDecision(
		request: PendingWorkflowDecision,
	): Promise<void> {
		await this.updateJournal((journal) => {
			journal.decisions ??= {};
			journal.decisions[request.proposalId] = request;
		});
	}
	async pendingWorkflowDecisions(): Promise<
		readonly PendingWorkflowDecision[]
	> {
		await this.journalWrite;
		const journal = await json<Journal>(join(this.directory, "journal.json"));
		return Object.values(journal.decisions ?? {});
	}
	async answerWorkflowDecision(
		proposalId: string,
		approved: boolean,
	): Promise<PendingWorkflowDecision | undefined> {
		return this.updateJournal((journal) => {
			const request = journal.decisions?.[proposalId];
			if (!request) return undefined;
			journal.completed[`decision/${proposalId}`] = {
	...
	async answerCheckpoint(
		name: string,
		approved: boolean,
	): Promise<AwaitingCheckpoint | undefined> {
		return this.updateJournal((journal) => {
			const checkpoint = Object.values(journal.awaiting ?? {}).find(
				(item) => item.name === name,
			);
			...
			journal.awaiting = Object.fromEntries(
				Object.entries(journal.awaiting ?? {}).filter(
	...
	private expectedWorktree(
		owner: string,
	): Pick<WorktreeReference, "path" | "branch"> {
		const key = createHash("sha256")
			.update(`${this.sessionId}\0${this.runId}\0${owner}`)
			.digest("hex")
			.slice(0, 16);
		return {
			path: join(this.directory, "worktrees", key),
			branch: `pi-extensible-workflows/${safePart(this.runId)}/${key}`,
		};
	}
	...
	private namedWorktreeOwner(name: string): string {
		if (!name.trim())
			throw new WorkflowError(
				"WORKTREE_FAILED",
				"Named worktree names must be non-empty",
			);
	...
	private structuralWorktree(
		...
		const relativePath =
			typeof candidate.path === "string"
				? relative(this.directory, candidate.path)
				: "..";
		const relativeCwd =
			typeof candidate.path === "string" && typeof candidate.cwd === "string"
				? relative(candidate.path, candidate.cwd)
				: "..";
		if (
			...
			resolve(candidate.path) !== expected.path ||
	...
	private async borrowedWorktreeRecords(
		wait = true,
	): Promise<readonly BorrowedWorktreeBinding[]> {
		if (wait) await this.borrowedWorktreeWrite;
		const records = await json<unknown[]>(
			join(this.directory, "borrowed-worktrees.json"),
		).catch((error: unknown) => {
			if ((error as NodeJS.ErrnoException).code === "ENOENT") return [];
			throw error;
		});
		if (!Array.isArray(records))
			throw new WorkflowError(
				"WORKTREE_FAILED",
				"Borrowed worktree bindings are invalid",
			);
		...
		return records.map((record) => {
			if (!record || typeof record !== "object")
				throw new WorkflowError(
					"WORKTREE_FAILED",
					"Borrowed worktree binding is invalid",
				);
			...
			if (
				...
				candidate.owner !== this.namedWorktreeOwner(candidate.name)
			)
				throw new WorkflowError(
					"WORKTREE_FAILED",
					"Borrowed worktree binding is invalid",
			...
			if (seen.has(candidate.name))
				throw new WorkflowError(
					"WORKTREE_FAILED",
					`Duplicate borrowed worktree binding for ${candidate.name}`,
				);
	...
	async borrowedWorktrees(): Promise<readonly BorrowedWorktreeBinding[]> {
		return this.borrowedWorktreeRecords();
	}

	private async borrowedWorktree(
		name: string,
	): Promise<BorrowedWorktreeBinding | undefined> {
		return (await this.borrowedWorktreeRecords()).find(
	...
	private async sourceRun(sourceRunId: string): Promise<RunStore> {
		if (!sourceRunId || sourceRunId === this.runId)
			throw new WorkflowError(
				"WORKTREE_FAILED",
				"Borrowed worktree source run is invalid",
			);
		const source = new RunStore(
			this.cwd,
			this.sessionId,
			sourceRunId,
			this.home,
	...
	async validateParentRun(parentRunId: string): Promise<void> {
		await this.sourceRun(parentRunId);
	}
	async validateRetrySource(): Promise<void> {
		const validate = async (
			...
			if (!retry) return;
			if (
				typeof retry.sourceRunId !== "string" ||
				!retry.sourceRunId ||
				retry.sourceRunId === current.runId ||
				typeof retry.lineageRootRunId !== "string" ||
				!retry.lineageRootRunId ||
				!Array.isArray(retry.completedPaths) ||
				retry.completedPaths.some((path) => typeof path !== "string") ||
				!Array.isArray(retry.incompletePaths) ||
				retry.incompletePaths.some((path) => typeof path !== "string") ||
				!Array.isArray(retry.namedWorktrees) ||
				retry.namedWorktrees.some((name) => typeof name !== "string")
			)
				throw new WorkflowError(
					"RESUME_INCOMPATIBLE",
					"Retry provenance is incomplete",
				);
			...
			if (loaded.run.parentRunId !== retry.sourceRunId)
				throw new WorkflowError(
					"RESUME_INCOMPATIBLE",
					"Retry parent run does not match its source run",
				);
			if (sourceRun.state !== "failed")
				throw new WorkflowError(
					"RESUME_INCOMPATIBLE",
					`Retry source run ${retry.sourceRunId} is not failed`,
				);
			const expectedLineageRoot =
				sourceRun.retry?.lineageRootRunId ?? sourceRun.id;
			if (retry.lineageRootRunId !== expectedLineageRoot)
				throw new WorkflowError(
					"RESUME_INCOMPATIBLE",
					"Retry lineage root does not match its source run",
				);
		...
		try {
			...
			throw error instanceof WorkflowError &&
				error.code === "RESUME_INCOMPATIBLE"
				? error
				: new WorkflowError(
						"RESUME_INCOMPATIBLE",
						error instanceof Error ? error.message : String(error),
	...
	private async ownedWorktree(
		...
		const records = await json<unknown[]>(
			join(this.directory, "worktrees.json"),
		);
		const matches = records.filter(
			(candidate) =>
				candidate &&
				typeof candidate === "object" &&
				(candidate as Partial<WorktreeReference>).owner === owner,
		);
		if (matches.length !== 1)
	...
	private async resolveBorrowedWorktree(
		binding: BorrowedWorktreeBinding,
		seen: Set<string>,
	): Promise<{
		...
		try {
			...
			if (!resolved)
				throw new Error(
					`Missing named worktree ${binding.name} in source run ${binding.sourceRunId}`,
				);
			if (resolved.owner !== binding.owner)
				throw new Error(
					`Borrowed worktree binding does not match source owner for ${binding.name}`,
				);
			return resolved;
		} catch (error) {
	...
	private async findNamedWorktree(
		name: string,
		seen: Set<string> = new Set(),
	): Promise<
		| { reference: WorktreeReference; sourceRunId: string; owner: string }
		| undefined
	> {
		...
		if (seen.has(this.runId))
			throw new WorkflowError(
				"WORKTREE_FAILED",
				"Borrowed worktree bindings contain a cycle",
			);
		const nextSeen = new Set(seen);
		nextSeen.add(this.runId);
		...
		if (binding) {
			...
			if (loaded.run.parentRunId === undefined)
				throw new WorkflowError(
					"WORKTREE_FAILED",
					`Borrowed worktree ${name} has no parent run`,
				);
			const parent = await this.sourceRun(loaded.run.parentRunId);
			const resolved = await parent.findNamedWorktree(name, nextSeen);
			if (
				!resolved ||
				resolved.sourceRunId !== binding.sourceRunId ||
				resolved.owner !== binding.owner
			)
				throw new WorkflowError(
					"WORKTREE_FAILED",
					`Borrowed worktree binding for ${name} is not inherited from its parent run`,
				);
			return resolved;
		}
		const records = await json<unknown[]>(
		...
		try {
			...
			records.forEach((record) => {
				if (
					...
					typeof (record as Partial<WorktreeReference>).owner !== "string"
				)
			...
			const entries = await readdir(join(this.directory, "worktrees"), {
				withFileTypes: true,
			}).catch((error: unknown) => {
			...
			for (const entry of entries)
				if (
					...
					!paths.has(resolve(join(this.directory, "worktrees", entry.name)))
				)
					throw new Error(
						`Unrecorded worktree artifact: ${join(this.directory, "worktrees", entry.name)}`,
					);
		} catch (error) {
			throw new WorkflowError(
				"WORKTREE_FAILED",
		...
		try {
			...
			for (const record of records) {
				const owner =
					...
					typeof record === "object" &&
					typeof (record as Partial<WorktreeReference>).owner === "string"
						? (record as Partial<WorktreeReference>).owner
						: undefined;
	...
	private async cleanupMarker(markerPath: string): Promise<void> {
		...
		if (marker.path !== expected.path || marker.branch !== expected.branch)
			return;
		const root = await git(this.cwd, ["rev-parse", "--show-toplevel"])
			.then((value) => value.trim())
			.catch(() => "");
		if (!root) return;
		const branchBase = await git(root, [
			"rev-parse",
			"--verify",
		...
		await git(root, ["worktree", "remove", "--force", expected.path]).catch(
			() => undefined,
		);
	...
	async validateWorktree(
		...
		try {
			...
			if (binding) {
				const resolved = await this.resolveBorrowedWorktree(
					binding,
					new Set([this.runId]),
				);
				if (
					cwd !== undefined &&
					resolve(cwd) !== resolve(resolved.reference.cwd)
				)
					throw new Error(`Invalid worktree record for ${owner}`);
	...
	async worktree(owner: string): Promise<WorktreeReference> {
		const write = this.worktreeWrite.then(async () => {
			...
			let records = await json<WorktreeReference[]>(recordsPath).catch(
				(error: unknown) => {
					if ((error as NodeJS.ErrnoException).code === "ENOENT") return [];
					throw error;
				},
			...
			if (name && loaded.run.parentRunId !== undefined) {
				const resolved = await this.resolveNamedWorktreeFromParent(
					name,
					loaded.run.parentRunId,
				);
				if (resolved) {
					await this.bindBorrowedWorktree({
						name,
			...
			if (
				name &&
				Array.isArray(loaded.run.retry?.namedWorktrees) &&
				loaded.run.retry.namedWorktrees.includes(name)
			)
				throw new WorkflowError(
					"WORKTREE_FAILED",
					`Missing inherited named worktree ${name}`,
				);
			...
			try {
				const root = (
					await git(this.cwd, ["rev-parse", "--show-toplevel"])
				).trim();
				...
				if (launchRelative === ".." || launchRelative.startsWith(`..${sep}`))
				...
				const tree = (
					await git(root, ["write-tree"], { GIT_INDEX_FILE: index })
				).trim();
				const commit = (
					await git(
						root,
						[
							...
							"HEAD",
							"-m",
							"pi-extensible-workflows runtime snapshot",
						],
						{ GIT_INDEX_FILE: index, ...gitIdentity },
					)
				).trim();
				const record = {
					owner,
				...
				await rm(index, { force: true });
				if (worktreeCreated)
					await git(this.cwd, ["worktree", "remove", "--force", path]).catch(
						() => undefined,
					);
				if (branchCreated)
				...
				try {
					...
					if (matches.length === 1) {
						...
						) as WorktreeReference[];
						await atomicJson(recordsPath, records);
					}
				} catch {
					/* Ownership changed or disappeared: do not delete anything. */
				}
				throw new WorkflowError(
					"WORKTREE_FAILED",
					error instanceof Error ? error.message : String(error),
	...
	private async resolveNamedWorktreeFromParent(
		name: string,
		parentRunId: string,
	): Promise<
		| { reference: WorktreeReference; sourceRunId: string; owner: string }
	...
	private async bindBorrowedWorktree(
		binding: BorrowedWorktreeBinding,
	): Promise<void> {
		const write = this.borrowedWorktreeWrite.then(async () => {
			...
			if (existing) {
				if (JSON.stringify(existing) !== JSON.stringify(binding))
					throw new WorkflowError(
						"WORKTREE_FAILED",
						`Borrowed worktree binding for ${binding.name} changed`,
					);
				return;
			}
		...
		this.borrowedWorktreeWrite = write.then(
			() => undefined,
			() => undefined,
		);
		await write;
			...
			const write = this.snapshotWrite.then(async () => {
				...
				for (let attempt = 0; attempt < 3; attempt += 1) {
					...
					try {
						await git(
							record.path,
							["commit", "-m", "pi-extensible-workflows runtime snapshot"],
							gitIdentity,
						);
						break;
					} catch (error) {
	...
	async worktrees(): Promise<readonly WorktreeReference[]> {
		const records = await json<WorktreeReference[]>(
			join(this.directory, "worktrees.json"),
		).catch((error: unknown) => {
			if ((error as NodeJS.ErrnoException).code === "ENOENT") return [];
			throw error;
		...
		const owned = await Promise.all(
			records
				.filter((record) => !boundOwners.has(record.owner))
				.map(async (record) => {
					try {
		...
		return [
			...owned.filter(
				(record): record is WorktreeReference => record !== undefined,
			),
			...borrowed,
		];
	}
	async validNamedWorktrees(): Promise<readonly string[]> {
		const load = async (path: string): Promise<unknown> => {
			try {
				return await json<unknown>(path);
			} catch (error) {
				if ((error as NodeJS.ErrnoException).code === "ENOENT") return [];
				throw error;
			}
		...
		for (const record of Array.isArray(rawRecords) ? rawRecords : []) {
			...
			if (
				!name ||
				owner !== this.namedWorktreeOwner(name) ||
				boundOwners.has(owner)
			)
				continue;
			try {
				...
				/* Do not advertise stale or invalid records. */
			}
		}
		for (const binding of bindings) {
			try {
				...
				/* Do not advertise stale inherited records. */
			}
		}
		return [...names];
	}
	...
	async delete(confirmed: boolean): Promise<void> {
		if (!confirmed)
			throw new WorkflowError(
				"CANCELLED",
				"Run deletion requires confirmation",
			);
		const records = await json<unknown[]>(
			join(this.directory, "worktrees.json"),
		).catch((error: unknown) => {
		...
		const validated = records.map((record) => {
			try {
				if (
					...
					throw new Error("Invalid worktree record");
				return this.structuralWorktree(
					(record as Partial<WorktreeReference>).owner as string,
					record,
				);
			} catch (error) {
				throw new WorkflowError(
		...
		for (const record of validated) {
			await git(this.cwd, ["worktree", "remove", "--force", record.path]).catch(
				() => undefined,
			);
			await git(this.cwd, ["branch", "-D", record.branch]).catch(
				() => undefined,
			);
		}
