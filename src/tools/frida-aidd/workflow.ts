...
// Evidencia de intento (#73): un agente que termina SIN texto (provider 500 —
// stopReason=error, 0 tokens; v. runs 8fb037a7/4ebc234c) se diagnostica como
// tal — nunca aparece como línea vacía en el expediente ni en el reintento.
function ev(s) {
	if (s === null || s === undefined) return "(agente terminó SIN texto — probable fallo del provider/gateway; revisa los logs del host)"
	const t = String(s).trim()
	return t || "(agente terminó SIN texto — probable fallo del provider/gateway; revisa los logs del host)"
}
