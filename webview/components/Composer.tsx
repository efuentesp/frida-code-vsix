...
// Callback ref para traer el item activo del autocomplete a la vista cuando se
// mueve con flechas (ArrowUp/Down cambian `sel`, pero no disparaban scroll). Se
// pasa solo al item con i === sel; al cambiar la selección, React llama al ref
// del nuevo activo → scrollIntoView. block:"nearest" no salta si ya es visible.
...
export function Composer({
	...
	const suggestions =
		activeQuery !== null && files && files.query === activeQuery
			? files.items
			: [];
	...
	function recompute(el: HTMLTextAreaElement) {
		...
		const same =
			aq &&
			argQuery &&
			aq.command === argQuery.command &&
			aq.prefix === argQuery.prefix;
	...
	function extractAtFiles(value: string): { full: string; rel: string }[] {
		...
		while ((m = re.exec(value)) !== null)
			out.push({ full: m[0], rel: m[1] ?? m[2] });
		return out;
	}

	function removeAt(full: string) {
		const el = ref.current;
		const next = text.replace(full, "").replace(/ {2,}/g, " ").trimStart();
		setText(next);
		requestAnimationFrame(() => {
			el?.focus();
			if (el) grow(el);
	...
	function onPaste(e: React.ClipboardEvent<HTMLTextAreaElement>) {
		...
		e.preventDefault();
		void Promise.all(imgs.map(readImageFile)).then((atts) =>
			setImages((p) => [...p, ...atts]),
		);
	}
	...
	function onKeyDown(e: React.KeyboardEvent<HTMLTextAreaElement>) {
		if (fileOpen) {
			...
			if (e.key === "ArrowDown") {
				e.preventDefault();
				setSel((s) => Math.min(s + 1, commandMatches.length - 1));
				return;
			}
			if (e.key === "ArrowUp") {
				e.preventDefault();
			...
			if (e.key === "Enter" || e.key === "Tab") {
				e.preventDefault();
				insertCommand(commandMatches[sel]);
				return;
			}
			if (e.key === "Escape") {
				e.preventDefault();
			...
			if (e.key === "ArrowDown") {
				e.preventDefault();
				setSel((s) => Math.min(s + 1, argMatches.length - 1));
				return;
			}
			if (e.key === "ArrowUp") {
				e.preventDefault();
			...
			if (e.key === "Enter" || e.key === "Tab") {
				e.preventDefault();
				insertArg(argMatches[sel].value);
				return;
			}
			if (e.key === "Escape") {
				e.preventDefault();
			...
			{commandOpen && (
				<div className="file-popup cmd-popup">
					{commandMatches.map((c, i) => (
						...
						>
							<span className={"cmd-kind " + c.kind}>
