...
tags: [plan, frida-traffic2api, skill-pack, frida-extensible-workflows, openapi, har, nav-graph, moat]
status: ready
parent: ".rpiv/artifacts/designs/2026-08-26_13-25-20_traffic2api-m9.md"
phase_count: 6
phases:
  - { n: 1, title: "Foundation — prompts, stages y resolver 3-capas", files: ["src/tools/frida-traffic2api/skills.ts", "src/tools/frida-traffic2api/resolver.ts", "test/frida-traffic2api/resolver.test.ts"], depends_on: [] }
  - { n: 2, title: "Generador del script de workflow — 8 fases, modos, inventario híbrido", files: ["src/tools/frida-traffic2api/workflow.ts"], depends_on: [1] }
  - { n: 3, title: "Patrón builtin + sonda moat + registro completo", files: ["src/tools/frida-traffic2api/index.ts", "src/pi-session.ts", "src/extension.ts", "test/frida-traffic2api/pattern.test.ts"], depends_on: [2] }
  - { n: 4, title: "Publicación — docs, how-to, README, cross-links de hermanos", files: ["docs/tools/frida-traffic2api.md", "docs/how-to-frida-traffic2api.md", "README.md", "docs/tools/frida-understand-app.md", "docs/how-to-frida-understand-app.md", "docs/tools/frida-app-walkthrough.md", "docs/how-to-frida-app-walkthrough.md", "docs/roadmap-extensiones.md"], depends_on: [3] }
  - { n: 5, title: "e2e sobre el motor real — mock network har, HARs externos, moat, degradaciones", files: ["test/frida-traffic2api/e2e.test.ts", "src/tools/frida-traffic2api/workflow.ts"], depends_on: [3] }
  - { n: 6, title: "Validación typebox de openapi.json + baseline final", files: ["test/frida-traffic2api/openapi-schema.test.ts"], depends_on: [5] }
last_updated: 2026-08-26T18:22:34-0600
last_updated_by: Edgar F. Fuentes Perea
last_updated_note: "Step 5: triaje de Plan Review — 5 hallazgos applied plan-local"
---

# M9 frida-traffic2api — Implementation Plan

## Overview

Implementa el skill pack `frida-traffic2api` (patrón builtin `traffic2api`, issue #135, M9 Pista M): captura el tráfico HTTP real de una app web — walk agéntico grabando HAR con `agent-browser network har start/stop` sobre una sesión pre-autenticada, o HAR externo devtools/mitmproxy — y deriva deterministamente `docs/api/` con la spec OpenAPI 3.1 de la API observada (errores 4xx/5xx incluidos), la matriz funcionalidad↔endpoint↔módulo con grounding del moat (pi-lens + frida-codebase-index), huérfanos bidireccionales, zona muerta calificada por alcanzabilidad y el grafo de navegación con frontera clasificada.

El pack compone al motor `frida-extensible-workflows` sin tocarlo (molde M8/M1, 0 líneas del motor): es el primer pack que combina los dos ejes de sus hermanos — moat declarativo (`meta.moat`, M1 understand-app) y sesión pinneada por args (`args.session`, M8 app-walkthrough), ejes ortogonales del spawner.

Los límites de fase se heredan 1:1 de los slices verificados por slice-verifier en el design (`## Slices`, Design History con 6/6 aprobados); los Success Criteria pasan verbatim. Todo el código proviene del `## Architecture` del design — estado final, incluye el fix cascade del Slice 5 ratificado en micro-checkpoint (REFS_JS mapa + consumo per-screen "@").

Referencia completa de decisiones: `.rpiv/artifacts/designs/2026-08-26_13-25-20_traffic2api-m9.md` (D1–D15).
...
## Phase 1: Foundation — prompts, stages y resolver 3-capas

### Overview

Capa de constantes y prompts del pack: los 4 stages agénticos (`walk`/`boundary`/`matrix`/`judge`, D11), prompts defaults es-MX, el preamble no-stage `TRAFFIC2API_PREAMBLE` (veto de irreversibles M8 + solo-escritura `docs/api/**` M1 con excepción `index_codebase` + seguridad del HAR — inalcanzable para overrides 3-capas) y el resolver 3-capas (quinto consumidor de `createLayeredStageResolver`), con su suite de tests. Fase sin dependencias: la base que todas las demás consumen.

### Changes Required

#### 1. Constantes, preamble no-stage y prompts defaults

**File**: `src/tools/frida-traffic2api/skills.ts`
**Changes**: NEW — `TRAFFIC2API_STAGES` (4 claves), constantes (`DEFAULT_ARTIFACT_LANGUAGE` "es-MX", `TRAFFIC2API_ARTIFACTS_DIR` "docs/api", `DEFAULT_SESSION_NAME` "app-walkthrough"), `TRAFFIC2API_PREAMBLE` (política de acciones + seguridad del tráfico) y `DEFAULT_STAGE_PROMPTS` es-MX por stage.
...
**File**: `src/tools/frida-traffic2api/resolver.ts`
**Changes**: NEW — reusa `createLayeredStageResolver` de frida-aidd (quinto consumidor): defaults (skills.ts) → equipo (`.frida/traffic2api/stages.json`) → usuario (`~/.frida/traffic2api/stages.json`). JSON inválido aborta ruidosamente; los invariantes de seguridad viven en el preamble no-stage y sobreviven a cualquier override.
...
#### 3. Suite de tests del resolver

**File**: `test/frida-traffic2api/resolver.test.ts`
**Changes**: NEW — 6 tests: defaults para los 4 stages, override de equipo, usuario gana sobre equipo, stages desconocidos/vacíos ignorados (las fases deterministas no son claves), JSON inválido aborta, y los vetos viven SOLO en el preamble (D11).
...
- [x] Test type checking passes: `npm run typecheck:test`
- [x] Resolver suite pasa (6 tests): `npx vitest run test/frida-traffic2api/resolver.test.ts`
- [x] Los 4 stages agénticos declarados: `grep -c "walk\|boundary\|matrix\|judge" src/tools/frida-traffic2api/skills.ts` retorna >= 4
- [x] Cero deps nuevas: `git diff --exit-code package.json package-lock.json`
...
## Phase 2: Generador del script de workflow — 8 fases, modos, inventario híbrido

### Overview

El corazón determinista del pack: `workflow.ts` con la validación eager de modos excluyentes (D2: `walk {url, maxScreens…}` vs `externo {harPath…}` con errores que instruyen el flujo) y el generador del script de 8 fases — `bootstrap` (gates + sondas) → `walk` (loop agéntico con captura HAR, D4/D5) → `ingest` (carve `node` host-side, D6) → `spec` (OpenAPI 3.1 con paths colapsados, D7) → `graph` (nav-graph + navegacion.md, D8) → `matrix` (prep zona muerta D9) → `synthesize` (writer único + veredicto de cobertura) → `judge` (auditor detached + checkpoint). Inventario híbrido M8+M1 con writer único. **Sin paralelismo interno**: el script es un solo template literal secuencial. El fence YA incluye el fix cascade del Slice 5 (REFS_JS lee el mapa real de refs; consumo per-screen normaliza "@"): el Architecture del design es el estado final.

### Changes Required

#### 1. Generador completo del script de workflow

**File**: `src/tools/frida-traffic2api/workflow.ts`
**Changes**: NEW — tipos de args por modo + `validateTraffic2ApiArgs` (eager, molde `validateAppWalkthroughArgs`), `MOAT_TOOL_CATALOG` (10 tools, molde M1), `FRAMEWORK_ROUTE_PATTERNS` (7 frameworks, D9), helpers `node` host-side (`HAR_CARVE_SOURCE`, `SPEC_BUILDER_SOURCE`, `OPENAPI_GATE_SOURCE`, `REFS_BY_STEP_SOURCE` con fix cascade), `stageConsts`/`lit`/`stepLimitFor`, y `generateTraffic2ApiWorkflow` que emite el script completo de 8 fases con inventario, schemas, cortes pre-LLM, degradaciones y checkpoint final.
...
- [x] Type checking passes: `npm run typecheck`
- [x] Test type checking passes: `npm run typecheck:test`
- [x] El generador declara las 8 fases: `grep -c 'phase("' src/tools/frida-traffic2api/workflow.ts` retorna exactamente 8
- [x] Validación eager de modos excluyentes: `grep -c "MUTUAMENTE EXCLUYENTES" src/tools/frida-traffic2api/workflow.ts` retorna 1 y `grep -c "falta el origen del tráfico" src/tools/frida-traffic2api/workflow.ts` retorna 1
- [x] El carve corre en node host-side, no en el sandbox: `grep -c "node " src/tools/frida-traffic2api/workflow.ts` retorna >= 3
- [x] Fence-guard propio: `grep -c "T2A_EOF" src/tools/frida-traffic2api/workflow.ts` retorna >= 2
- [x] Captura HAR con contrato del binario: `grep -c "network har start --content all" src/tools/frida-traffic2api/workflow.ts` retorna >= 1 y `grep -c "network har stop" src/tools/frida-traffic2api/workflow.ts` retorna >= 2
- [x] Cero deps nuevas: `git diff --exit-code package.json package-lock.json`
- [x] Motor intacto: `git diff --exit-code src/tools/frida-extensible-workflows/`
...
## Phase 3: Patrón builtin + sonda moat + registro completo

### Overview

Expone el pack al runtime: `TRAFFIC2API_PATTERN` (patrón builtin con `meta.moat {lens, codebaseIndex}` y `meta.requiredTools ["shell"]`), la sonda de capacidades host-side (D3: `piLensEntryPath` + `isInstalledAtPin` + toggle), la factory `createFridaTraffic2Api` que clona el patrón con closure re-sondeando en cada `resolve()` (molde understand-app), el alta en `pi-session.ts` (forma M1 con `agentDir` + getter `codebaseIndexEnabled`), la entrada `HELP_TOOLS` en `extension.ts` (alias sin colisiones, first-match) y la suite `pattern.test.ts`.

### Changes Required

#### 1. Patrón builtin + sonda + factory

**File**: `src/tools/frida-traffic2api/index.ts`
**Changes**: NEW — `detectTraffic2ApiCapabilities(agentDir, codebaseIndexEnabled)`, `resolveTraffic2ApiScript` (valida eager → resuelve prompts 3 capas → interpola CAPABILITIES → genera script), `TRAFFIC2API_PATTERN` (description/args/meta.moat) y `createFridaTraffic2Api(opts)` con registro idempotente.
...
#### 2. Alta en la sesión principal

**File**: `src/pi-session.ts`
**Changes**: MODIFY — import de `createFridaTraffic2Api` (~línea 122) + entrada en `extensionFactories` tras el bloque understand-app (~líneas 672-683), antes de frida-permission-system, con `agentDir` y getter del toggle (misma forma que M1).
...
#### 3. Entrada HELP_TOOLS

**File**: `src/extension.ts`
**Changes**: MODIFY — entrada data-only (match/file/howTo/label) tras la entrada de understand-app; alias `traffic2api`/`traffic`/`tráfico`/`api`/`openapi`/`har` auditados sin colisión contra TODAS las entradas previas (first-match).
...
#### 4. Suite de tests del patrón

**File**: `test/frida-traffic2api/pattern.test.ts`
**Changes**: NEW — validación eager de modos excluyentes, sonda de capacidades (fixtures `fixtureLensEntry`/`fixtureCodebaseIndexAtPin`, toggle apagado vence), forma del script por modo (8 fases en orden, contrato `network har`, CAPABILITIES interpolada, cortes, ctx.cwd), registro en runtime idempotente sobre el motor.
...
- [x] Type checking passes: `npm run typecheck`
- [x] Test type checking passes: `npm run typecheck:test`
- [x] Pattern suite pasa: `npx vitest run test/frida-traffic2api/pattern.test.ts`
- [x] Suite del Slice 1 sigue verde (regresión): `npx vitest run test/frida-traffic2api/resolver.test.ts`
- [x] Registro exacto en pi-session: `grep -c "frida-traffic2api" src/pi-session.ts` retorna exactamente 2 (import + factory)
- [x] Registro exacto en HELP_TOOLS: `grep -c "frida-traffic2api" src/extension.ts` retorna exactamente 3 (file, howTo, label)
- [x] Motor intacto: `git diff --exit-code src/tools/frida-extensible-workflows/`
- [x] Cero deps nuevas: `git diff --exit-code package.json package-lock.json`
...
## Phase 4: Publicación — docs, how-to, README, cross-links de hermanos

### Overview

Publicación completa en el MISMO commit que el código (D13, lesson c0db889): doc técnica `docs/tools/frida-traffic2api.md`, guía `docs/how-to-frida-traffic2api.md`, fila en la tabla de herramientas del README, actualización de los 4 cross-links de hermanos (los docs de M8/M1 citan M9 como futuro → apuntan al pack real) y fila del roadmap Pista M (pendiente → ✅). El staleness narrativo restante del roadmap queda FUERA del scope (deuda del tooling de release, misma decisión de delivery que M8/M1).
...
**File**: `docs/tools/frida-traffic2api.md`
**Changes**: NEW — molde TEMPLATE + hermano: qué es/no es, 8 fases, args por modo, entregables `docs/api/**`, correlación temporal y seguridad, moat, zona muerta y degradaciones, política de acciones, estructura, registro, customización 3-capas, metadatos, pruebas, ver también.
...
**File**: `docs/how-to-frida-traffic2api.md`
**Changes**: NEW — modelo en 30 s, cuándo sí/no, flujo típico (walk con pre-autenticación / externo), paso a paso (7 pasos), problemas frecuentes con remedios, recetas, qué produce, customización, límites honestos (incl. "no subas raw.har a un repo público").
...
#### 3. Fila en la tabla de herramientas

**File**: `README.md`
**Changes**: MODIFY — fila `frida-traffic2api` en la tabla de herramientas tras la de understand-app (~líneas 212-213), misma forma que las hermanas (una línea: descripción + doc + guía).
...
#### 4. Cross-links de hermanos (M1 understand-app)

**File**: `docs/tools/frida-understand-app.md`
**Changes**: MODIFY — pasaje "No es" (~líneas 28-29): el cruce funcional↔técnico ya no es "M9 futuro", apunta a `frida-traffic2api.md`.
...
**Changes**: MODIFY — pasaje "No" (~líneas 35-36): "capturar tráfico/API (M9)" → link al pack real.
...
#### 5. Cross-links de hermanos (M8 app-walkthrough)

**File**: `docs/tools/frida-app-walkthrough.md`
**Changes**: MODIFY — pasaje "No es" (~líneas 24-26): ingeniería inversa de APIs por tráfico → link al pack real.
...
**Changes**: MODIFY — pasaje "No" (~líneas 32-34): captura de tráfico/contratos → link al pack real.
...
**Changes**: MODIFY — fila 104 de la tabla Pista M: M9 pendiente → entregado (**P2** ✅) con dependencia "M8 ✅ (modo walk), o HAR externo".
...
- [x] Ambos docs nuevos existen no-vacíos: `test -s docs/tools/frida-traffic2api.md && test -s docs/how-to-frida-traffic2api.md`
- [x] Sin TODOs del TEMPLATE: `grep -c "TODO" docs/tools/frida-traffic2api.md docs/how-to-frida-traffic2api.md` retorna 0 en ambos
- [x] Fila README exactamente una: `grep -c "frida-traffic2api" README.md` retorna exactamente 1
- [x] Los 4 cross-links de hermanos apuntan al pack real: `grep -l "traffic2api" docs/tools/frida-understand-app.md docs/how-to-frida-understand-app.md docs/tools/frida-app-walkthrough.md docs/how-to-frida-app-walkthrough.md | wc -l` retorna 4
- [x] Roadmap marca M9 entregado: `grep "M9.*frida-traffic2api" docs/roadmap-extensiones.md | grep -c "✅"` retorna exactamente 1
- [x] Cero deps nuevas: `git diff --exit-code package.json package-lock.json`
...
## Phase 5: e2e sobre el motor real — mock network har, HARs externos, moat, degradaciones

### Overview

Integración end-to-end sobre el motor real (`runWorkflowInStore`) con doble mock: (1) binario `agent-browser` falsificado en PATH con app demo determinista de 5 pantallas y rama `network har` según el contrato REAL del binario (D15: envelope con refs MAPA `{e1:{name,role}}` keys sin "@", path SOLO en stop, `get url/title` tipado); (2) spawner mock por anclas de runtime context. 9 tests: walk feliz (join temporal FAKE_DATE, 422 documentado, colapso {id}, scrub, frontera clasificada), corte budget con checkpoint manual, corte wall-clock, externo devtools sin hermanos, externo mitmproxy con docs M8 (grafo derivado-de-m8), HAR vacío/walk cross-origin, juez FAIL no-abortivo, sesión muerta, determinismo. **Pre-requisito (D15)**: smoke manual del binario real ANTES de implementar — el mock deriva del contrato observado, no supuesto (lesson 30ef616).

### Changes Required

#### 1. Suite e2e sobre el motor real

**File**: `test/frida-traffic2api/e2e.test.ts`
**Changes**: NEW — doble mock (BROWSER_MOCK con rama network har + FAKE_DATE_MOCK compartido, makeSpawn por anclas), fixtures (fixtureRepoServer Express, devtoolsHar/mitmproxyHar inline con Authorization en TODAS las entries, fixtureM8Docs), tipos del inventario y 9 tests integración.
  ...
  if (prompt.includes("## Endpoints observados")) {
   ...
   if (sinCandidatas) {
    ...
    return {
     ...
     toolsUsed: [],
     degradations: [],
     summary: "sin candidatas de zona muerta (degradación registrada por el script)",
    };
   }
   return {
    matrix: externo
...
#### 2. Fix cascade al Slice 2 (ya incorporado en el fence de la Fase 2)

**File**: `src/tools/frida-traffic2api/workflow.ts`
**Changes**: MODIFY — fix cascade ratificado en el micro-checkpoint del Slice 5 del design: `REFS_BY_STEP_SOURCE` lee el mapa real de refs del contrato del binario (keys sin "@", más tolerancia de array) vía `toRefList`; el consumo per-screen normaliza `"@e1"`/`"e1"` en ambas direcciones (`ref.charAt(0) === "@"`); la falla del extractor registra degradación con evidencia y `navegacion.md` reporta "no evaluable" (nunca frontera vacía silenciosa). **NOTA**: el Architecture del design es el estado FINAL — el fence de la Fase 2 ya contiene este cascade completo; esta entrada documenta la procedencia (el design la lista como MODIFY del Slice 5) y los AV de esta fase lo verifican con grep. Sin él, con el contrato real del binario la frontera quedaría silenciosamente vacía.
...
- [x] Type checking passes: `npm run typecheck`
- [x] Test type checking passes: `npm run typecheck:test`
- [x] e2e suite pasa (9 tests): `npx vitest run test/frida-traffic2api/e2e.test.ts`
- [x] Regresión de slices previos verde: `npx vitest run test/frida-traffic2api/resolver.test.ts test/frida-traffic2api/pattern.test.ts`
- [x] El mock reproduce el contrato del binario (D15): `grep -Ec "har (start|stop)" test/frida-traffic2api/e2e.test.ts` retorna >= 3 y `grep -c "startedDateTime" test/frida-traffic2api/e2e.test.ts` retorna >= 4
- [x] Snapshots del mock en forma de mapa (contrato real): `grep -c "refs_for" test/frida-traffic2api/e2e.test.ts` retorna >= 2
- [x] Fix cascade presente en workflow.ts: `grep -c "toRefList" src/tools/frida-traffic2api/workflow.ts` retorna >= 2 y `grep -c 'ref.charAt(0) === "@"' src/tools/frida-traffic2api/workflow.ts` retorna 1
- [x] Fixtures HAR externos inline (devtools+mitmproxy): `grep -c "creator" test/frida-traffic2api/e2e.test.ts` retorna >= 3
- [x] Cero deps nuevas: `git diff --exit-code package.json package-lock.json`
- [x] Motor intacto: `git diff --exit-code src/tools/frida-extensible-workflows/`
...
## Phase 6: Validación typebox de openapi.json + baseline final

### Overview

Slice terminal: `openapi-schema.test.ts` con el criterio de aceptación host-side del FRD (D7 capa 2) — schema TypeBox del OpenAPI 3.1 mínimo que emite `SPEC_BUILDER_SOURCE`, validando fixtures del agregador (sabor devtools/mitmproxy), anti-fixtures (el schema tiene dientes: rechaza 3.0.0, paths ausente, operación sin responses, info sin version) y el ARTEFACTO REAL de una corrida e2e compacta en modo externo sobre el motor real (sin mock de navegador: el modo externo jamás llama agent-browser). Cierra con la baseline completa del proyecto (`npm run typecheck && npm run typecheck:test && npm test`).

### Changes Required

#### 1. Suite typebox de validación de openapi.json

**File**: `test/frida-traffic2api/openapi-schema.test.ts`
**Changes**: NEW — schema TypeBox (`OpenApi31Schema`: openapi "3.1.0" literal, info, servers, paths→method→{summary,responses,requestBody?}), `expectValid` con diagnóstico `instancePath` (lección typebox 1.1.38), fixtures devtools/mitmproxy, 4 anti-fixtures, mini HAR + corrida e2e compacta en modo externo con scrub/colapso verificados sobre el artefacto real.
...
- [x] Type checking passes: `npm run typecheck`
- [x] Test type checking passes: `npm run typecheck:test`
- [x] openapi-schema suite pasa (7 tests): `npx vitest run test/frida-traffic2api/openapi-schema.test.ts`
- [x] Regresión de suites previos verde: `npx vitest run test/frida-traffic2api/resolver.test.ts test/frida-traffic2api/pattern.test.ts test/frida-traffic2api/e2e.test.ts`
- [x] Baseline completa del proyecto (slice terminal): `npm run typecheck && npm run typecheck:test && npm test`
- [x] Validación typebox en el dialecto del repo: `grep -c "typebox" test/frida-traffic2api/openapi-schema.test.ts` retorna >= 2
- [x] Valida fixture Y artefacto real de corrida: `grep -c "runWorkflowInStore" test/frida-traffic2api/openapi-schema.test.ts` retorna >= 2 (import + corrida)
- [x] Anti-fixtures con dientes: `grep -c "toBe(false)" test/frida-traffic2api/openapi-schema.test.ts` retorna exactamente 4
- [x] Cero deps nuevas: `git diff --exit-code package.json package-lock.json`
- [x] Motor intacto: `git diff --exit-code src/tools/frida-extensible-workflows/`
...
- Type checking: `npm run typecheck` (todas las fases) y `npm run typecheck:test` (fases con tests)
- Suites del pack por fase: `npx vitest run test/frida-traffic2api/resolver.test.ts`, `pattern.test.ts`, `e2e.test.ts`, `openapi-schema.test.ts`
- Baseline completa del proyecto en la fase terminal (propiedad de `validate` a nivel whole-plan): `npm run typecheck && npm run typecheck:test && npm test`
- Gates estructurales por fase: grep-counts exactos, `git diff --exit-code package.json package-lock.json` (cero deps) y `git diff --exit-code src/tools/frida-extensible-workflows/` (motor intacto)

### Manual Testing Steps

1. Gates `test -s` tras cada fase productora + reintento informado UNA vez (lessons d203630/619d9e7): `raw.har`, `requests.jsonl`, `openapi.json`, `matriz.md`, `navegacion.md`, `README.md`, `inventory.json`.
2. `mkdir -p` de todos los directorios al arranque (lesson bffd6f1).
3. Smoke real del binario ANTES del e2e (lesson 30ef616, D15): `network har start --content all` → `open` → interacción → `network har stop <path>` → inspección del HAR (formato 1.2, `startedDateTime` por entry, hrefs en `data.snapshot`); el mock reproduce el contrato observado.
4. Grep-counts exactos: `grep -n "frida-traffic2api" src/pi-session.ts` = 2 líneas (import + factory); `grep -n "frida-traffic2api" src/extension.ts` = 3 (file, howTo, label); alias HELP_TOOLS sin colisiones (first-match contra la lista completa).
5. outputSchema en TODOS los agentes que devuelven datos (lessons 376aabe/5d99652).
6. Verde en suite ≠ correcto (lección #91): el e2e ejercita el wiring completo sobre el motor real.
7. Commit atómico pack+wiring+docs+tests; esperar el reformato automático de pi-lens post-commit (patrón c4cbe02).
8. Post-implementación (fuera del código): corrida piloto sobre app legacy open source (issue #135) con números y entregables enlazados.
...
_Revisión post-finalización independiente por artifact-code-reviewer y artifact-coverage-reviewer subagents. Hallazgos triados en Step 5. Los 4 hallazgos de código tienen causa raíz en el `## Architecture` del design (el plan copió verbatim) — el caveát de Step 5 aplica: fix upstream o plan-local con design follow-up._

| source | plan-loc | codebase-loc | severity | dimension | finding | recommendation | resolution |
| -------- | ----------------- | --------------------------- | ---------- | --------------------- | --------- | ---------------- | ------------------ |
| code | Phase 2 §workflow.ts fence, synthesize/graph del script generado (plan L1679, L1686, L1845-1853, L1897, L1901, L1940, L1963-1964) | src/tools/frida-understand-app/workflow.ts:604 | blocker | actionability | Secuencias `\`` (backslash doble + backtick SIN escapar) en 9 líneas y backticks bare en la rama HAR (L1901), dentro del template literal externo:`\` emite backslash y el backtick que sigue TERMINA el template → "Unterminated template literal" en `npm run typecheck`; bloquea Fases 2-6. El molde understand-app usa single-escape `\`` | Reemplazar cada `\`` por`\``(9 líneas) y escapar los backticks bare de L1901. La L1031 (`lit()`, función TS real fuera del template) es CORRECTA y no se toca | applied (plan-local; design follow-up: .rpiv/artifacts/designs/2026-08-26_13-25-20_traffic2api-m9.md) — single-escape `\`` en las 9 líneas + backticks bare de la rama HAR, molde understand-app:604 |
| code | Phase 5 §e2e.test.ts fence, BROWSER_MOCK rama `network` (plan L3497-3498) | <n/a> | blocker | actionability | `esac` duplicado: el segundo cierra prematuramente `case "$1"` → el `;;`/`*)` siguientes quedan huérfanos → bash parse error en cada invocación del mock → los 9 tests e2e fallan | Eliminar el `esac` duplicado (L3498): `*)` cierra `case "$2"`, un `esac` cierra `case "$1"`, el `;;` termina la rama `network)` y el `esac` final cierra `case "$cmd"` | applied (plan-local; design follow-up: .rpiv/artifacts/designs/2026-08-26_13-25-20_traffic2api-m9.md) — `esac` duplicado eliminado (verificado con bash -n) |
| code | Phase 5 §e2e.test.ts fence, test 4 (plan L4247-4299) vs rama matrix de makeSpawn (L3831-3855) | <n/a> | blocker | actionability | Test 4 corre SIN `fixtureRepoServer` → matrixBlocks lleva "VACÍO (degradación registrada)" (L1770) → el mock toma la rama `sinCandidatas` (`matrix: []` + una degradación extra) → asserts insatisfacibles: `matrixRows(2)` y `apiSinUi(1)` serían 0, `degradations(5)` sería 6; solo `deadZone(0)` es consistente | Alinear la rama `sinCandidatas` del mock: devolver la matriz externo (2 filas + `apiSinUi: 1`) con `deadZone: []` y `degradations: []` (el script ya registra la degradación "no enumerable") — preserva la intención del test 4 (matriz 2 columnas + huérfanos) y mantiene el test 3 verde | applied (plan-local; design follow-up: .rpiv/artifacts/designs/2026-08-26_13-25-20_traffic2api-m9.md) — rama `sinCandidatas` devuelve matriz externo condicional + `deadZone`/`degradations` vacías |
| code | Phase 4 §docs/tools/frida-traffic2api.md fence, tabla args (plan L2725) | <n/a> | concern | code-quality | La fila documenta default `0` con "`0` = sin tope", pero `parseMaxMinutes` (Fase 2) RECHAZA un `maxMinutes: 0` explícito ("entero 1-240 u omitirse") y pattern.test (Fase 3) lo aserta — la doc publicada documenta un valor que el workflow rechaza | Reformular la fila: default "— (omitido)" con "omitido = sin tope; valores aceptados 1-240" | applied (plan-local; design follow-up: .rpiv/artifacts/designs/2026-08-26_13-25-20_traffic2api-m9.md) — fila: default «— (omitido)», «omitido = sin tope; valores 1-240» |
| coverage | ## Testing Strategy ↔ design Key Discoveries §9 (lecciones como comentarios) | .rpiv/artifacts/designs/2026-08-26_13-25-20_traffic2api-m9.md## Current State Analysis | suggestion | verification-coverage | Lección "copiar las cadenas de comentarios a los puntos equivalentes" parcialmente cubierta: `bffd6f1` y `d397401` SÍ tienen espejo como comentarios en el fence de la Fase 2, pero `d203630`/`619d9e7` (gates `test -s` + reintento informado UNA vez) NO aparecen como comentario en los gates del fence de workflow.ts — solo se citan en Testing Strategy | Añadir el comentario de lección encima del bloque de gates del fence de Fase 2 (p. ej. `// Gates test -s por escritor + reintento informado UNA vez (lessons d203630/619d9e7)`) o un bullet AV con grep | applied (plan-local; design follow-up: .rpiv/artifacts/designs/2026-08-26_13-25-20_traffic2api-m9.md) — comentario de lección sobre el primer gate del fence de Fase 2 |

_Tally: 3 blockers, 1 concern, 1 suggestion._

## Developer Context

(Estructura de herencia: límites de fase 1:1 de los slices del design; Success Criteria verbatim del design `## Slices`; código verbatim del design `## Architecture` — extracción byte-exacta por rangos de línea. Paralelismo: 1→2→3→4 secuenciales; 5 tras 3 en paralelo con 4; 6 tras 5. Pre-requisito D15: smoke del binario antes de la Fase 5.)

**Step 5 (triaje 2026-08-26)**: 5/5 hallazgos del Plan Review aplicados plan-local (B1 escapes del template, B2 `esac` duplicado, B3 rama VACÍO del mock, C1 fila `maxMinutes` del doc, S1 comentario de lección en gates). El design queda como follow-up para sincronizar sus fences (caveát del skill: causa raíz upstream).

## References

- Design: `.rpiv/artifacts/designs/2026-08-26_13-25-20_traffic2api-m9.md`
- Research: `.rpiv/artifacts/research/2026-08-26_13-00-12_traffic2api-m9.md`
- FRD (discover): `.rpiv/artifacts/discover/2026-08-26_12-44-58_traffic2api.md`
- Issue #135 — M9: frida-traffic2api (GitHub)
