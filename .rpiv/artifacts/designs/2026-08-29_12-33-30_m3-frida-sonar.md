---
date: 2026-08-29T12:33:30-0600
author: Edgar F. Fuentes Perea
commit: 3002bae
branch: main
repository: frida-code
topic: "M3 frida-sonar — quality gate de código estático local con veredicto por familias, diff por turno y tendencia (tab «Sonar» del SettingsHub, patrón M2)"
tags: [design, frida-sonar, settings-hub, pi-lens, quality-gate, turn-end, snapshot]
status: ready
parent: .rpiv/artifacts/research/2026-08-29_11-55-22_m3-frida-sonar.md
last_updated: 2026-08-29T12:33:30-0600
last_updated_by: Edgar F. Fuentes Perea
---

# Design: M3 frida-sonar — quality gate local con veredicto por familias, diff por turno y tendencia

## Summary

`frida-sonar` replica el patrón M2 punta a punta: una lib host pura en `src/sonar/*` (Node-only, sin vscode) que consolida el bus crudo `pilens:diagnostics` (segundo consumidor fan-out junto a `mergeLens`), computa un veredicto PASS/WARN/FAIL por familias desde los agregados `details` de `lens_diagnostics mode=all` (cache-only instantáneo, invocado async fire-and-forget en `case "turn_end"`), deriva el diff `+N −M` por `path:line:ruleId|tool` contra el snapshot del turno anterior, y persiste un snapshot versionado `frida-sonar-gate/v1` por proyecto en globalStorage (JSON único con poda FIFO). La UI es un tab «Sonar» del SettingsHub + un badge en la franja del chat, ambos alimentados por un único mensaje `sonar_gate_state` con reducer replace plano; el gate completo (`mode=full + refreshRunners=all`, timeout interno 5 min) corre sólo bajo demanda con degradaciones honestas. Contrato congelado contra el pi-lens 3.8.72 de `~/.frida` (runtime real por ADR-0010) con parse lenient.

## Requirements

Del FRD `.rpiv/artifacts/discover/2026-08-29_11-35-42_m3-frida-sonar.md` (issue #144, Refs #144 en commits):

- **FR-1**: Tab nuevo «Sonar» en el SettingsHub con patrón M2 exacto: lib pura `src/sonar/*`, estado host `sonarState` + broadcast `{type:"sonar_gate_state"}`, case delgado `case "sonar_gate"`, componente `SonarTab`, reducers en `webview/store.ts`, prefijo CSS `.sn-`, tests emparejados `test/sonar-lib.test.ts` + `test/sonar-tab.test.ts`.
- **FR-2**: En `case "turn_end"` el host computa async (fire-and-forget, nunca bloquea el fin de turno) un snapshot de gate: consolidación del bridge + `lens_diagnostics mode=all`, veredicto y diff vs. snapshot del turno anterior (issues nuevas `+N` / resueltas `−M` por archivo+línea+regla).
- **FR-3**: Veredicto por familias bloqueantes: FAIL con ≥1 hallazgo error-severity (errores LSP blocking, secrets, CVEs); WARN con warnings (LSP, complejidad, duplicación, ciclos, dead-code); PASS en otro caso.
- **FR-4**: Familia sin datos (no corrió, fría, fallida) = «no disponible» con causa (`coldRunners` de mode=full cuando existan; `failedAnalyzers`/`coldReasons` NO existen en 3.8.72 — parse lenient, omitir si no llegan) — nunca PASS silencioso.
- **FR-5**: Umbrales en `contributes.configuration`: `frida.sonar.maxWarnings` (default 0), `frida.sonar.disabledFamilies` (array), `frida.sonar.historyLimit` (default 500).
- **FR-6**: Badge de gate por turno en la franja del panel de diagnósticos existente bajo el composer: veredicto + `+N −M` (codicons, sin glifos unicode).
- **FR-7**: Botón «Ejecutar gate completo» que corre `lens_diagnostics mode=full` con `refreshRunners=all`, indicador de progreso, timeout 5 min (parcial honesto), veredicto enriquecido con desglose por familia y degradaciones.
- **FR-8**: El tab renderiza veredicto actual, diff del último turno, issues actuales por familia con `file:line` (clic → abrir archivo en la línea, reuso del `case "open_file"` existente) y tendencia (serie del snapshot histórico).
- **FR-9**: Snapshot persistido en globalStorage por proyecto (slug del cwd, patrón workflows `<encodeCwd>`), schema versionado `"frida-sonar-gate/v1"`, append de una entrada por turno `{ts, verdict, countsPorFamilia, diff}` + poda FIFO al `historyLimit`.
- **FR-10**: Si pi-lens no está cargado (`isLensLoaded()===false`), el tab degrada con «pi-lens no disponible» + hint accionable — sin crashear.
- **NFR Security**: hallazgos de gitleaks NUNCA muestran contenido del secreto (sólo `file:line` + regla); el snapshot persistido NO contiene contenido de código fuente (ni `message` de diagnóstico) — sólo conteos y referencias.
- **NFR UX**: UI es-MX, codicons, variables `--vscode-*`, prefijo `.sn-` conforme a `docs/webview-ui-styles.md`.
- **NFR Reliability**: fail-soft en todas las dependencias con degradación visible y causa (patrón honestidad M10 #139).

## Current State Analysis

### Qué existe

- **Patrón M2 completo y fresco** (commits e6ff92d→fca5e6f, 2026-08-29, sin bugfixes post): lib pura con unión de estados honestos (`src/project-map/lens-project-report.ts:86-210`), `let pmState` host + convención #111 (`src/extension.ts:631`), broadcaster 2 líneas (`postProjectMapState` `src/extension.ts:678`), case delgado (`:3575`), re-post en `webview_ready` (`:2646`), registro de tab 3 toques (`webview/components/SettingsHub.tsx:23,35,465`), espejo de tipos unión discriminada («builds separados», `webview/types.ts:583-704`), reducer con merge (`webview/store.ts:616`), tests emparejados (`test/project-map-lib.test.ts`, `test/project-map-tab.test.ts`).
- **Flujo D16 de diagnósticos**: bus `pilens:diagnostics` → `createLensDiagnosticsBridge` (`src/lens-diagnostics-bridge.ts:71-86`, registrado en `src/pi-session.ts:997`) → callback único `opts.onLensDiagnostics` (`src/pi-session.ts:292`) → `mergeLens` (`src/extension.ts:1068`, reduce a SOLO conteos por archivo, full-replace por archivo) → `lensAccum` → `flushLens` (`:1104`, publica `LensSummary`, null si vacío). Limpieza en `turn_start` (`:2420`), `newSession` y `switchSession`.
- **Hook `turn_end`** ya existe (`src/extension.ts:2445-2451`: `flushLens()` + `scheduleWorkspaceRefresh()`), 100% síncrono hoy; el precedente async no-bloqueante en el MISMO archivo es el debounce 500ms de `scheduleWorkspaceRefresh` (`:2340`) y el patrón `void p().catch(...)` de `bootstrapSession` (`:2311`).
- **Detector pi-lens**: `isLensLoaded()` (`src/extension.ts:1047`) recorre `resourceLoader.getExtensions().extensions` buscando `lens_diagnostics` en `tools.keys()` — mismo recorrido que usará la invocación programática.
- **`case "open_file"`** existente (`src/extension.ts:3613`) con guard `safeResolveWithin` — reuso directo para FR-8.
- **Moldes de persistencia**: `encodeCwd` (`src/tools/frida-workflow/audit.ts:22-25`), JSON merge + write 0600 no-throw (`src/pilens-config.ts:33-53`), schema versionado aditivo (`src/usage/report-schema.ts:12`), JSON único load→mutate→write (`names.json`).
- **Molde settings en vivo**: `readGatePatterns()` (`src/settings.ts:118`) getter sin caché ↔ evaluador pura `evaluate(input)` (`src/tools/frida-permission-system/policy.ts:98`) con patrones como parámetro.

### Qué falta

- Todo `src/sonar/*` (consolidación per-issue, veredicto, diff, snapshot store), settings `frida.sonar.*`, hook async en `turn_end`, fan-out del bus, seam webview (mensaje/reducer/tipos), tab «Sonar», badge, gate completo.

### Key Discoveries

- **El contrato del externo miente** (`src/lens-diagnostics-bridge.ts:23-32`): la interface `LensDiagnostic` declara `range`/`code`/`source`/`semantic` que el bus NUNCA envía. Payload real verificado en `~/.frida/npm/node_modules/pi-lens/dist/index.js:56918` (`toPilensDiagnosticEntry`): `{severity, message, tool, ruleId?, line?, col?, fixable?}`. Clave natural del diff: `path + (line ?? "?") + (ruleId ?? tool)` — replicar la convención `diagnosticDedupKey` de pi-lens (`dist/tools/lens-diagnostics.js:551`: `path.resolve(filePath):(line ?? "?"):normalizeRuleForDedup(rule ?? tool)`, con `ast-grep:` prefix y `-js` suffix stripping).
- **Cap del bus**: `MAX_DIAGNOSTICS_PER_FILE_EVENT = 12` diagnósticos/archivo/evento (`dist/clients/diagnostics-publish.js:68`), `seq` contador monótono global (`:73`) — «mayor seq gana» ante out-of-order; semántica full-replace por archivo (`diagnostics: []` = limpio explícito). El diff vía bus SUB-CONTIENE archivos con >12 hallazgos (flag `truncated` viaja).
- **`details` de mode=all** (verificado en runtime 3.8.72, `dist/tools/lens-diagnostics.js:1138-1266`): `{mode:"all", filesWithIssues, totalBlocking, totalErrors, totalWarnings, staleDropped, filesChecked}` — **SIN desglose por familia y SIN per-issue**. `filesChecked` sólo aparece en la rama sin issues (`:1186`) — `filesChecked===0` ⇔ «No files diagnosed yet this session» = estado `no-data` honesto.
- **mode=full details añade**: `coldRunners` (SIEMPRE presente, incluso vacío — `:1040-1049`), `timedOut` (sólo si abortó por wall-clock, `:1085-1108`), `partial: "aborted"` (`:914`), `analyzerTimingsMs`, `analyzersAborted`, `analyzersAbortedIds`. `failedAnalyzers`/`coldReasons` NO existen en 3.8.72 (0 matches) — parse lenient.
- **Asimetría del full-scan** (hallazgo adicional del design, verificado `dist/tools/lens-diagnostics.js:880-905`): el barrido LSP de mode=full SE reconcilia al widget-state (`reconcileScanDiagnostics` :883 → visible en mode=all posteriores), pero los hallazgos de los analyzers pesados (jscpd/madge/knip/gitleaks/govulncheck/trivy/dead-code) se pliegan SÓLO al render del full (`foldExtraDiagnosticsIntoSnapshot` :746) — nunca viajan per-issue por el bus ni quedan en widget summaries. Consecuencia: sus totales sólo afectan el veredicto del gate completo; el desglose per-issue del tab cubre los archivos editados de la sesión.
- **Blocking ⇔ error** (`:480,:522`): `semantic === "blocking"` ⇔ severity error — FAIL = `totalBlocking > 0 || totalErrors > 0`.
- **Timeout interno YA es 300s**: `FULL_SCAN_WALL_CLOCK_MS` default 300_000 env-tunable (`:49-52`); al abortar RETORNA parciales (`details.timedOut`, texto «results are partial») — el host NO necesita `Promise.race`.
- **Tool stateful**: la tool viva de la sesión (`resourceLoader.getExtensions()` → `e.tools.get("lens_diagnostics")`) comparte caches/LSP con los turnos; una factory fresca arrancaría fría y duplicaría servers. `loadUpstreamTools` (`src/extension.ts:3352` zona) está cableado a codebase-index — NO reutilizable.
- **Badge ≠ extensión de LensSummary**: los guards del panel (`webview/components/LensDiagnostics.tsx:20,22`) ocultan el caso PASS (totales 0) y el host manda `summary: null` con `lensAccum` vacío (`flushLens`) — extender `LensSummary` rompe por construcción. Solución: mensaje propio `sonar_gate_state` + campo `sonar?` en State (molde `lensStatus`).
- **Regla #126**: todo mensaje nuevo de estado DEBE caer al `dispatch(msg)` (`webview/App.tsx:126-147` sólo intercepta `open_settings`/`open_models` sin despachar) — `sonar_gate_state` cae solo, sin cambios en App.tsx para el estado (sí para el badge).
- **webview_ready NO re-postea lens summary** — `sonar_gate_state` DEBE re-postearse (molde `postProjectMapState` en `:2646`).
- **Guarda `dist-bundle-integrity`**: todo commit que toque `webview/**` debe commitear fuente + `dist-webview/` juntos (lección M2); `npm run build` reconstruye ambos.
- **Flakiness preexistente del baseline**: e2e traffic2api ENOENT ~50% bajo `npm test` completo — atribuir con `git diff --stat` antes de asumir que el tab nuevo rompió algo.

## Scope

### Building

- Lib pura `src/sonar/gate.ts` + `src/sonar/snapshot-store.ts` (Node-only, sin vscode, SIEMPRE resuelve).
- Consolidador del bus crudo como segundo consumidor (fan-out junto a `mergeLens` en los 2 wiring points, wrapper en el host — sin tocar `pi-session.ts` ni el bridge).
- Snapshot por turno en `turn_end`: veredicto desde details de mode=all (tool viva), diff vs. issues persistidas, append FIFO, persistencia por proyecto.
- Mensaje único `sonar_gate_state` (In) + `sonar_gate` (Out) + campo `sonar?` en State + reducer replace plano + re-post en `webview_ready`.
- Tab «Sonar» del SettingsHub: veredicto, diff, issues por familia (clic → `open_file`), tendencia, degradaciones, estados honestos (not-installed/no-data/error), settings activos visibles.
- Badge `SonarGateBadge` en la franja del chat junto al panel lens existente (estado propio; auto-ocultamiento del panel intacto).
- Gate completo bajo demanda: botón del tab, tool viva `mode=full + refreshRunners=all`, progreso vía `onUpdate` (4º arg), busy/busySince #111, degradaciones honestas (coldRunners/timedOut/partial).
- Settings `frida.sonar.maxWarnings` / `frida.sonar.disabledFamilies` / `frida.sonar.historyLimit` en `contributes.configuration` + `readSonarConfig()` getter en vivo.
- Tests emparejados: `test/sonar-lib.test.ts` (Node puro), corolario reducer en `test/webview-store.test.ts`, `test/sonar-tab.test.ts` (renderToStaticMarkup + post=vi.fn()).
...
- Interceptación de `tool_execution_end` cuando el agente invoca `lens_diagnostics` — redundante: el recompute de `turn_end` (posterior en el mismo turno) lee el mismo widget-state; una sola fuente de disparo.
...
- Parser del texto de `formatAllMode`/`formatFullMode` — descartado en research (frágil: elipsis 50/archivo, `L?`, marcadores stale).

## Decisions

### D1: Contrato congelado contra pi-lens 3.8.72 de `~/.frida` (runtime), parse lenient

**Ambiguity**: el FRD citó el contrato contra `~/.pi/agent` (pi-lens 4.1.2) pero el runtime de Frida carga `~/.frida` (3.8.72, ADR-0010 — `src/pi-session.ts:1226-1232`, moat `src/tools/frida-extensible-workflows/moat-factories.ts:59-66`), donde `failedAnalyzers`/`coldReasons` NO existen (`coldRunners` sí).
**Explored**: (A) congelar/fumigar contra 3.8.72 con parse lenient — igual que M2; (B) contra 4.1.2 (contrato del FRD original).
**Decision**: (A) 3.8.72 runtime + lenient — verificado en vivo durante design (todas las citas `dist/tools/lens-diagnostics.js` de este documento son del 3.8.72 instalado). FR-4: `coldRunners` se reporta; `failedAnalyzers`/`coldReasons` se omiten si no llegan («cuando existan», literal del FRD). 4.1.2 queda como referencia de evolución. *(Heredada del checkpoint de research; re-verificada en design.)*

### D2: Detalle per-issue del bus crudo consolidado; veredicto de los agregados

**Ambiguity**: el diff `archivo+línea+regla` necesita detalle per-issue que no existe estructurado (mode=all es texto cap 50 + agregados).
**Explored**: (A) segundo consumidor fan-out del bus crudo, consolidado por path+mayor-seq, clave `path:line:ruleId|tool`, tolerando `line` ausente (`?`) y cap 12 (`truncated` viaja) — veredicto bloqueante de `details`; (B) parser del texto de mode=all (frágil); (C) híbrido.
**Decision**: (A) — fan-out `(p) => { mergeLens(p); sonarOnLens(p); }` en los DOS wiring points (`src/extension.ts:1249` ensureSession y `:5694` switchSession); el callback único `onLensDiagnostics` (`src/pi-session.ts:292`) no cambia (molde más contenido: wrapper en el host, sin tocar bridge ni pi-session). La consolidación replica la semántica full-replace por archivo y «mayor seq gana» del productor. *(Heredada del checkpoint de research.)*

### D3: Veredicto FAIL/WARN/PASS sobre totales de details, con exclusión best-effort

**Decision** (checkpoint de design): `computeVerdict` recibe `{filesChecked, filesWithIssues, totalBlocking, totalErrors, totalWarnings}` + consolidado per-issue + umbrales:

- **FAIL** si `totalBlocking > 0 || totalErrors > 0` (evidencia: blocking ⇔ error, `lens-diagnostics.js:480,522`; las familias bloqueantes — errores, secrets, CVEs — jamás deshabilitables).
- **WARN** si `warnings efectivas > maxWarnings`, donde `warnings efectivas = totalWarnings − min(Σ warnings de familias deshabilitadas según el consolidado per-issue, totalWarnings)` (clamp ≥0). Si el bus sub-cuenta, resta menos → el gate falla hacia el lado estricto (fail-safe). Efecto nulo para knip/jscpd/madge/gitleaks/trivy (nunca viajan per-issue) — documentado en el tab.
- **PASS** en otro caso.
- **no-data** honesto si `filesChecked === 0 && filesWithIssues === 0 && consolidado vacío` (evidencia `:1186`: filesChecked sólo en rama sin issues; texto «No files diagnosed yet this session») — nunca PASS silencioso sobre vacío.
- Degradado (gate completo): `timedOut || partial` → veredicto con flag `degraded` + causas; `coldRunners` no vacío → familias «no disponible» con causa (FR-4/#533: «absence of findings is NOT a clean verdict»).

### D4: Mensaje único `sonar_gate_state` + reducer replace plano

**Decision** (checkpoint de design): un solo InMessage lleva TODO el estado Sonar (veredicto del turno, diff, issues ≤400 con flag de truncado, tendencia ≤50 puntos, degradaciones, gate completo en curso con busy/busySince/progress, settings activos, `lensLoaded`). Ningún campo se llena por un segundo mensaje → replace plano (el merge de M2 `webview/store.ts:616` sólo era necesario porque `shots` llegaba por mensaje aparte). El badge y el tab leen el mismo `state.sonar`.

### D5: Snapshot JSON único por proyecto con FIFO en la escritura

**Decision**: `<globalStorageUri>/sonar/<encodeCwd(cwd)>/snapshot.json` (espejo del patrón workflows `src/tools/frida-workflow/audit.ts:22-25`), shape `{schema: "frida-sonar-gate/v1", cwd, entries: SonarEntry[], issues: SonarIssue[]}`:

- `entries` = `{ts, verdict, degraded?, blocking, errors, warnings, diff: {added, resolved}, countsPorFamilia}` (FR-9) con poda FIFO al `historyLimit` COMO PARTE DEL CONTRATO DE ESCRITURA (load tolerante → push → shift → write `mkdirSync recursive` + `writeFileSync mode 0o600`, no-throw — moldes `pilens-config.ts:33-53` + `names.json`). Ningún JSONL del repo poda → JSON único.
- `issues` = issue-set consolidado ACTUAL (refs: `{key, path, line?, rule?, tool, severity, family}` — **SIN `message`**, NFR secrets) persistido para que el diff sobreviva reloads/reinicios (FR-2 exige «vs. el snapshot del turno anterior»).
- Schema aditivo = sigue v1 (molde `USAGE_REPORT_SCHEMA`, regla «lo aditivo sigue siendo v1; sólo un cambio breaking sube a v2»); guard descriptivo al cargar (mismatch → snapshot vacío honesto, nunca crash).

### D6: Gate completo vía tool viva de la sesión, timeout interno, progreso onUpdate

**Decision**: invocación `resourceLoader.getExtensions().extensions` → `e.tools.get("lens_diagnostics")` → `execute(toolCallId, {mode:"full", refreshRunners:"all"}, undefined, onUpdate, {cwd})` (mismo recorrido que `isLensLoaded()`, `src/extension.ts:1047`; molde de firma: llamada a `t.execute` de codebase-index en el `case "codebase_index_action"`). El timeout de 5 min YA es el default interno de pi-lens (retorna parciales con `details.timedOut`) — sin `Promise.race` host-side. Progreso: 4º argumento `onUpdate` (streaming throttled) → línea de progreso estilo `ciLastLine` + `busy`/`busySince` #111. Veredicto del full: mismos umbrales sobre sus totals + degradaciones (D3).

### D7: Settings getter en vivo ↔ evaluador puro

**Decision**: `readSonarConfig()` en `src/settings.ts` (sección raíz + claves relativas `sonar.*`, SIN caché — patrón `readGatePatterns` `src/settings.ts:118`), consumida fresca en cada cómputo; `computeVerdict`/`diffIssues`/`appendEntry` reciben umbrales/límites como PARÁMETROS de datos (molde `evaluate(input)` `policy.ts:98`). AC «cambiar maxWarnings cambia el veredicto sin recompilar» = dos invocaciones con umbrales literales sobre el mismo fixture.

### D8: Turn_end async fire-and-forget con serialización por cadena de promesas

**Decision**: `case "turn_end"` añade `void computeTurnSnapshot().catch(err => ...)` (patrón `bootstrapSession` `src/extension.ts:2311` — catch ruidoso: console.warn + estado visible, nunca silencio f3112ec). Turnos rápidos consecutivos se serializan con una cadena `let sonarComputeChain = sonarComputeChain.then(...)` (orden de entries FIFO garantizado; sin epochs — cada cómputo es barato y lee el consolidado vigente al ejecutarse). El consolidado NO se limpia por turno (representa el estado conocido de la sesión, como mode=all); SÍ se limpia en `newSession`/`switchSession` (molde `lensAccum.clear()`).

### D9: Estados honestos del tab (unión, molde PmTechnicalState)

**Decision**: `SonarUiState` degrada honesto: `not-installed` (`isLensLoaded()===false`, hint accionable FR-10, SIN botón de reintento), `no-data` (primera carga sin diagnósticos), `error` (hint visible), `ready`. Por familia: sin datos = «no disponible» con causa; deshabilitada = «deshabilitada» (setting). Catch ruidoso en todo el path host.

## Architecture

### src/sonar/gate.ts — NEW

Lib pura del gate: tipos, familias, consolidación del bus, veredicto, diff, parse lenient de details.
...
//   partial:"aborted" (:914). failedAnalyzers/coldReasons NO existen en 3.8.72
...
export interface LensDetailsAggregates {
 ...
 timedOut?: boolean;
 /** "aborted" si el escaneo quedó parcial. */
 partial?: string;
}
...
export function parseLensDetails(details: unknown): LensDetailsAggregates {
 ...
 return {
  ...
  partial: typeof d.partial === "string" ? d.partial : undefined,
 };
}
...
### src/sonar/snapshot-store.ts — NEW

Persistencia del snapshot por proyecto (JSON único, FIFO, 0600, no-throw).
...
### test/sonar-lib.test.ts — NEW

Suite Node-puro de la lib (fixtures honestos del contrato 3.8.72 verificado).
...
describe("sonar lib · computeVerdict (D3)", () => {
 ...
 it("degradado: timedOut/partial → degraded con causas; coldRunners → familias no disponibles", () => {
  const r = computeVerdict({
   details: det({ totalWarnings: 1, timedOut: true, partial: "aborted", coldRunners: ["trivy", "weird-runner"] }),
   issues: [],
...
### src/settings.ts:200-226 — MODIFY

`readSonarConfig()` getter en vivo (molde `readGatePatterns`).
...
### package.json:280 — MODIFY

Claves `frida.sonar.*` en `contributes.configuration.properties` — inserción tras `frida.gates.dangerousCommandSubstrings` (agrupación temática de gates).
...
### webview/types.ts:838-858,1060,1240-1250,1420 — MODIFY

Espejo `SonarUiState`/`SonarIssue`/`SonarFamily` (unión discriminada, builds separados), campo `sonar?` en `State`, `InMessage sonar_gate_state`, `OutMessage sonar_gate`.
...
### webview/store.ts:698-706 — MODIFY

Case reducer `sonar_gate_state` (replace plano) junto a `lens_status`.
...
### test/webview-store.test.ts:153 — MODIFY

Corolario: «sonar_gate_state llena state.sonar» (replace plano, molde línea 114).
...
Estado host `sonarState` + `postSonarState()`, case `sonar_gate`, fan-out del bus en los 2 wiring points, hook `turn_end`, `runFullGate`, re-post en `webview_ready`, cleanup en `newSession`/`switchSession`.

> **Porción Slices 2–3 + 6 (seam #126 + snapshot por turno + gate completo bajo demanda FR-7)** — fence final fusionado.

```typescript
// ══ M3 (#144): frida-sonar — SEAM + SNAPSHOT POR TURNO + GATE COMPLETO (Slices 2-3, 6) ══

// ── (1) Imports — en el import existente de "./settings" se añade `readSonarConfig`;
//        se importan tipos y funciones de la lib pura (Slice 1) ──
import {
 anyTruncated,
 buildTurnSnapshot,
 flattenIssues,
...
### webview/components/SonarTab.tsx — NEW

Tab «Sonar» del SettingsHub (contrato `{state, post}`, carga al montar, veredicto/diff/familias/tendencia; el gate completo FR-7 llega en el Slice 6 sobre este archivo).
...
### webview/components/SettingsHub.tsx:23,35,465 — MODIFY

Registro del tab: unión `SettingsTab` + entrada `TABS` + render condicional (patrón M2, 4 toques).
...
Familia `.sn-*` del tab (añadida AL FINAL del archivo, tras la familia `.pm-*` y su media query reduced-motion; badge de la franja del chat fusionado — Slice 5).
...
Familia `.sn-*` documentada en la guía canónica + botones nuevos al inventario auditado (regla del propio doc: «Si agregás un botón nuevo, incluílo en este inventario y verificá su hover contra button:hover» — precedente M2 con `.pm-*`).
...
### test/sonar-tab.test.ts — NEW

Tab test: renderToStaticMarkup + `post=vi.fn()` con `React.createElement` (sin JSX), estados honestos y aserciones negativas (molde `project-map-tab.test.ts`; los efectos NO corren — carga al montar probada en vivo).
...
### webview/components/SonarGateBadge.tsx — NEW

Badge de gate en la franja del chat (veredicto + `+N −M`, codicons, estado propio). Sólo `status === "ready"` renderiza (D9/Desired End State #5); clic abre el tab «Sonar» (callback inyectado por App — molde `onOpenProviders`). Reúsa `VERDICT_META` del tab (Slice 4, merge de 1 palabra para exportarlo) y las clases `.sn-diff*`.
...
### webview/App.tsx:760 — MODIFY

Montaje del badge junto a `<LensDiagnostics>` en la franja footer; clic → hub directo al tab «sonar».
...
### Slice 1: Lib pura src/sonar/* — veredicto, diff, consolidación y snapshot

**Files**: `src/sonar/gate.ts`, `src/sonar/snapshot-store.ts`, `test/sonar-lib.test.ts`

#### Automated Verification

- [ ] Type checking pasa: `npm run typecheck`
- [ ] Typecheck de tests pasa: `npm run typecheck:test`
- [ ] Suite nueva en verde: `npx vitest run test/sonar-lib.test.ts`
- [ ] Lib Node-pura: `grep -c "from \"vscode\"" src/sonar/gate.ts src/sonar/snapshot-store.ts` devuelve 0 en ambos
- [ ] Schema congelada presente: `grep -c "frida-sonar-gate/v1" src/sonar/gate.ts` >= 1

#### Manual Verification

- [ ] Fixtures del bus reproducen el shape REAL 3.8.72 (severity dual string|número, sin `range:`/`semantic:` por diagnóstico) — revisados contra `~/.frida/npm/node_modules/pi-lens@3.8.72`
- [ ] AC literal del FRD cubierto por test dedicado: fixture con `historyLimit+1` entradas queda podado FIFO a `historyLimit` (la más vieja evicted)

### Slice 2: Seam completo #126 — settings + mensajes + reducer + case

**Files**: `src/settings.ts`, `package.json`, `webview/types.ts`, `webview/store.ts`, `test/webview-store.test.ts`, `src/extension.ts`

#### Automated Verification

- [ ] Type checking pasa: `npm run typecheck`
- [ ] Typecheck de tests pasa: `npm run typecheck:test`
- [ ] Corolario reducer en verde: `npx vitest run test/webview-store.test.ts`
- [ ] Seam completo #126: `grep -c "sonar_gate_state" webview/types.ts webview/store.ts src/extension.ts` ≥ 1 en los tres
- [ ] Settings registrados: `grep -c "frida.sonar" package.json` ≥ 3
- [ ] Getter en vivo definido y consumido: `grep -n "readSonarConfig" src/settings.ts src/extension.ts`
- [ ] Guarda dist-bundle-integrity: `npm run build` (incluye `vite build` → `dist-webview/`) — todo commit que toque `webview/**` lleva fuente + bundle juntos
...
### Slice 3: Snapshot por turno — consolidador del bus + hook turn_end

**Files**: `src/extension.ts`

#### Automated Verification

- [ ] Formato del host pasa: `npx prettier --check src/extension.ts`
- [ ] Type checking pasa: `npm run typecheck`
- [ ] Typecheck de tests pasa: `npm run typecheck:test`
- [ ] Suite de contrato de la lib permanece verde: `npx vitest run test/sonar-lib.test.ts`
- [ ] Fan-out cableado en los dos constructores de sesión: `grep -c "onLensDiagnostics: lensAndSonar" src/extension.ts` devuelve 2
- [ ] Hook único de snapshot por turno: `grep -c "scheduleSonarTurnSnapshot();" src/extension.ts` devuelve 1
- [ ] El modo por turno sigue siendo cache-only: `grep -n 'mode: "all"' src/extension.ts` muestra la invocación de `computeTurnSnapshot` y no usa `refreshRunners`
...
### Slice 4: Tab «Sonar» del SettingsHub — veredicto, familias, tendencia

**Files**: `webview/components/SonarTab.tsx`, `webview/components/SettingsHub.tsx`, `webview/styles.css`, `test/sonar-tab.test.ts`, `docs/webview-ui-styles.md`

#### Automated Verification

- [ ] Type checking pasa: `npm run typecheck`
- [ ] Typecheck de tests pasa: `npm run typecheck:test`
- [ ] Suite del tab en verde: `npx vitest run test/sonar-tab.test.ts`
- [ ] Tab registrado en SettingsHub (import + unión + TABS + render): `grep -ci "sonar" webview/components/SettingsHub.tsx` devuelve 4
- [ ] Familia CSS per-tab con prefijo propio: `grep -c "^\.sn-" webview/styles.css` >= 20
- [ ] Familia documentada en la guía de estilos: `grep -c "\.sn-" docs/webview-ui-styles.md` >= 8
- [ ] Sin glifos unicode como iconos: `grep -cE "[●○◆▲▼▁▂▃▄▅▆▇█]" webview/components/SonarTab.tsx` devuelve 0
- [ ] Hover auditado en bundle contra la cascada global: tras `npm run build`, `grep -o "sn-issue:hover{[^}]*}" dist-webview/assets/index-*.css` muestra el fondo del propio :hover (docs/webview-ui-styles.md)
- [ ] Guarda dist-bundle-integrity: `npm run build` (incluye `vite build` → `dist-webview/`) — todo commit que toque `webview/**` lleva fuente + bundle juntos
...
### Slice 5: Badge de gate en la franja del chat (FR-6)

**Files**: `webview/components/SonarGateBadge.tsx`, `webview/App.tsx`, `webview/styles.css`, `test/sonar-tab.test.ts`, `webview/components/SonarTab.tsx` (merge 1 palabra: export VERDICT_META), `docs/webview-ui-styles.md` (merge inventario + bullet)

#### Automated Verification

- [ ] Type checking pasa: `npm run typecheck`
- [ ] Typecheck de tests pasa: `npm run typecheck:test`
- [ ] Suites webview en verde: `npx vitest run test/sonar-tab.test.ts test/webview-store.test.ts`
- [ ] Badge montado en la franja junto al panel lens: `grep -n "SonarGateBadge" webview/App.tsx` muestra import + montaje (2 hits)
- [ ] Reúso del mapa de veredicto del tab (sin duplicación): `grep -c "Record<SonarVerdict" webview/components/SonarGateBadge.tsx` devuelve 0 (sin mapa local) y `grep -c "export const VERDICT_META" webview/components/SonarTab.tsx` devuelve 1
- [ ] Sin glifos unicode como iconos: `grep -cE "[●○◆▲▼▁▂▃▄▅▆▇█]" webview/components/SonarGateBadge.tsx` devuelve 0
- [ ] Hover del badge auditado en bundle contra la cascada global: tras `npm run build`, `grep -o "sn-badge:hover{[^}]*}" dist-webview/assets/index-*.css` muestra el fondo del propio :hover (docs/webview-ui-styles.md)
- [ ] Guarda dist-bundle-integrity: `npm run build` (incluye `vite build` → `dist-webview/`) — todo commit que toque `webview/**` lleva fuente + bundle juntos
...
### Slice 6: Gate completo bajo demanda (FR-7) — botón + progreso + degradaciones

**Files**: `src/extension.ts`, `src/sonar/gate.ts`, `webview/types.ts`, `webview/components/SonarTab.tsx`, `test/sonar-lib.test.ts`, `test/sonar-tab.test.ts`

#### Automated Verification

- [ ] Type checking pasa: `npm run typecheck`
- [ ] Typecheck de tests pasa: `npm run typecheck:test`
- [ ] Suites del feature en verde: `npx vitest run test/sonar-lib.test.ts test/sonar-tab.test.ts test/webview-store.test.ts`
- [ ] Acción cableada punta a punta: `grep -n "run_full_gate" src/extension.ts webview/types.ts webview/components/SonarTab.tsx` ≥ 1 en los tres
- [ ] Full vía tool viva sin race host: `grep -n 'mode: "full"' src/extension.ts` muestra la invocación con `refreshRunners: "all"` y `grep -c "Promise.race" src/extension.ts` devuelve 0
- [ ] Guard de generación contra corridas obsoletas: `grep -c "busySince !== startedAt" src/extension.ts` devuelve 3 (onUpdate + éxito + catch)
- [ ] Progreso ASCII (sin la barra de bloques del productor): `grep -cE "[█░]" src/sonar/gate.ts src/extension.ts webview/components/SonarTab.tsx` devuelve 0 en los tres
- [ ] Parse lenient del progreso compartido: `grep -c "progressLineFromUpdate" src/sonar/gate.ts src/extension.ts test/sonar-lib.test.ts` ≥ 1 en los tres
- [ ] El historial sigue siendo sólo por turno: `grep -c "appendEntry(" src/extension.ts` devuelve 1 (único append en computeTurnSnapshot)
- [ ] Guarda dist-bundle-integrity: `npm run build` (incluye `vite build` → `dist-webview/`) — todo commit que toque `webview/**` lleva fuente + bundle juntos
...
Perspectiva del consumidor (desarrollador que corrige código con Frida):
...
```
src/sonar/gate.ts                        # NEW — lib pura: familias, consolidado bus, veredicto, diff, parse lenient details, progreso ASCII onUpdate
src/sonar/snapshot-store.ts              # NEW — persistencia snapshot v1 por proyecto (JSON único, FIFO, 0600, no-throw)
test/sonar-lib.test.ts                   # NEW — suite Node-pura de la lib (fixtures honestos contrato 3.8.72)
src/settings.ts                          # MODIFY — readSonarConfig() getter en vivo (molde readGatePatterns)
package.json                             # MODIFY — frida.sonar.maxWarnings/disabledFamilies/historyLimit en contributes
webview/types.ts                         # MODIFY — espejo Sonar* + State.sonar + In/Out messages (builds separados)
webview/store.ts                         # MODIFY — case sonar_gate_state (replace plano)
test/webview-store.test.ts               # MODIFY — corolario reducer
src/extension.ts                         # MODIFY — sonarState/postSonarState/case/fan-out ×2/turn_end/runFullGate/webview_ready/cleanup
webview/components/SonarTab.tsx          # NEW — tab «Sonar» (veredicto/diff/familias/tendencia/gate completo)
webview/components/SettingsHub.tsx       # MODIFY — registro del tab (unión + TABS + render)
webview/styles.css                       # MODIFY — familia .sn-* + badge (moldes auditados)
docs/webview-ui-styles.md                # MODIFY — familia .sn-* documentada + .sn-issue/.sn-retry al inventario de botones auditado
test/sonar-tab.test.ts                   # NEW — tab + badge render tests
webview/components/SonarGateBadge.tsx    # NEW — badge veredicto +N −M en la franja del chat
webview/App.tsx                          # MODIFY — montaje del badge junto a LensDiagnostics
```

## Ordering Constraints

- Slice 1 (lib pura) PRIMERO: todo lo demás consume sus tipos/funciones.
- Slice 2 (seam #126) completo en un slice: Out-message + case dispatcher + case reducer + (re)post aterrizan juntos — convención #126/Fase 1 de M2.
- Slice 3 requiere 1-2 (consolidador publica por el seam; snapshot usa la lib).
- Slices 4-5 requieren 2 (state.sonar) y consumen datos de 3; entre sí son independientes pero se secuencian 4→5 (el badge reusa helpers visuales del tab).
- Slice 6 (gate completo) al final: terminal slice con baseline completo; requiere 1-5.
- Commits: fuente + `dist-webview/` juntos en cada commit que toque `webview/**` (guarda `dist-bundle-integrity`); formatear antes de commitear.

## Verification Notes

Llevadas del research (formato verificable):

- **Contrato del externo miente** — el mock del bus debe reproducir el shape REAL verificado `{severity, message, tool, ruleId?, line?, col?, fixable?}` (severity dual string|número — `classifySeverity` ya la maneja), `ruleId?`/`line?` opcionales; NO mockear `range`/`code`/`source`/`semantic` por diagnóstico. Verificar: `grep -c "range:" test/sonar-lib.test.ts` == 0 y `grep -c "semantic:" test/sonar-lib.test.ts` == 0 (los fixtures del bus no declaran campos que el bus nunca envía).
- **Una sola fuente de verdad** (lección 40c7d20): veredicto, diff y tendencia derivan del MISMO snapshot — test de invariante: `entries[last].countsPorFamilia` sumado == issues agrupadas por familia del mismo archivo.
- **Reducer replace**: test «sonar_gate_state llena state.sonar» + test negativo de que NO muta otros campos (badge y tab leen el mismo campo).
- **Poda FIFO**: fixture con `historyLimit+1` entradas → `appendEntry` → releer → `length === historyLimit` y la más vieja evicted (AC literal del FRD).
- **Umbrales en vivo**: `computeVerdict(fixture, {maxWarnings: 0})` → WARN; `computeVerdict(fixture, {maxWarnings: 100})` → PASS, mismo fixture (AC del FRD).
- **Seam completo**: `grep -c "sonar_gate_state" webview/store.ts src/extension.ts webview/types.ts` ≥ 1 en los tres.
- **Fan-out en los 2 wiring points**: `grep -n "sonarOnLens\|mergeLens(p)" src/extension.ts` → exactamente 2 sitios con el fan-out.
- **Flakiness preexistente**: atribuir fallos del `npm test` completo con `git diff --stat` antes de asumir culpa del tab nuevo; correr las suites nuevas en aislado primero.
- **Baseline**: `npm run typecheck` (host + webview), `npm run typecheck:test`, `npm run build` (incluye `vite build` → dist-webview), `npx vitest run test/sonar-lib.test.ts test/sonar-tab.test.ts`.
- **Smoke manual del badge sin romper auto-ocultamiento**: turno limpio (0 err/0 warn) → panel lens se auto-oculta (comportamiento previo intacto) y el badge SÍ aparece (PASS  −0); turno con error → badge FAIL +1.
- **Snapshot sin mensajes**: `grep -c "message:" src/sonar/snapshot-store.ts` == 0 (ningún campo `message` en escritura — NFR secrets; la garantía de tipo la da `SonarIssue` sin message y el test de roundtrip lo aserta).
...
No aplica migración de datos existentes: `<globalStorage>/sonar/` es nuevo. El guard de schema (`frida-sonar-gate/v1`) rechaza versiones futuras desconocidas degradando a snapshot vacío honesto (sin crash, sin sobreescribir el archivo hasta el primer append válido).

## Pattern References

- `src/project-map/lens-project-report.ts:86-210` — unión de estados honestos, SIEMPRE resuelve, catch ruidoso, coerción defensiva, constantes congeladas por test.
- `src/extension.ts:631-680` — `let pmState` + broadcaster 2 líneas + busy/busySince #111 (molde de `sonarState`/`postSonarState`).
- `src/extension.ts:3575-3607` — case delgado: cero lógica de negocio, SIEMPRE responde y SIEMPRE postea.
- `src/extension.ts:2311-2317,2340-2348` — `void p().catch(...)` fire-and-forget + debounce async (molde del hook turn_end).
- `src/extension.ts:1047-1052` — `isLensLoaded()` vía `resourceLoader.getExtensions()` (mismo recorrido del gate completo).
- `src/extension.ts:2646` — re-post de estado en `webview_ready` para re-montes fríos.
- `src/settings.ts:118-126` + `src/tools/frida-permission-system/policy.ts:98` — getter en vivo ↔ evaluador puro con parámetros de datos.
- `src/pilens-config.ts:33-53` — JSON tolerante + `mkdirSync recursive` + write 0600 no-throw.
- `src/tools/frida-workflow/audit.ts:22-25` — `encodeCwd` (slug de cwd a segmento de path).
- `src/usage/report-schema.ts:12` — schema versionado aditivo «lo aditivo sigue siendo v1».
- `webview/store.ts:616-620,687-692` — reducers M2/lens_status (molde del case replace).
- `webview/components/ProjectMapTab.tsx:44-47` — carga al montar vía useEffect + `{state, post}`.
- `test/project-map-lib.test.ts` — «Node puro, sin vscode», fixtures honestos, mkdtempSync + cleanup.
- `test/webview-store.test.ts:114` — corolario reducer.
- `docs/webview-ui-styles.md:101-106` — prefijo per-tab `.sn-`, inventario de botones, cascada `button:hover`.

## Developer Context

**Q (research, checkpoint — `src/pi-session.ts:1226-1232` + `moat-factories.ts:59-66`): ¿Contra qué pi-lens se congela el contrato?**
A: 3.8.72 runtime (`~/.frida`), lenient; `failedAnalyzers`/`coldReasons` se omiten si no llegan. *(Heredada; re-verificada en vivo durante design.)*

**Q (research, checkpoint — `src/lens-diagnostics-bridge.ts:23-32` + `src/extension.ts:1068`): ¿De dónde sale el detalle per-issue del snapshot?**
A: Bus crudo consolidado — segundo consumidor fan-out junto a `mergeLens` en `:1249` y `:5694`, consolidado por path+mayor-seq, clave `path:line:ruleId|tool`; veredicto bloqueante de `details` de mode=all. Descartados parser de texto e híbrido. *(Heredada.)*

**Q (design, Step 4 — semántica de `frida.sonar.disabledFamilies`, evidence `~/.frida/.../lens-diagnostics.js:1258-1266`): los agregados de mode=all/full no traen desglose por familia y el per-issue sólo existe en el consolidado del bus (cap 12/archivo, cobertura de archivos editados) — una exclusión «del veredicto» no puede restarse honestamente de los totales en todos los casos. ¿Qué semántica?**
A: **Exclusión best-effort** — el conteo WARN del veredicto resta las warnings de familias deshabilitadas según el consolidado per-issue (clamp ≥0, fail-strict); bloqueantes jamás deshabilitables; también las excluye del diff/issue-list/countsPorFamilia; efecto nulo para knip/jscpd/madge documentado en el tab. (D3.)

**Q (design, Step 4 — forma del mensaje host→webview, evidence lección merge M2 `webview/store.ts:616`): ¿un mensaje `sonar_gate_state` con todo + reducer replace, o dos mensajes (estado + tendencia) con merge?**
A: **Un mensaje + replace** — ningún campo se llena por un segundo mensaje; tendencia ≤50 puntos ~3KB viaja en cada push. (D4.)

## Design History

- Slice 1: Lib pura src/sonar/* — veredicto, diff, consolidación y snapshot — approved as generated
- Slice 2: Seam completo #126 — settings + mensajes + reducer + case — approved as generated
- Slice 3: Snapshot por turno — consolidador del bus + hook turn_end — approved as generated
- Slice 4: Tab «Sonar» del SettingsHub — veredicto, familias, tendencia — approved as generated (fixes del slice-verifier incorporados: criterio grep -ci, familia .sn-* + botones documentados en docs/webview-ui-styles.md, Recargar oculto en not-installed — D9)
- Slice 5: Badge de gate en la franja del chat (FR-6) — approved as generated (fix del slice-verifier incorporado: criterio de reúso VERDICT_META reescrito a greps pasables — sin mapa local + export único)
- Slice 6: Gate completo bajo demanda (FR-7) — botón + progreso + degradaciones — approved as generated (extensión de Files ratificada: +gate.ts/types.ts/sonar-tab.test.ts; fixes del slice-verifier incorporados: guard de generación busySince===startedAt contra corridas obsoletas tras newSession/switchSession, y progreso ASCII derivado de details — nunca de la barra de bloques del productor)

## References

- Research: `.rpiv/artifacts/research/2026-08-29_11-55-22_m3-frida-sonar.md`
- FRD: `.rpiv/artifacts/discover/2026-08-29_11-35-42_m3-frida-sonar.md` (issue #144, Refs #144)
- Patrón M2: `.rpiv/artifacts/research/2026-08-29_00-04-25_m2-panel-mapa-del-proyecto.md` + `.rpiv/artifacts/plans/2026-08-29_03-02-03_m2-panel-mapa-del-proyecto.md`
- Contrato runtime congelado: `~/.frida/npm/node_modules/pi-lens@3.8.72/dist/tools/lens-diagnostics.js` (details :1138-1266, dedupKey :545-553, ceiling :49-52, coldRunners :1040-1049, timedOut :1085-1108, reconcileScan :883), `dist/clients/diagnostics-publish.js:68,73` (cap 12, seq), `dist/index.js:56918` (shape real del bus)
- Referencia de evolución (NO runtime): `~/.pi/agent/npm/node_modules/pi-lens@4.1.2/dist/tools/lens-diagnostics.js:181-182,1588`
- Lecciones: 40c7d20 (una sola fuente de verdad), 30ef616/9d6d8bb (el contrato del externo miente), f3112ec (catch ruidoso), Plan Review M2 (reducer merge), 3054869 (dist-webview en el mismo commit)
