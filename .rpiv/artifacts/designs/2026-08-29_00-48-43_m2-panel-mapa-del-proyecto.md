...
tags: [design, m2, webview, settingshub, pi-lens, project-map, frida-app-walkthrough, frida-traffic2api]
status: ready
parent: .rpiv/artifacts/research/2026-08-29_00-04-25_m2-panel-mapa-del-proyecto.md
last_updated: 2026-08-29T00:48:43-0600
last_updated_by: Edgar F. Fuentes Perea
---

# Design: M2 — panel Mapa del proyecto (webview): journeys M8 + mapa técnico pi-lens

## Summary

Tab `projectMap` ("Mapa") en el SettingsHub de la webview con dos vistas conmutables renderizadas en React nativo: **Funcional** (journeys `J01..` derivados determinísticamente del `actionLog` de `docs/funcional/artifacts/inventory.json` de M8, con corte por `goto`) y **Técnica** (subsystems/hubs/entryPoints/riskHotspots vía `projectReport(cwd)` de pi-lens con import dinámico host-side de `dist/clients/lens-engine.js`). El host vive como lib pura `src/project-map/*` + cases delgados en `extension.ts`; la verdad del estado queda en el host (convención #111) publicada por push `project_map_state`. Clic en nodo → `open_file` (texto vía `openAtLine`, binarios vía `vscode.open`, siempre rebase + `safeJoin`); screenshots como data-URI on-demand; cruce técnico↔funcional condicionado a la matriz M9; export HTML autónomo híbrido (webview serializa layout, host ensambla + inlina PNGs base64). Grafo SVG propio por columnas con scroll bidireccional, colapsado por defecto.

## Requirements

- **FR-1** Registrar tab `projectMap` en el SettingsHub (unión `SettingsTab` + array `TABS` + rama de render) con el contrato `{ state, post }`.
- **FR-2** Dos vistas conmutables dentro del tab: Funcional y Técnica.
- **FR-3** Vista funcional: grafo de journeys desde `docs/funcional/artifacts/inventory.json` (screens `P01..` + `actionLog` step→screenId→kind/outcome), agrupado por journey y colapsado por defecto, con toggle "mostrar todo".
- **FR-4** Vista técnica: `projectReport(cwd)` vía import dinámico host-side de `dist/clients/lens-engine.js` (patrón moat), renderizando subsystems (directorios + edges), hubs y entryPoints, acotado por defecto (top N) con toggle.
- **FR-5** Vista técnica: overlay de riesgo — `riskHotspots` resaltados; `deadWeight` listado sutil.
- **FR-6** Clic en nodo → `{ type: "open_file", file, line? }` que el host resuelve con `openAtLine()` (texto) o `vscode.open` (binarios); nodos funcionales abren su evidencia (screenshot/snapshot/documento).
- **FR-7** Estados vacíos accionables: sin `docs/funcional/` → "corre el patrón app-walkthrough (M8)"; build pi-lens → "construyendo…" con re-poll automático; sin matriz M9 → cruce omitido con nota, sin error.
- **FR-8** Con matriz M9 existente: enlazar pantallas↔módulos entre ambas vistas.
- **FR-9** Export HTML autónomo de la vista actual (JSON embebido, sin dependencias externas) vía diálogo de guardado.
- **FR-10** Vista funcional carga al abrir el tab + botón refresh manual; sin watchers.
- **NFR** Performance: cientos de pantallas sin bloquear (colapso por defecto, caps `slice(0,N)`); `projectReport` corre en el host.
- **NFR** Security: solo lectura del workspace con `safeJoin`; export escribe únicamente con diálogo confirmado; sin red.
- **NFR** UX/a11y: tokens `--vscode-*` + Codicons + filas planas; teclado (Tab/Enter/↑↓); `prefers-reduced-motion`; panel angosto.
- **NFR** Reliability: sin insumo o corrupto → vacío accionable, nunca spinner eterno (issue #142); contrato no-bloqueante de cache fría respetado.
- Issue #143 · Conventional Commits con `Refs #143` · motor `frida-extensible-workflows` intocado.

## Current State Analysis

No existe ningún componente de grafo/mapa en `webview/components/` (52 componentes; solo 2 precedentes SVG: `DonutChart.tsx:28` geometría manual y `FridaRobotIcon.tsx:21` stroke currentColor). El SettingsHub registra tabs con 3 toques (unión `SettingsHub.tsx:13-22`, `TABS:24-34`, rama `{state, post}` `:456-461`); los tabs recientes cargan desde su propio componente (molde `ProductivityTab.tsx:44-47`). El dispatcher `handleWebviewMessage` (`src/extension.ts:2521`, ~150 cases, sin `default`) y el reducer (`webview/store.ts:221`, `default: return state`) ignoran variantes desconocidas en silencio — el seam mensaje→reducer→render debe aterrizar completo (#126). No existe variante `open_file` en `OutMessage`; `openAtLine` (`src/extension.ts:4691-4698`) solo sirve texto con paths absolutos. La instalación de referencia de pi-lens es 3.8.72 en `~/.frida/npm/node_modules/pi-lens`; el seam declarado para host adapters es `dist/clients/lens-engine.js` (re-exporta `projectReport`). No existe `docs/funcional/` real en este repo (se genera en el workspace de la app objetivo).

### Key Discoveries

- **Registro de tab = 3 piezas + carga interna** (`SettingsHub.tsx:13/24/456`); `App.tsx` no se toca. `SettingsTab` solo se consume en `App.tsx:999` (cast) — agregar `"projectMap"` no rompe nada.
- **Verdad del estado en el host** (convención #111): `ciBusySince` (`src/extension.ts:602`) + `postCodebaseIndexState()` (`:617-655`) re-posteado en `webview_ready` (`:2529`) — sobrevive re-montes fríos. `lensStatus` viola este patrón y pierde el badge tras re-monte frío: M2 no debe copiar ese hueco.
- **Anatomía de acción async host** (molde `codebase_index_action`, `src/extension.ts:3166-3290`): acción read-only sin busy con try/catch que SIEMPRE responde; busy con epoch antes del trabajo async; poll host-side 2s best-effort; `finally` que limpia SIEMPRE; catch final que convierte error en línea visible.
- **Contrato `projectReport(cwd, options)`** (`~/.frida/npm/.../project-report.js:501-567`): `{available:true, trust, hubs, entryPoints, subsystems, riskHotspots, deadWeight}`; dos ramas `available:false` con semántica opuesta — size-skip permanente (hint `review graph disabled: ...` → NO re-polear) vs cache fría transitoria (`retry this call shortly` → re-poll). El hint es el único discriminador accesible desde el seam. NO existe `subsystems[].name` — `subsystems = {directories, edges, cycles, violations}`.
- **Journeys `J01..` no existen en JSON**: `journeys.md` lo escribe un LLM (`frida-app-walkthrough/workflow.ts:182-186`); la vista debe derivarlos. Algoritmo canónico de aristas: join inter-pasos M9 (`frida-traffic2api/workflow.ts:1053-1064`).
- **CSP ya permite data-URIs** (`src/webview-html-core.ts:17`: `img-src ${cspSource} data:`), probado en producción (`Turn.tsx:76`); `localResourceRoots` queda en `dist-webview`+`media` (`src/extension.ts:2417-2420`) — sin ensanchar raíces.
- **`post()`** (`src/extension.ts:789-792`): `view?.webview.postMessage(msg)` — seguro sin view. Lets del tab Index viven en la clausura de `activate()` (584-618).
- **`defaultAgentDir()`** (`src/pi-session.ts:1226-1233`) = `~/.frida`; no existe builder para `clients/lens-engine.js` — M2 escribe el suyo (importar `piLensEntryPath` de moat-factories no crea ciclo pero devuelve `dist/index.js`, la entry de extensión que NO sirve).
- **Matriz M9**: `docs/api/artifacts/inventory.json` → `inv.matrix[] = {id, functionality, screenIds[], endpoints[], modules[{path, evidence}]}`; `endpoints` único campo required; normalización `M01..` por orden.
- **Molde export**: `exportUsage` (`src/extension.ts:5905-5938`) — `showSaveDialog` → cancelar no-op → `workspace.fs.writeFile` → toast. Molde HTML autónomo M8 (`workflow.ts:576-615`): `escHtml`, JSON embebido con `.split("</").join("<\\/")`, CSS inline paleta fija, render vanilla `createElement`.
- **Alerta working tree** (2026-08-29, `202751d`): `dist-webview/assets/index-gE2gVxhC.js` modificado sin commitear y `test/dist-bundle-integrity.test.ts` sin trackear — deben aterrizar ANTES del primer commit de M2.
- **vite** (`vite.config.ts`): `emptyOutDir: true`, hash de contenido — cualquier toque a `webview/**` regenera `dist-webview/assets/index-*.js`; commit conjunto fuente+bundle SIEMPRE (`590b946`/`d01621c`).

## Scope

### Building

- Tab `projectMap` en SettingsHub + comando de paleta `frida.projectMap`.
- Vista Funcional: derivación determinista de journeys (corte por goto), grafo SVG columnas+scroll colapsado por defecto, badge cobertura parcial (`stoppedBy`), clic→evidencia (snapshot/screenshot/documento), screenshots on-demand.
- Vista Técnica: subsystems/hubs/entryPoints/riskHotspots/deadWeight, overlay de riesgo, cache fría con re-poll acotado, size-skip con paro + hint verbatim.
- Cruce técnico↔funcional condicionado a matriz M9 (con nota de omisión).
- Export HTML autónomo con screenshots inlinados base64.
- Mensajes `project_map` / `open_file` / `export_map` (Out) + `project_map_state` / `project_map_shot` (In) + campo `State.projectMap`.
- Tests: lib host pura (Node), componente (renderToStaticMarkup), contrato reducer.
...
### Organización del código host — lib pura + cases delgados

**Ambiguity**: Dos patrones vigentes — inline en la clausura `activate()` (tab Index, `ci*` en `src/extension.ts:584-655/3164-3290`) vs lib pura `src/…/*` + cases delgados (tab Uso, 6 módulos `src/usage/*` + `extension.ts` +99, commit `30f50bd`).
**Explored**: (A) inline — acceso directo a `post()`/`workspaceCwd()`/`context` sin pasar deps, pero suma ~500 líneas a un archivo de 6 330 y la lógica (journeys/cruce/export) no es testeable sin mockear vscode; (B) lib — testeable en Node con vitest puro, `extension.ts` se mantiene, molde del tab Uso.
**Decision**: **Lib `src/project-map/*` + cases delgados** (developer checkpoint). Las `let pm*` mínimas + `postProjectMapState()` viven en la clausura; la lógica en módulos puros que reciben `cwd`/deps por parámetro.

### Semántica de journeys `J01..` — corte por goto

**Ambiguity**: Los IDs no existen en JSON (research: `workflow.ts:182-186`); había que fijar la heurística de segmentación.
**Explored**: (A) corte por `goto` — un journey = secuencia maximal de aristas traversed entre gotos; el goto marca entrada explícita (nueva intención); fails no cortan (quedan como attempted-failed del journey en curso); fiel al timeline, una pantalla puede aparecer en varios journeys. (B) raíces estructurales — journeys disjuntos por BFS desde pantallas sin entrada, pero funde gotos distintos y pierde orden temporal.
**Decision**: **Corte por goto** (developer checkpoint). La primera acción (si no es goto) abre `J01`. Clasificación de aristas por el algoritmo canónico M9 re-implementado: `progressed = next && next.screenId !== a.screenId`; fail→attempted-failed(cause shell-error); validate→attempted-failed(app-validation); click/form sin progresión→attempted-failed(no-progression); click/goto/form con progresión→traversed.

### Layout del grafo SVG — columnas + scroll bidireccional

**Ambiguity**: Panel angosto (WebviewView lateral ~300-600 px), cero deps de grafo, 2 precedentes SVG manuales.
**Explored**: (A) columnas fijas (~140 px) por journey (Funcional) / subsystem (Técnica), filas por orden de visita/impacto, aristas bezier, contenedor `overflow:auto` — determinista, mapa completo visible; (B) DAG por capas — requiere resolver ciclos manualmente, alto explota con fan-out; (C) lista jerárquica + mini-SVGs — barato pero pierde la vista global de la demo.
**Decision**: **Columnas + scroll** (developer checkpoint). Colapso = columnas fuera del DOM (molde TreePanel `visibleIds`), no CSS hide.

### Screenshots — data-URI host-side on-demand

Decisión heredada del research (checkpoint): CSP `img-src ... data:` ya lo permite (`webview-html-core.ts:17`, probado en `Turn.tsx:76`); `localResourceRoots` NO se ensancha. Host lee el PNG con `safeJoin(workspaceCwd(), rel)` y postea data-URI solo al expandir/clic un nodo (base64 infla +33% — jamás el set completo). Mensaje dedicado `project_map_shot` (molde `codebase_index_files`: consulta read-only on-demand separada del estado).

### `open_file` — texto/binario con rebase + contención

`openAtLine` (`src/extension.ts:4691-4698`) solo sirve texto y espera paths absolutos; los paths del inventory son relativos al cwd de la corrida → rebase `path.resolve(workspaceCwd(), rel)` SIEMPRE + `safeJoin` anti-traversal (`agents-sync.ts:89-94`) antes de abrir. Binarios (PNG): `vscode.commands.executeCommand("vscode.open", Uri.file(abs))` — código nuevo sin precedente local (pariente lejano `vscode.openFolder` en `worktree/command.ts:217`); try/catch degrada a `showErrorMessage`. Los `actionLog[].ref` NO son paths (refs DOM `@eN`) — no son evidencia abrible.

### Re-poll de cache fría — 2s→5s→10s, cap ~10 intentos

Adopta la propuesta del research (consistente con #142): backoff 2s→5s→10s acotado a ~10 intentos (~90-110 s); al agotar → detener spinner + hint verbatim + botón Reintentar. **Paro inmediato** si el hint parsea como size-skip (`/^review graph disabled/i` — parse lenient, NO hardcodear strings completos: en 4.1.2 los hints cambiaron). El reloj vive en el host (`busySince`, #111). Poll host-side con `finally` que limpia SIEMPRE (molde `:3272-3277`).

### Entry de pi-lens — `dist/clients/lens-engine.js`

Decisión heredada del research (checkpoint): seam declarado para host adapters (`lens-engine.js:1-14`), re-exporta `projectReport` (`:35`). Alternativa `project-report.js` directo descartada (viola la regla del seam). Import: `await import(pathToFileURL(entry).href)` (#57 — probado en producción desde M1) + destructurar export nombrado `const { projectReport } = await import(url)`. El caller atrapa rechazos de la LLAMADA completa (el `await import("./review-graph/builder.js")` interno no está envuelto). Sonda: `fs.existsSync` → estado "no instalado" sin throw. Catch → `console.warn` + estado vacío visible, nunca silencio (`f3112ec`).

### Tipos — productores en el host, espejo UI en webview

El host no importa `webview/types.ts` (builds separados, 0 matches de `OutMessage` en src/). La lib define los tipos canónicos del payload; `webview/types.ts` declara el espejo UI (molde `UsageReportView` ↔ `UsageSnapshot`, comment types.ts "espeja UsageSnapshot del host (build separado)").

### Comando de paleta — `frida.projectMap`

Directional confirm: molde exacto `frida.codebaseIndex` (`src/extension.ts:5954-5967` + `package.json:93-96`): `pendingSettingsTab = "projectMap"` + `frida.openPanel` + post directo en caliente con guard `webviewReady` (flush frío en `webview_ready`).

### Estilos — prefijo `.pm-*`

Directional confirm: convención per-tab (`.ci-`/`.usage-`/`.prod-`/`.env-`, `styles.css:9909+/8368+/8709+`). ~20 clases nuevas `.pm-*`.

### Tests — 3 moldes de la casa

Directional confirm: componente `renderToStaticMarkup` + `post = vi.fn()` (`test/productivity-tab.test.ts:77-118`, efectos NO corren); contrato reducer `reduce(initialState, msg)` (`test/webview-store.test.ts:16-21`); lib host en Node puro con fixtures honestos del inventory (forma `test/frida-traffic2api/e2e.test.ts:497-560` AMPLIADA a todos los campos que M2 lee: `origin`, `firstSeenStep`, `snapshot`, `screenshot`, `purpose`, `userRoles`, `mainElements`, `stoppedBy`).

### Render React nativo · colapsado por defecto · refresh manual · re-poll automático

Decisiones heredadas del discover (checkpoints, no re-abrir): React nativo en la webview (vs embed de index.html M8); colapsado con toggle; carga al abrir + refresh manual (sin watcher); re-poll automático en cache fría.

### Límites de `projectReport`

`options.limit` default 10 (`DEFAULT_LIMIT`), toggle a 25/50 en la vista técnica (FR-4 "top N con toggle"). Pedir JSON sin `view` (el render compacto es función separada). `entryPointFiles` es uncapped — cap visual `slice(0,N)` en la UI si hace falta.

## Architecture

### src/project-map/functional-inventory.ts — NEW

Lectura y validación host-side de `docs/funcional/artifacts/inventory.json` (M8): probe existsSync, `JSON.parse` en try/catch, exigir `Array.isArray(screens) && Array.isArray(actionLog)`, forma de pantalla/actionLog, huérfanos, badge stoppedBy. Emite payload UI de pantallas.
...
// ══ Slice 3: fusión — estado técnico del tab (tipos del seam pi-lens; sin
//    ciclo: lens-project-report NO importa de functional-inventory) ══
import type { PmTechnicalState } from "./lens-project-report";
// ══ Slice 4: fusión — estado del cruce (tipos del seam M9; sin ciclo:
...
export interface ProjectMapHostState {
 ...
 // ══ Slice 4: cruce técnico↔funcional (matriz M9) ══
 cross?: PmCrossState;
}
...
// ══ Slice 2: guard de contención + lector de PNGs como data-URI ══
...
### src/project-map/journeys.ts — NEW

Derivación determinista de journeys `J01..` desde el actionLog plano (corte por goto + clasificación de aristas M9 re-implementada).
...
Import dinámico host-side de `lens-engine.js` (pathToFileURL + sonda existsSync + catch ruidoso), parse de hints (size-skip vs cache fría), tipos espejo del payload `projectReport` 3.8.72.
...
### src/project-map/matrix-cross.ts — NEW

Cruce técnico↔funcional: lee `docs/api/artifacts/inventory.json` (M9), join `matrix[].screenIds ⊆ screens M8`, `dirname(modules[].path)` ↔ `subsystems.directories` por prefijo de segmentos completos, normalización de paths LLM.
...
// (cero confianza en paths del cliente — molde del shot on-demand del slice 2).
//
...
### webview/components/ProjectMapTab.tsx — NEW

Shell del tab: seg-toggle Funcional/Técnica, refresh con spin, estados loading/empty accionables, delegación a vistas hijas, botón export.
...
// project_map_state. Slice 2: el cuerpo ready delega a FunctionalView (grafo
// SVG por columnas + evidencia); slice 3: conmutador Funcional/Técnica — la
// vista Técnica delega a TechnicalView (pi-lens) y su carga dispara el MISMO
// mensaje project_map con view:"technical" (el efecto de [view] re-dispara
// al conmutar). La vista activa y el plegado (open) siguen siendo
// estado LOCAL del componente — NO campos del store global (análogo
...
export function ProjectMapTab({
 ...
 // ══ Slice 4: cruce técnico↔funcional (matriz M9) — se pasa a ambas vistas ══
 ...
 // ══ Slice 5 (FR-9): export HTML autónomo de la vista ACTIVA — la webview
 ...
 return (
  <div className="pm-tab">
   <div className="pm-head">
    ...
    {/* ══ Slice 5: export HTML autónomo (.pc-save primario, codicon export) ══ */}
    <button
     type="button"
     className="pc-save"
     disabled={!exportable}
...
### webview/components/project-map/GraphCanvas.tsx — NEW

Renderer SVG compartido: columnas fijas (~140px), nodos apilados por fila, aristas bezier (misma columna vertical / cruzada horizontal con sag + lanes), previews de screenshot en 3 estados, navegación teclado (Tab/↑↓/Enter/Espacio), reduced-motion, collapse real (fuera del DOM — el consumidor decide qué columnas manda). Reutilizado por el slice 3 (Técnica: subsystems como columnas, tone "danger").
...
// El slice 3 (vista Técnica) reutiliza este canvas: columnas = subsystems con
...
export interface GraphNode {
 ...
 /** danger = borde rojo (overlay de riesgo del slice 3). */
 tone?: "default" | "danger";
}
...
### webview/components/project-map/FunctionalView.tsx — NEW

Vista Funcional: journey expandido → sus pantallas como COLUMNAS del grafo en fila horizontal (fiel al Desired End State), colapsado por defecto, badge stoppedBy, clic→evidencia (screenshot > snapshot), screenshots on-demand con dedup por ref, attempted-failed listados bajo el grafo, toggle "mostrar todo".
...
/** ══ Slice 4: filas de cruce del journey — solo pantallas con módulos
...
export function FunctionalView({
 ...
 post: (m: OutMessage) => void;
 /** ══ Slice 4: cruce técnico↔funcional (matriz M9) — opcional para no
  *  romper consumers sin cruce (tests de los slices previos). */
 cross?: PmCrossState;
}) {
 ...
 return (
  <>
   ...
   {/* ══ Slice 4: notas del cruce (FR-7 omisión + matriz stale) ══ */}
      ...
      <div key={j.id} className="pm-journey">
       ...
       {isOpen && (
        <div className="pm-journey-body">
         ...
         {/* ══ Slice 4: chips de módulo (open_file) para las pantallas del
...
// ══ Slice 5 (FR-9): serializa la vista Funcional para el export HTML.
...
### webview/components/project-map/TechnicalView.tsx — NEW

Vista Técnica: subsystems columnas, hubs/entryPoints listas clicables, overlay riskHotspots, deadWeight sutil, toggle límite, hint verbatim + Reintentar.
...
export function TechnicalView({
 ...
 post: (m: OutMessage) => void;
 /** ══ Slice 4: cruce técnico↔funcional (matriz M9) — opcional para no
  *  romper consumers sin cruce (tests del slice 3). */
 cross?: PmCrossState;
}) {
 ...
 return (
  <>
   ...
   {/* ══ Slice 4: cruce funcional — pantallas cubiertas por directorio
...
// ══ Slice 5 (FR-9): serializa la vista Técnica para el export HTML.
...
`ProjectMapUiState` espejo + campo `State.projectMap` + variantes `project_map`/`open_file`/`export_map`/`project_map_shot` (Out) + `project_map_state`/`project_map_shot` (In).

```typescript
// ══ Slice 1: tipos espejo (insertar tras CodebaseIndexUiState, ~linea 900) ══
...
// ══ Slice 3: espejo técnico (productor src/project-map/lens-project-report.ts;
...
export type PmTechnicalState =
 ...
 | { status: "ready"; data: PmTechnicalData; loadedAt: number; limit: number };
// ══ Slice 4: espejo del cruce técnico↔funcional (productor
...
export interface ProjectMapUiState {
 ...
 // ══ Slice 4: cruce técnico↔funcional (matriz M9) ══
 cross?: PmCrossState;
 // ══ Slice 2: cache de screenshots on-demand ══
 /**
...
// ══ Slice 5: espejo del export HTML autónomo (productor
...
// ══ dentro de la unión InMessage, junto a codebase_index_state (~linea 1044) ══
 ...
 // ══ Slice 2: respuesta del shot on-demand (molde codebase_index_files
...
// ══ dentro de la unión OutMessage, junto a codebase_index_action (~linea 1190) ══
 ...
 // ══ Slice 2: petición de shot on-demand + apertura de evidencia (FR-6) ══
 | { type: "project_map_shot"; screenId: string }
 | { type: "open_file"; file: string; line?: number }
 // ══ Slice 5: export HTML autónomo de la vista activa (FR-9) — la webview
...
Cases reducer `project_map_state` + `project_map_shot`.

```typescript
// ══ Slice 1: junto al case codebase_index_state (~linea 608) ══
 ...
 case "project_map_state":
  return { ...state, projectMap: msg.state };

// ══ Slice 2: junto al anterior — cache del shot on-demand ══
...
3 toques del registro: unión + TABS + rama de render.

```tsx
// ══ Slice 1: los 3 toques del registro del tab (contrato {state, post}) ══
...
`pm*` lets + `postProjectMapState()` + re-posteo en `webview_ready` + cases `project_map`/`open_file`/`export_map` + comando `frida.projectMap`.

```typescript
// ══ Slice 1: wiring host del mapa funcional (lib src/project-map/*) ══
...
// ══ Slice 3: mapa Técnico (pi-lens) — seam lens-engine.js ══
import {
 loadTechnicalMap,
 TECH_POLL_DELAYS_MS,
...
// ══ Slice 5: export HTML autónomo (FR-9) ══
   ...
   try {
    for (let attempt = 0; ; attempt++) {
     ...
     if (st.status === "ready" || st.status === "empty") {
      ...
      refreshPmCross(); // ══ Slice 4: dirs disponibles → join técnico ══
      postProjectMapState();
      return;
     }
...
//  antes de check_environment — ~linea 3446)
   ...
   // Slice 3: la rama Técnica delega al re-poll con epoch (función propia).
   case "project_map": {
    ...
    refreshPmCross(); // ══ Slice 4: pantallas disponibles → join funcional ══
    postProjectMapState();
    break;
   }

// ══ Slice 2: evidencia + screenshots on-demand (FR-6/FR-3) ══
...
// ══ Slice 5: export HTML autónomo de la vista activa (FR-9) ══
   ...
   // del shot on-demand del slice 2). Molde del diálogo: exportUsage
...
`contributes.commands`: entrada `frida.projectMap`.

```json
// (Slice 1: nueva entrada tras frida.codebaseIndex — ajustar comas al fusionar)
{
...
Clases `.pm-*` del tab (shell, canvas, nodos, aristas, badges, listas, export).

```css
/* ══ Slice 1: tab Mapa del proyecto (.pm-*) — adición al final ══ */
.pm-tab {
 display: flex;
...
/* (revisión en cascada ratificada en el slice 5: background declarado en el
   propio :hover — (0,2,0) vence al button:hover global (0,1,1) que inyectaba
   el azul primario; ver docs/webview-ui-styles.md) */
...
/* ══ Slice 2: grafo SVG (.pm-canvas/.pm-graph/…) — donde estaban los chips ══ */
.pm-canvas {
 overflow: auto;
 max-height: 56vh;
 border: 1px solid var(--vscode-panel-border);
...
/* ══ Slice 3: vista Técnica (.pm-list/.pm-row/.pm-note/…) ══ */
.pm-list {
 display: flex;
 flex-direction: column;
 gap: 2px;
...
/* ══ Slice 4: cruce técnico↔funcional (.pm-cross*) ══ */
.pm-cross {
 display: flex;
 flex-direction: column;
 gap: 3px;
...
### test/webview-store.test.ts — MODIFY

Contrato reducer: `project_map_state` llena `state.projectMap`; `project_map_shot` cachea screenshot.

```typescript
// ══ Slice 1: adición al final del archivo ══
...
describe("webview store · project_map_state/project_map_shot llegan al reducer", () => {
 ...
 // ══ Slice 2: cache del shot on-demand (merge, no replace) ══
...
### test/project-map-lib.test.ts — NEW

Lib host pura: fixtures honestos del inventory M8, journeys corte-por-goto, validación/corrupción, size-skip parse, cruce matriz, export HTML.
...
// ── Helpers de tmpdir compartidos (subidos a nivel de archivo en la fusión
//    del slice 2: los describe del guard y del lector de PNG los reutilizan) ──
...
// ══ Slice 3: seam pi-lens — mock honesto del contrato (layout espejo del
...
// ══ Slice 4: cruce técnico↔funcional — fixtures honestos del schema
...
// ══ Slice 5: export HTML autónomo (molde M8: escHtml + JSON embebido con
...
### test/project-map-tab.test.ts — NEW

Componente: render de vistas, estados vacíos, toggle, badges.
 ...
 journeys: [
  {
   ...
   edges: [
    ...
    // Slice 2: edge attempted-failed para el test de la lista de fallos.
    {
     type: "attempted-failed",
     from: "P01",
     to: "",
...
// ══ Slice 2: FunctionalView directo (open inyectado — el toggle vive en
...
// ══ Slice 3: vista Técnica (TechnicalView directo — open/busy inyectados;
...
// ══ Slice 4: cruce técnico↔funcional en ambas vistas ══
...
// ══ Slice 5: serializadores del export (la mitad webview del seam FR-9;
...
### docs/webview-ui-styles.md — MODIFY

Documentar el tab Mapa y las clases `.pm-*` (fence llenado en el slice 5 — docs del tab completo + regla de cascada veraz según slice-verifier).
...
### Slice 1: Contrato del tab + mapa funcional host + lista honesta

**Files**: `src/project-map/functional-inventory.ts`, `src/project-map/journeys.ts`, `webview/components/ProjectMapTab.tsx`, `test/project-map-lib.test.ts`, `test/project-map-tab.test.ts`, `webview/types.ts`, `webview/store.ts`, `webview/components/SettingsHub.tsx`, `src/extension.ts`, `package.json`, `webview/styles.css`, `test/webview-store.test.ts`
...
### Slice 3: Vista técnica (pi-lens) + re-poll

**Files**: `src/project-map/lens-project-report.ts`, `webview/components/project-map/TechnicalView.tsx`, `src/extension.ts`, `webview/types.ts`, `webview/components/ProjectMapTab.tsx`, `webview/styles.css`, `test/project-map-lib.test.ts`, `test/project-map-tab.test.ts`, `src/project-map/functional-inventory.ts`

*(Nota de fusión: `src/project-map/functional-inventory.ts` se añadió sobre el skeleton — `ProjectMapHostState` gana `technical?: PmTechnicalState` y su unión `busy` se amplía a `"technical"` (punto de estado host que Architecture/File Map prometen); el resto del archivo locked queda intacto y `lens-project-report.ts` no importa de `functional-inventory` — sin ciclo.)*
...
```text
src/project-map/functional-inventory.ts       # NEW — lee/valida inventory M8 + payload pantallas + guard safeResolveWithin + lector PNG data-URI + ProjectMapHostState (host)
src/project-map/journeys.ts                   # NEW — deriva J01.. (corte por goto, join M9)
src/project-map/lens-project-report.ts        # NEW — import dinámico lens-engine.js + parse hints
src/project-map/matrix-cross.ts               # NEW — cruce matriz M9 ↔ pantallas ↔ subsystems
src/project-map/export-html.ts                # NEW — arma HTML autónomo + inlina PNGs base64
webview/components/ProjectMapTab.tsx          # NEW — shell del tab (toggle/refresh/export)
webview/components/project-map/GraphCanvas.tsx    # NEW — renderer SVG columnas+scroll+teclado
webview/components/project-map/FunctionalView.tsx # NEW — vista Funcional (journeys)
webview/components/project-map/TechnicalView.tsx  # NEW — vista Técnica (pi-lens)
test/project-map-lib.test.ts                  # NEW — lib host pura con fixtures honestos
test/project-map-tab.test.ts                  # NEW — componente renderToStaticMarkup
webview/types.ts                              # MODIFY — ProjectMapUiState + State + mensajes
webview/store.ts                              # MODIFY — cases project_map_state/shot
webview/components/SettingsHub.tsx            # MODIFY — registro del tab (3 toques)
src/extension.ts                              # MODIFY — pm* lets + cases + comando + re-posteo
package.json                                  # MODIFY — contributes.commands frida.projectMap
webview/styles.css                            # MODIFY — clases .pm-*
test/webview-store.test.ts                    # MODIFY — contrato reducer
docs/webview-ui-styles.md                     # MODIFY — documentar tab Mapa
```

## Ordering Constraints

1. **ANTES del primer commit de M2**: aterrizar el estado del working tree — commit conjunto con `test/dist-bundle-integrity.test.ts` (hoy sin trackear) + rebuild de `dist-webview/assets/index-gE2gVxhC.js` (hoy modificado sin commitear). La guarda del FRD debe existir en git antes de tocar `webview/`.
2. Slices estrictamente secuenciales 1→5: cada uno typecheck + tests verdes antes del siguiente.
3. Dentro de cada slice: la tríada dispatcher + reducer + render aterriza JUNTA (#126 — el host puede publicar bien y el panel seguir en blanco).
4. Todo commit que toque `webview/**` regenera y commitea `dist-webview/` en el MISMO commit (`590b946`/`d01621c`); diff funcional limpio (`git diff --check`) — biome corre solo si el entorno lo provee (sin config de biome en el repo).
5. Slice 5 es terminal: lleva el baseline completo del proyecto y los docs.
6. Commits: `feat(webview): …` / `feat(mapa): …` con `Refs #143`.

## Verification Notes

- **Baseline del proyecto**: `npm test` (vitest run) en verde; `npm run typecheck` (host + webview tsconfigs) limpio; `npm run build:webview` + `test/dist-bundle-integrity.test.ts` en verde; commit conjunto fuente+`dist-webview/`.
- **Contratos upstream congelados en mocks honestos** (`30ef616`/`9d6d8bb`): el fixture del inventory debe reproducir el schema REAL del writer M8 (todos los campos que M2 lee); los hints de pi-lens se tratan como opacos — test de parse con `/^review graph disabled/i` lenient, sin hardcodear strings completos.
- **Reducer seam (#126)**: test de regresión `project_map_state` vía `reduce(initialState, msg)` — el mensaje DEBE caer al dispatch general.
- **Import dinámico (#57)**: `pathToFileURL(entry).href` SIEMPRE; catch ruidoso (`console.warn` + estado vacío visible), nunca silencio (`f3112ec`).
- **Sin spinner eterno (#142)**: cache fría → "construyendo…" con reloj derivado de `busySince` del host + re-poll que resuelve solo; insumo faltante/corrupto → vacío accionable con workaround textual.
- **Guards null**: botones probados contra `null`/`undefined` (`379a8b7` — guard falsy para null).
- **Smoke manual** (slice 5): panel angosto (~350 px), temas oscuro y claro, `prefers-reduced-motion`, con y sin `docs/funcional/`, cache fría y caliente de pi-lens.
- **Motor congelado**: `core/registry.ts` intocado — `git diff --stat src/tools/frida-extensible-workflows/core/` vacío.
- **Issue #143**: al cerrar tras verificación, comentario de evidencia con commits (`Refs #143` en cada commit).
...
- `webview/components/ProductivityTab.tsx:37-66` — exemplar de tab `{state, post}` con carga-on-mount, stub loading, empty state y guard de coincidencia filtro/dato.
- `src/extension.ts:3166-3290` — anatomía completa de acción async host (busy, poll 2s, finally, catch).
- `src/extension.ts:617-655` — `postCodebaseIndexState()`: publicación push + cacheo para re-monte (molde directo de `postProjectMapState`).
- `webview/components/TreePanel.tsx:162-273` — `Set` de plegados + `visibleIds`/`flatVisible` memoizados (render condicional de colapso).
- `src/tools/frida-extensible-workflows/moat-factories.ts:77-99` — sonda existsSync + `pathToFileURL` + import warn-no-throw (subconjunto textual).
- `src/tools/frida-app-walkthrough/workflow.ts:576-615` — generador HTML self-contained (molde del export).
- `src/tools/frida-traffic2api/workflow.ts:1048-1064` — join inter-pasos del actionLog (algoritmo canónico de aristas).
- `src/extension.ts:5905-5938` — `exportUsage`: showSaveDialog + writeFile + toast.
- `test/productivity-tab.test.ts:77-118` · `test/webview-store.test.ts:16-21` — moldes de test.
- `webview/components/usage/DonutChart.tsx:22-53` — precedente SVG manual (geometría JS puro, cap de datos, empty state).

## Developer Context

**Q (barrido direccional, lote)**: ¿Propagar la convención de prefijo propio de estilos por tab (`.ci-`/`.usage-`/`.prod-`/`.env-`, `styles.css:9909+/8368+/8709+`) usando `.pm-*`?
**A**: Seguir `.pm-*`.

**Q (barrido direccional, lote)**: ¿Tests siguiendo los 3 moldes de la casa (componente renderToStaticMarkup `productivity-tab.test.ts:77-118`, contrato reducer `webview-store.test.ts:16-21`, lib host Node con fixtures honestos forma `traffic2api/e2e.test.ts:497-560` ampliada)?
**A**: Seguir moldes.

**Q (barrido direccional, lote)**: ¿Incluir comando `frida.projectMap` en la paleta (molde `frida.codebaseIndex`, `extension.ts:5954-5967` + `package.json:93-96`)? El FRD no lo pide.
**A**: Incluir comando.

**Q (ambigüedad genuina 1 — organización host)**: 2 patrones en conflicto: inline en clausura `activate()` (tab Index, ci* `extension.ts:584-655`) vs lib pura + cases delgados (tab Uso, `src/usage/*` + `extension.ts` +99, commit `30f50bd`). M2 arrastra ~500 líneas host.
**A**: Lib + cases — módulos `src/project-map/*` + `pm*` lets mínimas; testeable en Node sin mockear vscode.

**Q (ambigüedad genuina 2 — journeys)**: ¿Segmentación de `J01..` sobre el timeline del actionLog? Evidence: actionLog lineal (`workflow.ts:468`), algoritmo de aristas M9 (`traffic2api/workflow.ts:1053-1064`), journeys.md escrito por LLM.
**A**: Corte por goto — journey = secuencia maximal de aristas traversed entre gotos; fails no cortan; fiel al timeline.

**Q (ambigüedad genuina 3 — layout SVG)**: ¿Layout del grafo en panel angosto con cero deps? Evidence: WebviewView lateral `retainContextWhenHidden` (`extension.ts:5962-5966`), solo 2 precedentes SVG manuales, NFR "panel angosto".
**A**: Columnas + scroll bidireccional — colWidth ~140 px, filas por orden de visita/impacto, aristas bezier, contenedor overflow auto.

**Micro-checkpoint Slice 2** (presentado condensado; aprobado sin cambios): 11 files (2 NEW + 9 MODIFY-fusión, 4 añadidos sobre el skeleton para completar el seam store/reducer + guard testeable). Notas del slice-verifier incorporadas: (1) criterio manual de re-monte rehecho — el hub desmonta tabs condicionalmente (`SettingsHub.tsx:455-459`) y el plegado es estado local por decisión registrada (análogo `period/scope` de `ProductivityTab.tsx:37-38`), así que "conserva el grafo sin re-cargar" era insatisficiente → "re-monta con journeys re-colapsados, re-carga <1 s, shots del store sin re-pedido"; (2) `React.Fragment` sin import rompía bajo `jsx: "react-jsx"` → `import { Fragment } from "react"` (molde `App.tsx:2,561`); (3) `GraphNode.sub` eliminado (YAGNI — lo reintroduce el slice 3 si lo necesita) y `GraphColumn.title` renderizado (`.pm-col-title` con emisor).

**Micro-checkpoint Slice 3** (presentado condensado; aprobado sin cambios): 9 files (2 NEW + 7 MODIFY-fusión, `functional-inventory.ts` añadido sobre el skeleton — `ProjectMapHostState.technical?` + ampliación de `busy`). Notas del slice-verifier incorporadas: (1) `ProjectMapUiState` fusionado omitía el campo locked `shots` del slice 2 (VIOLATION Cross-slice) → restaurado carácter por carácter; (2) `vi.spyOn` sin import de `vi` bajo `globals: false` (VIOLATION atomicidad) → import fusionado `import { afterEach, describe, expect, it, vi } from "vitest"`; (3) Files del slice sin `functional-inventory.ts` (VIOLATION bookkeeping) → 9 archivos + nota de fusión; WARNING de duración corregido: sleeps reales ≈69 s (rampa 2s→5s→10s y cap 10 intactos — lado seguro de #142). Comportamiento menor conocido y aceptado: Reintentar desde estado vacío re-setea el límite a 10 (el estado empty no porta límite; un click lo restablece).

**Micro-checkpoint Slice 5** (presentado condensado; aprobado sin cambios): 9 archivos (1 NEW `export-html.ts` + 8 MODIFY/fusión; `styles.css` retirado del skeleton por cero CSS nuevo — botón reusa `.pc-save` + `codicon-export` verificado en el set real; `test/project-map-tab.test.ts` añadido con tests de serializadores reusando fixtures locked). Notas del slice-verifier incorporadas en dos pasadas: (1) VIOLATION Cross-slice — el docs afirmaba que los 4 hovers `.pm-*` "definen fondo+texto" pero `.pm-journey-head:hover` del slice 1 solo declaraba `filter: brightness(1.1)` y el `button:hover` global (`styles.css:2759`, (0,1,1)) le inyectaba el azul primario sobre la base (0,1,0) — síntoma exacto de la guía de cascada → revisión en cascada de 1 línea al fence locked del slice 1 (background list-hoverBackground en el propio :hover) + docs con frase veraz por botón; (2) WARNING — `buildExportHtml` corría ANTES del `showSaveDialog` (bloqueaba el host con lecturas síncronas y desperdiciaba trabajo al cancelar) → orden rehecho a validar-forma → diálogo → ensamblar → escribir; (3) WARNING — import muerto `PmCrossState` en la adición al ProjectMapTab → eliminado; (4) re-verificación focalizada dejó ABIERTA solo la prosa del docs (`.pm-expand-all:hover` declara el color en el propio :hover, no "del base") → redacción exacta del verificador adoptada. Reintrodujo semántica de shots del export: `undefined` = resolver vía host, `""` = sin captura definitivo, data-URI = inlinado — espejo de la semántica del GraphCanvas.

## Design History

- Slice 1: Contrato del tab + mapa funcional host + lista honesta — approved as generated (fixes del slice-verifier incorporados: espejo UI como unión discriminada, criterios con Ordering Constraints 1/4, fixture maxScreens=4 honesto; re-verificación Decisions/Cross-slice/Research OK) · revisión en cascada ratificada en el slice 5: `.pm-journey-head:hover` gana `background: var(--vscode-list-hoverBackground)` (el `button:hover` global le inyectaba el azul primario)
- Slice 2: Grafo SVG funcional + evidencia — approved as generated (fixes del slice-verifier incorporados: criterio de re-monte rehecho — fiel al desmonte condicional del hub y al useEffect de montaje, con shots del store sin re-pedido; `GraphColumn.title` renderizado con `Fragment` importado nombrado — jsx react-jsx, molde App.tsx:2,561; `GraphNode.sub` eliminado por YAGNI; re-verificación Decisions/Cross-slice/Research OK)
- Slice 3: Vista técnica (pi-lens) + re-poll — approved as generated (fixes del slice-verifier incorporados: campo `shots` del slice 2 restaurado en la fusión de `ProjectMapUiState`; import `vi` añadido a test/project-map-lib.test.ts — `globals: false`; Files ampliado a 9 con `functional-inventory.ts` (fusión aditiva del punto de estado host, sin ciclo); comentarios de duración corregidos a ≈69 s reales de sleeps; re-verificación focalizada Decisions/Cross-slice/Research OK)
- Slice 4: Cruce técnico↔funcional (matriz M9) — approved as generated (fixes del slice-verifier incorporados: `ProjectMapHostState` gana `cross?: PmCrossState` — import type desde ./matrix-cross sin ciclo, molde de la fusión `technical?` del slice 3 — porque la asignación `{ ...pmState, cross }` contra la interfaz locked rompía `npm run typecheck` del host (TS2353, invisible para vitest/esbuild); Files ampliado a 10 con `ProjectMapTab.tsx` (plumbing de la prop `cross`) + `test/project-map-tab.test.ts` (tests de componente del render del cruce, molde slices 2-3) + `functional-inventory.ts` (fusión del punto de estado host); re-verificación focalizada Decisions/Cross-slice/Research OK)
- Slice 5: Export HTML autónomo + aterrizaje — approved as generated (dos pasadas de slice-verifier; fixes incorporados: (1) orden del case export_map rehecho a DIÁLOGO→ensamblar→escribir — molde exportUsage :5872-5937, las lecturas síncronas de PNGs (techo 4 MB c/u) solo ocurren tras confirmar y cancelar es no-op; (2) import muerto `PmCrossState` eliminado de la adición al ProjectMapTab (`cross` se infiere de `state.projectMap?.cross`, fence locked importa solo `OutMessage, State`); (3) frase de cascada del docs veraz — `.pm-expand-all:hover` declara el color DENTRO del propio :hover, no "del base"; re-verificación focalizada: FIX A/B/C cerrados. Revisión en cascada sobre el slice 1: `.pm-journey-head:hover` + `background` (VIOLATION Cross-slice — mis-claim del docs sobre CSS locked destapó el hueco real). Bookkeeping: Files corregido — `styles.css` retirado (cero CSS nuevo: botón reusa `.pc-save`), `test/project-map-tab.test.ts` añadido (serializadores, fixtures locked reusados); 9 archivos: 1 NEW + 8 MODIFY/fusión)
...
- Issue #143 — "M2: panel Mapa del proyecto"
