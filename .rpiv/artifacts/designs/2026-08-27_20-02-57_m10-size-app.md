---
date: 2026-08-27T20:02:57-0600
author: Edgar F. Fuentes Perea
commit: 9d6d8bb
branch: main
repository: frida-code
topic: "M10 size-app — dimensionamiento cuantitativo de apps para preventa"
tags: [design, frida-size-app, scc, cocomo, frida-extensible-workflows, skill-pack, installer]
status: ready
parent: .rpiv/artifacts/research/2026-08-27_06-28-43_m10-size-app.md
last_updated: 2026-08-27T20:02:57-0600
last_updated_by: Edgar F. Fuentes Perea
---

# Design: M10 size-app — dimensionamiento cuantitativo de apps para preventa

## Summary

Skill pack `src/tools/frida-size-app/` de 6 archivos (molde M1/M9 + `constants.ts`/`installer.ts` propios) que registra el patrón builtin `size-app` componiendo el motor `frida-extensible-workflows` intacto. El installer `ensureBinary` descarga **scc v4.0.0** al agentDir con sha256 verificado (matrix completa de 8 assets del release, digests reales del `checksums.txt`), disparado fire-and-forget al registrar la factory. El script generado corre `bootstrap → metrics → analyze → synthesize → judge`: determinista al inicio (sondas scc/lizard → `docs/dimensionamiento/artifacts/metrics.json` con writer único) y al final (`dimensionamiento.md` 100% derivado de `metrics.json`: COCOMO con spread EAF 0.85/1.00/1.15, SQALE proxy, olas de migración), agéntico solo en analyze (fan-out de 3 escritores fijos de anexos interpretativos) y judge (rúbrica numérica PASS/CONCERNS/FAIL + checkpoint si `review=manual`). Incluye `checkScc` como 8º check del doctor.

## Requirements

Del FRD `.rpiv/artifacts/discover/2026-08-27_04-15-43_m10-size-app.md` (issue #139):

- Registrar el patrón builtin `size-app` visible en `workflow_catalog` y picker `/wf`, con `meta.moat {lens, codebaseIndex}` y `requiredTools: ["shell"]` (FR-1).
- Validación eager con disciplina D13: `wage` requerido (error que instruye `ask_user_question` pre-launch); `cocomoType` (default `semi-detached`), `exclude[]` (default curada ampliable), `maxMinutes` (1–240, 0=sin tope), `review`, `language` (FR-2).
- Installer `ensureBinary`: scc@PIN al agentDir con checksum verificado, `isInstalledAtPin()` por corrida, degradación con causa+hint si falla (FR-3).
- Fase bootstrap determinista: `docs/dimensionamiento/`, capacidades, epoch vía shell (FR-4).
- Fase metrics determinista: sondas `scc --by-file --format json`, `scc -a`, `scc --hotspots --coupling --by-author`, `lizard --csv` solo si existe; normalización a `metrics.json` con writer único (FR-5).
- Exclusiones default SIEMPRE aplicadas + `exclude[]` usuario; el informe declara qué se excluyó y el volumen (FR-6).
- Familias solo si su fuente existe; ausentes → degradación `{familia, causa, hint}` en `metrics.json` y "no disponible" en el informe (FR-7).
- Fase analyze: fan-out `parallel()` sobre tareas derivadas del inventario, `outputSchema` + reportes a rutas deterministas (FR-8).
- Fase synthesize determinista: KLOC efectivos, distribución p50/p90/p99, SQALE proxy `deuda/(0.5h × NCLOC)`, COCOMO con spread EAF 0.85/1.00/1.15 → rango persona-mes + costo (wage), olas de migración por hotspots (FR-9).
- Fase judge detached: PASS/CONCERNS/FAIL; familia declarada sin sustento = FAIL; familia degradada = CONCERNS; corte por presupuesto = CONCERNS; checkpoint final solo si `review=manual` (FR-10).
- `maxMinutes` corta el descubrimiento pero NUNCA salta analyze/synthesize/judge sobre lo alcanzado (FR-11).
- Piloto sobre Apache OFBiz con números de evidencia en el issue #139 (criterio de aceptación).

## Current State Analysis

El molde está validado dos veces (M1 `frida-understand-app` #134, M9 `frida-traffic2api` #135) y el seam de registro publica el patrón sin tocar el motor: `registerBuiltinPattern` (`src/tools/frida-extensible-workflows/builtin-patterns.ts:481`) es splice/push idempotente por nombre sin validación runtime — todo el contrato vive en el pack (tipos TS en compile-time). M9 entregó con "motor intacto (0 líneas)".

### Key Discoveries

- **scc v4.0.0** (2026-08-24) es la ÚNICA versión con `--hotspots/--coupling/--by-author/--cognitive` (v3.4.0 no las tiene). Assets del release: 8 binarios + `checksums.txt` sha256 único (formato sha256sum, 2 espacios). Nomenclatura SIN versión: `scc_<Os-title>_<arch>.tar.gz` (Unix) / `.zip` (Windows), amd64→x86_64. Binarios ~6.3-7.2 MB. MIT.
- **Digests reales v4.0.0** (obtenidos del `checksums.txt` en este design, para `SCC_DIGESTS`): ver `src/tools/frida-size-app/constants.ts` en Architecture.
- **scc implementa Basic COCOMO 81** (no II): constantes `processor/cocomo.go` organic {2.4, 1.05, 2.5, 0.38}, semi-detached {3.0, 1.12, 2.5, **0.35**}, embedded {3.6, 1.20, 2.5, **0.32**}; TDEV = 2.5·E^d con d variable por modo; costo = E·(avgWage/12)·**overhead 2.4** con wage ANUAL default 56286 USD. Nuestro synthesize replica estas constantes EXACTAS con wage MENSUAL preguntado (≡ anual/12) — números idénticos a la salida nativa de scc (decisión del checkpoint de research).
- **No existe primitiva tar.gz en el árbol**: el único `node:zlib` es `fetch.ts` (inflateRawSync); `unzipSync` (`src/tools/frida-cc-plugins/fetch.ts:126`) SÍ está exportada y cubre los assets win32 `.zip`. La extracción tar.gz es más simple que zip (bloques 512 bytes, sin central directory; gzip aporta CRC) — parser mínimo sobre `zlib.gunzipSync` con guard anti path-traversal (tar-slip).
- **`resolve()` es síncrona por contrato** (`builtin-patterns.ts:389`, invocada sin await en `index.ts:331` del motor): la sonda scc es síncrona (marker propio + `existsSync` del binario); `ensureBinary` (async) se dispara fire-and-forget desde la factory (molde `src/tools/frida-hermes-memory/index.ts:181-190`).
- **Sandbox del motor** (`core/execution.ts:319-323`): `JSON`/`Math` inyectados (`Math.pow` ✓ → COCOMO computable), `Date`/`Intl`/`console` tapados (tiempo vía `date +%s`, slugs/tablas manuales, formatos `toFixed`). **RPC 10 MB** (`execution.ts:45`): `scc --by-file --format json` puede rebasar → volcar a archivo y procesar con helper node en el host que devuelve stdout delgado (molde fase ingest M9, `src/tools/frida-traffic2api/workflow.ts:950-970`).
- **Salidas scc v4.0.0** (contrato del binario, base del parser y de los mocks): `--by-file --format json` → array LanguageSummary con `Files[]` de FileJob (`Location/Language/Lines/Code/Comment/Blank/Complexity/Cognitive/WeightedComplexity/Bytes/Uloc/Minified/Generated`); `-a` → DRYness+ULOC; `--hotspots` CSV `File,Language,Complexity,Commits,LinesChanged,Authors,CodeChurn,CommentChurn,Score`; `--coupling` CSV `FileA,FileB,Shared,CommitsA,CommitsB,Degree`; `--by-author` CSV `Author,Email,Code,Complexity,Comment,Files,OwnsPercent,LastCommit,BeforeWindow`. Fuera de un repo git, `--hotspots/--coupling/--by-author` omiten el reporte (degradación natural por familia).
- **Exclusiones scc**: `--exclude-dir` aditivo al default `[.git,.hg,.svn]`; respeta `.gitignore`/`.ignore` por defecto. node_modules NO es caso especial — la default curada del FRD es necesaria.
- **`requireStringArray` es privada del motor y rechaza `[]`** (`builtin-patterns.ts:57-68`) — semántica inadecuada para `exclude[]` (vacío = solo defaults): el pack lleva helper local.
- **Doctor**: `checkEnvironment` (`src/environment/doctor.ts:502`) agrega 7 checks con `Promise.all`; 3 congelaciones del 7 en `test/environment-doctor.test.ts:187,297,309`. La firma `deps` acepta `exec/platform/arch` pero NO `agentDir` — `checkScc` la extiende. `src/extension.ts:3445` es el único call site productivo (habría que pasarle `defaultAgentDir()`). Cero cambios webview (campos existentes bastan; icono default `tools` en `EnvironmentTab.tsx:25-26`).
- **Wiring de publicación M9** (replicable): entrada `HELP_TOOLS` tras traffic2api (`src/extension.ts:4074-4091`, alias sin colisión first-match), fila README entre traffic2api y sandboxes (`README.md:214`), docs par (~230-260 líneas con esqueleto fijo), roadmap fila 105 (celda dependencia `scc ⚠️` → `scc ✅`; Prioridad `**P2**` → `**P2** ✅`). `.vscodeignore` NO aplica: la descarga es runtime al agentDir, no build-time.
- **Tests del pack** (molde documentado): trío pattern/resolver/e2e + dominio. E2E corre el script en el MOTOR REAL (`runWorkflowInStore`, `frida-host.ts:325`) con `spawnAgent` mockeado por anclas verbatim y binarios falsos en `.mock-bin` (PATH antepuesto); FAKE_DATE con contador en disco (+30 s); mocks honestos que escriben archivos reales; `clearRegisteredBuiltinPatterns()` en afterEach; never conteos globales del catálogo; validar mocks bash con `bash -n` durante desarrollo.
- **Lecciones de contrato del binario** (`30ef616`, `59517f7`, `9d6d8bb` — 3 veces): el contrato del binario externo miente en los mocks. **Smoke REAL de scc v4.0.0 sobre OFBiz ANTES de congelar los mocks del e2e** (paso del plan, no de este diseño).

## Scope

### Building

- `src/tools/frida-size-app/` completo: `constants.ts`, `installer.ts`, `skills.ts`, `resolver.ts`, `workflow.ts`, `index.ts`.
- `ensureBinary`: descarga https (solo https, redirects https, límite 256 MiB) + sha256 ANTES de extraer + extracción tar.gz nueva (Unix) / `unzipSync` existente (win32) + copia a `<agentDir>/bin/scc` + `chmod 0o755` (no-win32) + marker de pin; idempotente por `isSccInstalledAtPin`.
- Patrón builtin `size-app` con args `{wage (req), cocomoType, exclude[], maxMinutes, review, language, currency}` y script de 5 fases (`bootstrap/metrics/analyze/synthesize/judge`).
- `metrics.json` como única fuente de verdad (writer único) con familias, exclusiones declaradas con volumen, degradaciones `{familia, causa, hint}`.
- `dimensionamiento.md` + `README.md` 100% deterministas desde `metrics.json`: COCOMO 3 corridas EAF, SQALE proxy, percentiles nearest-rank, olas de migración 1/3+1/3+resto, bus factor, hotspots/coupling/autores.
- Anexos agénticos `docs/dimensionamiento/analisis/{hotspots,deuda-modulos,riesgos-tamano}.md` (fan-out de 3 escritores fijos).
- `checkScc` en el doctor (8º check, `deps.agentDir` inyectable) + descongelar tests 7→8 + pasar `defaultAgentDir()` en `extension.ts:3445`.
- Trío de tests + dominio COCOMO + installer tests (sin red, deps inyectables).
- Publicación: `pi-session.ts` entrada, `HELP_TOOLS`, fila README, docs par, roadmap ✅.
...
### D1: Pack de 6 archivos (constants + installer separados)

**Directional confirmada.** El FRD decía "4 archivos + installer"; el molde `frida-codebase-index` (`constants.ts:9` pin/spec, `installer.ts:170` ensureInstalled) separa constantes de installer, y el research recomendó `constants.ts` propio. **Decisión**: 6 archivos — `constants.ts` (SCC_PIN/SCC_ASSETS/SCC_DIGESTS/currentSccAsset) + `installer.ts` (ensureBinary/isSccInstalledAtPin/extractTarGz) aparte. El bump deliberado de pin (editar juntos PIN+ASSETS+DIGESTS) queda visible en un solo archivo.

### D2: Disparo fire-and-forget al arranque

**Directional confirmada.** `resolve()` es síncrona → `ensureBinary` no puede correr ahí. Molde hermes (`index.ts:181-190`): `void doEnsureInstalled(...).then(...).catch(...)` al registrar la factory — nunca bloquea ni tumba la sesión. La primera sesión tras instalar Frida descarga scc (~7 MB) al agentDir; gate `isSccInstalledAtPin` lo hace no-op después. La primera corrida de size-app ya encuentra el binario. Corrida protegida por `CAPABILITIES.scc=false` + degradación determinista si la descarga aún no terminó.

### D3: Frontera determinista/agéntica — determinismo total del informe

**Directional confirmada.** `dimensionamiento.md` y `README.md` se derivan EXCLUSIVAMENTE de `metrics.json` (FR-9). El fan-out de analyze son 3 escritores FIJOS de insumos interpretativos bajo `docs/dimensionamiento/analisis/` (molde `ANALYZE_WRITERS` M1 `workflow.ts`): `hotspots.md` (narrativa de top hotspots), `deuda-modulos.md` (deuda por módulo), `riesgos-tamano.md` (riesgos de tamaño). `dimensionamiento.md` cita esos anexos por ruta; ninguna cifra del informe vive fuera de `metrics.json`. El juez de números: cada cifra o está en `metrics.json` tal cual o se re-deriva con la fórmula declarada.

### D4: SCC_ASSETS matrix completa (8 assets, digests reales)

**Decisión del checkpoint.** El piso FRD era darwin/arm64 + linux/x64; el techo natural es la matrix completa del release v4.0.0. **Decisión**: 8 assets con tabla explícita `platform-arch → asset` (lección codebase-index `constants.ts:51-57`: los sufijos NO son derivables) + `SCC_DIGESTS` con los 8 sha256 REALES obtenidos del `checksums.txt` v4.0.0 en este design. Costo marginal cero; `unzipSync` (exportada, `fetch.ts:126`) cubre win32; ningún usuario de Frida queda fuera. `currentSccAsset()` devuelve `undefined` → degradación con guía para plataformas exóticas (musl, freebsd).

### D5: checkScc incluido en M10

**Decisión del checkpoint.** El FRD lo listaba como follow-up ("no requerida para runtime"); el impacto ya está mapeado (doctor.ts + 3 congelaciones de test + `defaultAgentDir()` en extension.ts:3445; cero webview). **Decisión**: incluir. Check honesto: scc al pin en agentDir = installed con versión; ausente = `category: "extension"` con guía accionable, no error. `deps.agentDir?: string` inyectable (patrón idéntico a `exec/platform/arch`).

### D6: COCOMO replicando scc exacto (decisión heredada del checkpoint de research)

Constantes `{a,b,c,d}` por modo con d=0.35 semi-detached; E = a·KSLOC^b·EAF (persona-mes); TDEV = 2.5·E^d (meses); personas pico = E/TDEV; costo = E·wageMensual·overhead 2.4. Spread EAF 0.85/1.00/1.15 (etiquetado explícitamente "supuesto conservador del analista, no estándar" — lo documentado es "0.9 a 1.4 típico"). Etiqueta del informe: "Basic COCOMO 81 (Boehm)". Números idénticos a la salida nativa de scc.

### D7: wage requerido pre-launch (D13) + currency etiqueta

`wage` es requerido A PROPÓSITO (molde `maxHotspots` M1): número > 0, `Number.isFinite`, SIN `Number.isInteger` (montos con decimales válidos — molde `adversarial-review.threshold`, `builtin-patterns.ts:443-450`: número 0-1 sin chequeo de enteridad; la redacción original citaba `reviewers`, que exige entero — corregido en Slice 3 tras verificación), interpretado MENSUAL. `currency` opcional default `"USD"` — etiqueta pura del informe (sin conversión). El error de wage ausente instruye `ask_user_question` con opciones típicas MXN/USD embebidas.

### D8: Default curada de exclusiones + medición del volumen excluido

Default SIEMPRE aplicada (aditiva a la de scc): `--exclude-dir dist,build,node_modules,vendor,target,out,.next,coverage` + regex para `*.min.js`. `exclude[]` del usuario AMPLÍA (helper local del pack — `requireStringArray` del motor es privada y rechaza `[]`; vacío = solo defaults). El volumen excluido se MIDE con una segunda pasada `scc --by-file` SIN la default curada (solo defaults git de scc): delta LOC/archivos por directorio → `metrics.json.exclusions` y tabla del informe. Dos pasadas by-file son baratas (~segundos en ~1M LOC).

### D9: Métricas derivadas — fórmulas declaradas

- **KLOC efectivos**: SLOC (Code) total de la corrida con exclusiones.
- **Percentiles p50/p90/p99 nearest-rank**: `sorted[ceil(p·N)−1]` sobre complejidad (scc, por archivo, siempre) y CCN (lizard, por función, si existe) — etiquetados distintamente; sin Intl, tie-break binario determinista.
- **SQALE proxy (etiquetado "proxy, no es SonarQube")**: `deudaHoras = Σ por archivo max(0, cognitive−15) × 0.5h` (umbral 15 = estándar SonarQube de complejidad alta); `rating = deudaHoras / (0.5h × NCLOC)` con umbrales SonarQube A ≤0.05, B ≤0.10, C ≤0.20, D ≤0.50, E >0.50.
- **Bus factor**: acumular autores por `Code` desc hasta cubrir ≥50% del Code total → ese conteo (derivado del CSV `--by-author`).
- **Olas strangler-fig**: módulos = top-level dirs con LOC ≥ 1% del total; ordenados por deudaHoras desc; ola 1 acumula hasta ~1/3 de la deuda total, ola 2 el siguiente 1/3, ola 3 el resto; semanas por ola = fracción de deuda × TDEV(EAF central) en semanas. Fórmula en `metrics.json.derived.waves` para auditoría.
- **Deuda por módulo**: Σ deudaHoras de archivos agrupados por top-level dir.

### D10: RPC 10 MB — by-file via archivo + helper node host-side

`scc --by-file --format json > artifacts/scc-by-file.json` (redirect de shell, sin límite RPC) → helper node escrito a disco que corre EN EL HOST vía `shell("node …")` (molde carve M9), lee el JSON gordo, agrega (por lenguaje, por módulo, complejidades) y devuelve stdout delgado (`JSON.parse(r.stdout)` en el sandbox). El JSON crudo queda como evidencia auditable en `artifacts/`.

### D11: Stages agénticos = analyze + judge (2 claves de resolver)

bootstrap/metrics/synthesize son deterministas (sin prompt overrideable — los invariantes viven en el código del script). El veto de solo-escritura y el "juez de números" viajan en `SIZE_APP_PREAMBLE` (no-stage, sobrevive overrides). La regla cut→CONCERNS vive DOS veces (prompt default de judge + runtime block) para sobrevivir overrides — molde M9.

### D12: Fence SA_EOF + naming

Fence heredoc `SA_EOF` (análogo `UA_EOF`/`T2A_EOF`). Dir de entregables `docs/dimensionamiento`. Tests `sess-size-N`. Helper local `requireStringArrayAllowEmpty` en workflow.ts del pack.

## Architecture

### src/tools/frida-size-app/constants.ts — NEW

Constantes del binario scc: pin, tabla plataforma→asset (claves Node), digests reales v4.0.0, URL builder, rutas del binario/marker.
...
### src/tools/frida-size-app/installer.ts — NEW

`ensureBinary` con fetch/sha256/extract inyectables + `extractTarGz` (primitiva nueva del árbol) + `isSccInstalledAtPin` + error con guía accionable.
...
### src/tools/frida-size-app/skills.ts — NEW

Stages `["analyze","judge"]`, preamble no-stage con vetos + juez de números, prompts default es-MX.
...
### src/tools/frida-size-app/resolver.ts — NEW

Resolver 3-capas (defaults → `.frida/size-app/stages.json` → `~/.frida/size-app/stages.json`) sobre `createLayeredStageResolver` de frida-aidd.
...
### src/tools/frida-size-app/workflow.ts — NEW

Args + validación eager (D13) + generador COMPLETO del script de 5 fases (Slices 3→5): tipos del contrato (`SizeAppArgs`/`SizeAppCapabilities`/`CocomoType`), `validateSizeAppArgs`, constantes COCOMO/anexos exportadas, y el generador con bootstrap/metrics deterministas (Slice 4), fan-out analyze (Slice 5), synthesize determinista con `derived` (KLOC/COCOMO 3-EAF/SQALE/olas/bus factor, Slice 5) y judge detached con checkpoint condicional (Slice 5). Validado con harness ejecutable: template compila, script generado JS válido, corrida degradada sin abortar (kloc null + 9 degradaciones + judge CONCERNS), y matemática COCOMO exacta (kloc=100 → 443.1/521.3/599.5 PM = 521.3×0.85/1.0/1.15).
...
### src/tools/frida-size-app/index.ts — NEW

Patrón `size-app` + sonda de capacidades síncrona (scc marker + moat) + factory con fire-and-forget `ensureBinary`.
...
Entrada `frida-size-app` en `extensionFactories` junto a M9, con `agentDir` + getter del toggle.
...
### src/environment/doctor.ts — MODIFY

`checkScc` (8º check) + `deps.agentDir` en `checkEnvironment`.
...
### test/environment-doctor.test.ts — MODIFY

Descongelar 7→8 (líneas 187, 297-título, 309).
...
Entrada `HELP_TOOLS` (tras traffic2api, alias sin colisión) + pasar `defaultAgentDir()` a `checkEnvironment()` en el case `check_environment`.
...
Fila `frida-size-app` en la tabla Herramientas (entre traffic2api y sandboxes).
...
### docs/tools/frida-size-app.md — NEW

Doc de referencia técnica (esqueleto fijo de los hermanos, ~230 líneas).
...
### docs/how-to-frida-size-app.md — NEW

Guía de uso (esqueleto fijo, ~250 líneas).
...
### docs/roadmap-extensiones.md — MODIFY

Fila M10: dependencia `scc ⚠️ (bundling por decidir)` → `scc ✅ (pin v4.0.0 al agentDir)`; Prioridad `**P2**` → `**P2** ✅`.
...
### test/frida-size-app/installer.test.ts — NEW

Tests del installer sin red: deps inyectables, tar.gz fixture armada en test, sha256 negativo con estado inmutable, tar-slip, plataformas inyectables.
...
### test/frida-size-app/resolver.test.ts — NEW

3 capas + veto y juez de números solo en preamble (molde M1) + claves de fases deterministas ignoradas.
...
### test/frida-size-app/pattern.test.ts — NEW

Validación eager (wage D13), sonda de capacidades, forma del script, registro/idempotencia/CAPABILITIES por agentDir.
...
### test/frida-size-app/e2e.test.ts — NEW

Corridas e2e sobre el motor real con scc falso instalado al pin en el HOME aislado (ruta absoluta SCC_BIN, no PATH), FAKE_DATE parametrizable, juez por anclas, degradaciones congeladas, FAIL sin abortar, determinismo.
...
### test/frida-size-app/cocomo-domain.test.ts — NEW

Verification Note V2: schema TypeBox del bloque COCOMO + fixture congelada (521.3 PM) + anti-fixtures + corrida e2e compacta verificando `derived.cocomo` deep-equal y el borde SQALE B en el informe.
...
### Slice 1: Fundaciones del binario — constants + installer

**Files**: `src/tools/frida-size-app/constants.ts`, `src/tools/frida-size-app/installer.ts`, `test/frida-size-app/installer.test.ts`

#### Automated Verification

- [ ] Typecheck limpio: `npm run typecheck && npm run typecheck:test`
- [ ] Tests del installer en verde: `npx vitest run test/frida-size-app/installer.test.ts` (8 casos)
- [ ] Negativo de integridad congela estado inmutable: sha incorrecto → error matcheable `/sha256|integridad/` y sin binario/marker (caso "sha256 incorrecto")
- [ ] Guardián del bump en verde: caso "SCC_DIGESTS cubre exactamente SCC_ASSETS" (8↔8)
...
### Slice 2: Prompts y resolver 3-capas

**Files**: `src/tools/frida-size-app/skills.ts`, `src/tools/frida-size-app/resolver.ts`, `test/frida-size-app/resolver.test.ts`

#### Automated Verification

- [ ] Typecheck limpio: `npm run typecheck && npm run typecheck:test`
- [ ] Tests del resolver en verde: `npx vitest run test/frida-size-app/resolver.test.ts` (6 casos)
- [ ] Solo 2 claves overrideables: el caso de fases deterministas (bootstrap/metrics/synthesize + stage hermano + valores vacíos/no-string) deja todo en defaults
- [ ] Invariantes fuera del alcance de overrides: caso "el veto de solo-escritura y el juez de números viven SOLO en el preamble no-stage" en verde

#### Manual Verification

- [ ] Prompts default es-MX coherentes con el design: analyze (escritor de anexo: ruta exacta, metrics.json única fuente, sin re-sondear) y judge (familia sin sustento=FAIL, degradada=CONCERNS, corte por presupuesto=CONCERNS, etiquetas de método honestas)
- [ ] El prompt default de judge contiene la regla corte por presupuesto → CONCERNS (FR-11/D11 — el duplicado runtime llegará con el script en Slice 5)

### Slice 3: Args y validación eager (workflow.ts parte 1)

**Files**: `src/tools/frida-size-app/workflow.ts`

#### Automated Verification

- [ ] Typecheck limpio: `npm run typecheck && npm run typecheck:test`
- [ ] Tests previos del pack siguen en verde (regresión Slices 1-2): `npx vitest run test/frida-size-app/`
- [ ] Error de wage instruye pre-launch (FR-2/D7): `grep -c "ask_user_question" src/tools/frida-size-app/workflow.ts` ≥ 1 y `grep -c "MXN" src/tools/frida-size-app/workflow.ts` ≥ 1
- [ ] wage sin chequeo de enteridad, maxMinutes sí (D7): `grep -c "Number.isInteger(record.maxMinutes)" src/tools/frida-size-app/workflow.ts` = 1 y `grep -c "Number.isInteger(record.wage)" src/tools/frida-size-app/workflow.ts` = 0
- [ ] Helper local del pack presente (D8/D12): `grep -c "requireStringArrayAllowEmpty" src/tools/frida-size-app/workflow.ts` ≥ 2 (definición + uso)

#### Manual Verification

- [ ] validateSizeAppArgs: wage acepta decimales (35000.50) y rechaza 0/negativos/NaN; cocomoType inválido lista los 3 modos; exclude acepta [] (= solo defaults); maxMinutes entero 1-240 (0/omiso = sin tope); review default "manual"; language default es-MX; currency default "USD" etiqueta pura
- [ ] El mensaje de wage ausente embebe las opciones del Desired End State (MXN $35,000 / USD $6,000 / monto propio) y mapea wage+currency explícitos
- [ ] SizeAppArgs/SizeAppCapabilities documentados sin forward-refs: no referencian símbolos que llegan en Slices 4-5 (generateSizeAppWorkflow, sondas, script)

### Slice 4: Generador — bootstrap + metrics (workflow.ts parte 2)
...
- [ ] Typecheck limpio: `npm run typecheck && npm run typecheck:test`
- [ ] Regresión Slices 1-3 en verde (workflow.ts aún sin test propio — trío llega en Slices 6-7): `npx vitest run test/frida-size-app/`
- [ ] Generador exportado con firma completa: `grep -c "export function generateSizeAppWorkflow" src/tools/frida-size-app/workflow.ts` = 1
- [ ] Fence propio (D12): `grep -c "SA_EOF" src/tools/frida-size-app/workflow.ts` >= 3
- [ ] scc JAMÁS del PATH (ruta absoluta shq-quoted siempre): `grep -c "shq(SCC_BIN)" src/tools/frida-size-app/workflow.ts` >= 5
- [ ] Helper host-side (D10): `grep -c "metrics-agg.js" src/tools/frida-size-app/workflow.ts` >= 2 (escritura + corrida node) y by-file via redirect: `grep -c "scc-by-file.json" src/tools/frida-size-app/workflow.ts` >= 2 y `grep -c "scc-by-file-raw.json" src/tools/frida-size-app/workflow.ts` >= 2
- [ ] Default curada SIEMPRE + sanitización (D8): `grep -c "CURATED_EXCLUDE_DIRS" src/tools/frida-size-app/workflow.ts` >= 3 y `grep -c '"node_modules"' src/tools/frida-size-app/workflow.ts` = 1
- [ ] Gates por familia (FR-7): `grep -c "command -v lizard" src/tools/frida-size-app/workflow.ts` = 1, `grep -c "registerDegradation" src/tools/frida-size-app/workflow.ts` >= 3, `grep -c "hasDegradation" src/tools/frida-size-app/workflow.ts` >= 2
- [ ] Percentiles nearest-rank + SQALE insumo (D9): `grep -c "p99" src/tools/frida-size-app/workflow.ts` >= 2 y `grep -c "SQALE_THRESHOLD" src/tools/frida-size-app/workflow.ts` >= 2

#### Manual Verification

- [ ] metrics.json shape completo: run (wage/currency/cocomoType/exclude user/curatedExclude/maxMinutes/review/sccVersion/startedAt), capabilities (scc/sccPin/git/lizard/lens/codebaseIndex), exclusions[] con volumen, families con status ok|absent|empty|parse-error por familia, degradations {familia, causa, hint}, derived: null (se llena en synthesize)
- [ ] Degradaciones deterministas de bootstrap: scc ausente ×5 familias, sin git ×3, lizard ausente ×1 — causa+hint accionables, la corrida NO aborta (D2)
- [ ] Percentiles etiquetados distintamente: complexity (scc, por archivo) en families["by-file"].percentiles vs ccn (lizard, por función) en families["ccn-funcion"].percentiles; samples registrado en ambos
- [ ] debtHours = Σ max(0, cognitive−15) × 0.5h por archivo (agregado por módulo y total; constantes SQALE_THRESHOLD/SQALE_HOURS_PER_POINT del helper); el rating SQALE llega en derived (Slice 5)
- [ ] Exclusiones declaradas con volumen medido por delta de la 2ª pasada raw + patrón *.min.js con su volumen; min.js excluido de la agregación
- [ ] CSVs parseados tolerantes (by-author from-right para autores con coma; caps top-200/100/10 con truncated/skipped honestos); sort determinista con tie-break lexicográfico
...
### Slice 5: Generador — analyze + synthesize + judge (workflow.ts parte 3)
...
- [ ] Typecheck limpio: `npm run typecheck && npm run typecheck:test`
- [ ] Regresión Slices 1-4 en verde (trío de tests llega en Slices 6-7): `npx vitest run test/frida-size-app/`
- [ ] COCOMO 3 corridas EAF (D6): `grep -cF "EAF_SPREAD" src/tools/frida-size-app/workflow.ts` = 3 y `grep -cF "COCOMO_CONSTANTS" src/tools/frida-size-app/workflow.ts` = 2
- [ ] Etiqueta del método + costo con overhead (D6): `grep -cF '"## COCOMO — Basic COCOMO 81 (Boehm), tipo "' src/tools/frida-size-app/workflow.ts` = 1 y `grep -cF "cost: Math.round(effort * wage * OVERHEAD)" src/tools/frida-size-app/workflow.ts` = 1
- [ ] Fan-out 3 escritores fijos (D3/FR-8): `grep -cF "riesgos-tamano" src/tools/frida-size-app/workflow.ts` = 3 y `grep -cF "deuda-modulos" src/tools/frida-size-app/workflow.ts` = 3
- [ ] Fases 3-5 presentes: `grep -cF 'phase("analyze")' src/tools/frida-size-app/workflow.ts` = 1, `grep -cF 'phase("synthesize")'` = 1, `grep -cF 'phase("judge")'` = 1
- [ ] Writer único de entregables (D3): `grep -cF 'writeText(ART + "/dimensionamiento.md"' src/tools/frida-size-app/workflow.ts` = 1 y `grep -cF 'writeText(ART + "/README.md"' src/tools/frida-size-app/workflow.ts` = 1
- [ ] Regla corte→CONCERNS runtime duplicada + deadline (D11/FR-11): `grep -cF "CONCERNS, no FAIL" src/tools/frida-size-app/workflow.ts` = 1 y `grep -cF "const deadline" src/tools/frida-size-app/workflow.ts` = 1
- [ ] Derivadas D9: `grep -cF "computeBusFactor" src/tools/frida-size-app/workflow.ts` = 2, `grep -cF "buildWaves" src/tools/frida-size-app/workflow.ts` = 2, `grep -cF "sqaleRating" src/tools/frida-size-app/workflow.ts` = 2, `grep -cF "const ratio = debtHours / (0.5 * BF.loc)" src/tools/frida-size-app/workflow.ts` = 1
- [ ] Checkpoint solo manual (FR-10): `grep -cF 'if (review === "manual") {' src/tools/frida-size-app/workflow.ts` = 1

#### Manual Verification

- [ ] Constantes COCOMO exactas de scc (D6): organic {2.4,1.05,2.5,0.38}, semi-detached {3.0,1.12,2.5,0.35}, embedded {3.6,1.2,2.5,0.32}; costo = E·wage·2.4; personas = E/TDEV; tabla 3 filas con central EAF 1.00 en bold y fórmulas a pie con etiqueta "Basic COCOMO 81 (Boehm)" y spread como supuesto del analista (no estándar, 0.9–1.4 documentado)
- [ ] derived completo y auditable: kloc full precision (loc/1000), sqale {ratio 3 dec, rating A-E, formula, thresholds}, cocomo {constants, overhead, wageMonthly, currency, klocSource, rows}, percentiles etiquetados complexity (scc/archivo) vs ccn (lizard/función), waves {eligibleModules, consideredDebtHours, tdevCentralMonths, weeksPerMonth, waves[{modules, moduleCount, debtHours, share, weeks}]}
- [ ] Informe 100% desde metrics.json (D3): resumen ejecutivo, COCOMO, percentiles, SQALE "proxy, NO es SonarQube", deuda por módulo, olas, autores/bus factor, hotspots, coupling, duplicación, exclusiones con volumen, degradaciones→"no disponible" con causa, anexos citados por ruta (o corte honesto), cómo auditar
- [ ] Rama no computable (by-file absent/0 LOC): sección "no computable" + familias independientes siguen renderizándose; la corrida NO aborta (D2/FR-7)
- [ ] Escapes doblados verificados (lección S4 extendida a strings): el script generado contiene `\n` literales en runtime blocks, `\"$f\"` en el gate, mdCell `.replace(/\|/g, "\\\\|")` y fmtInt `/\B(?=(\d{3})+(?!\d))/g`; sin `${` literal ni backticks sin escapar
- [ ] Olas greedy 1/3-1/3-resto reservando ≥1 módulo por ola restante; semanas = share × TDEV central × 52/12; sin deuda considerada (>0) o TDEV central ≤ 0 → `derived.waves.waves = []` (early-return documentado en buildWaves; el spread EAF es fijo y rows siempre = 3, así que el TDEV central nunca falta cuando derived.computed)
- [ ] Gate test -s + reintento informado único por anexo (molde M1 lessons d203630/619d9e7); tras reintento fallido aborta con diagnóstico ls
- [ ] Judge: veredicto viaja en el return sin abortar; checkpoint solo si review=manual; el README NO contiene el veredicto del juez (corre después); la sección "Cómo auditar" redacta el juez en presente/atermporal ("incluye el veredicto… que muestrea"), sin pretérito pre-juez

### Slice 6: Patrón + factory + registro

**Files**: `src/tools/frida-size-app/index.ts`, `src/pi-session.ts`, `test/frida-size-app/pattern.test.ts`

#### Automated Verification

- [ ] Typecheck limpio: `npm run typecheck && npm run typecheck:test`
- [ ] Tests del patrón en verde: `npx vitest run test/frida-size-app/pattern.test.ts` (31 casos)
- [ ] V6 — la instalación nunca bloquea: `npx vitest run test/frida-size-app/pattern.test.ts -t "aunque ensureBinary rechace"` en verde
- [ ] Regresión Slices 1-5 en verde: `npx vitest run test/frida-size-app/`
- [ ] Entrada registrada en la sesión: `grep -c "createFridaSizeApp" src/pi-session.ts` = 2 y `grep -c "SIZE_APP_FACTORY_NAME" src/pi-session.ts` = 2 (import + name)
- [ ] Sonda síncrona + fire-and-forget presentes: `grep -c "scc: isSccInstalledAtPin(agentDir)" src/tools/frida-size-app/index.ts` = 1, `grep -c "if (!isSccInstalledAtPin(agentDir))" src/tools/frida-size-app/index.ts` = 1, `grep -c "void ensureBinary" src/tools/frida-size-app/index.ts` = 1
- [ ] Registro runtime idempotente: `grep -c "registerBuiltinPattern(pattern)" src/tools/frida-size-app/index.ts` = 1

#### Manual Verification

- [ ] args del patrón documenta wage requerido con opciones MXN/USD embebidas (D7/D13) y el resto de args (currency/cocomoType/exclude/maxMinutes/language/review); la description menciona docs/dimensionamiento/ y metrics.json como única fuente de verdad
- [ ] FR-1: patrón visible en workflow_catalog y picker /wf con meta.moat {lens, codebaseIndex} y requiredTools ["shell"], postura autónoma
- [ ] resolve() en 3 pasos espejo M1/M9: validate eager → resolver 3 capas con ctx.cwd del launch → genera con CAPABILITIES re-sondeada por resolve + SCC_BIN ruta absoluta (jamás PATH, D12)
- [ ] Fire-and-forget D2: gate isSccInstalledAtPin antes de disparar; catch → console.warn con e.guide (molde moat-factories.ts:92); log de éxito solo en producción (ensureDeps undefined); la factory JAMÁS bloquea ni tumba la sesión (V6)
- [ ] Entrada pi-session junto a traffic2api con agentDir + getter codebaseIndexEnabled (misma forma M1/M9); name vía SIZE_APP_FACTORY_NAME (contrato del Slice 1); sin toggle propio
- [ ] ensureDeps documentado como seam de tests sin red; producción (pi-session) NO lo pasa

### Slice 7: E2E + dominio COCOMO

**Files**: `test/frida-size-app/e2e.test.ts`, `test/frida-size-app/cocomo-domain.test.ts`

#### Automated Verification

- [ ] Typecheck limpio: `npm run typecheck && npm run typecheck:test`
- [ ] E2E en verde (9 casos): `npx vitest run test/frida-size-app/e2e.test.ts`
- [ ] Dominio en verde (7 casos): `npx vitest run test/frida-size-app/cocomo-domain.test.ts`
- [ ] Regresión Slices 1-6 intacta: `npx vitest run test/frida-size-app/` completo en verde
- [ ] V2 congelada: `npx vitest run test/frida-size-app/cocomo-domain.test.ts -t "fixture congelada"` verifica E(1.00)=521.3 PM (443.1/599.5 en bordes) con literales del test
- [ ] V5 plumbing: `npx vitest run test/frida-size-app/e2e.test.ts -t "caso negativo del juez"` (FAIL viaja en el return sin abortar)
- [ ] Fan-out honesto: `npx vitest run test/frida-size-app/e2e.test.ts -t "escritor"` cubre mentiroso (gate + reintento + falta) y flaky (reintento rescata)

#### Manual Verification

- [ ] El fake scc emite el contrato v4.0.0 documentado (FileJob Location/Language/Lines/Code/Comment/Blank/Complexity/Cognitive; CSVs hotspots 9 col/coupling 6 col/by-author 9 col con header; -a con DRYness/ULOC); el smoke real V1 (paso del plan) corre ANTES de implementar este slice y ajusta los canned si el contrato observado difiere (Ordering Constraints, lecciones 30ef616/59517f7/9d6d8bb)
- [ ] Los bash mocks (SCC_MOCK ×2, FAKE_DATE, LIZARD_MOCK) validan con `bash -n` durante el desarrollo
- [ ] e2e feliz congela: kloc 3.6 (4 archivos/3600 SLOC curated + min.js 5000 filtrado), exclusiones dist 1/3,000 + node_modules 1/2,000 + *.min.js 1/5,000 por delta de la 2ª pasada raw, COCOMO 10.7/12.6/14.5 PM con costs 899,245/1,057,936/1,216,626 (wage 35,000 MXN), deuda 22.5 h, SQALE A ratio 0.013 (raw 0.0125), bus factor 1, olas core/app/lib 56%/33%/11% → 14.7/8.8/2.9 semanas (tdev central 6.1), percentiles complexity 20/60/60 + ccn 5/10/10
- [ ] Degradaciones congeladas: total sin scc/git/lizard = 9 (composición locked 5+3+1); by-file corrupto mid-run = 5 (3 git + 1 lizard + 1 parse-error del helper SIN duplicar gates de bootstrap — camino hasDegradation de FR-7)
- [ ] Corte wall-clock (maxMinutes=1 + FAKE_DATE +90 s/paso): epoch 1750000090→180 ≥ deadline 1750000150 (solo 2 lecturas de epoch en el camino — +30 s no cortaría); anexos no generados, synthesize/judge corren (FR-11), judge CONCERNS por regla runtime duplicada (D11)
- [ ] Dominio: SQALE B en el borde — rating sobre ratio sin redondear (2501/(0.5·100000)=0.05002>0.05) con stored round3 0.05; informe pinta "= 0.050 → **B**", "**2501.0 h**" y la fila central "| **1.00** | **521.3** | **22.3** | **23.3** | **$1,251,217** |"
- [ ] Determinismo e2e: derived completo + exclusions + families["by-file"] deep-equal entre 2 corridas idénticas (sin FAKE_DATE — timestamps fuera de la comparación)
- [ ] Supuesto de entorno documentado: los conteos de degradaciones 9/5/4 asumen `lizard` ausente del PATH del runner (familia opcional de Python, `pip install lizard`; con lizard presente bajan en 1 y el dominio pasa a familiesOk 3) — comentario en el beforeEach del e2e

### Slice 8: Doctor + publicación

**Files**: `src/environment/doctor.ts`, `test/environment-doctor.test.ts`, `src/extension.ts`, `README.md`, `docs/tools/frida-size-app.md`, `docs/how-to-frida-size-app.md`, `docs/roadmap-extensiones.md`

#### Automated Verification

- [ ] Typecheck limpio: `npm run typecheck && npm run typecheck:test`
- [ ] Tests del doctor en verde (19 casos: 16 previos + 3 de scc): `npx vitest run test/environment-doctor.test.ts`
- [ ] Regresión del pack intacta: `npx vitest run test/frida-size-app/` en verde
- [ ] 8º check cableado: `grep -cF "export async function checkScc" src/environment/doctor.ts` = 1, `grep -cF "checkScc(agentDir, platform, arch)" src/environment/doctor.ts` = 1 y `grep -cF "agentDir?: string" src/environment/doctor.ts` = 1
- [ ] Call site productivo pasa agentDir: `grep -cF "await checkEnvironment({" src/extension.ts` = 1 y `grep -A3 -F "await checkEnvironment({" src/extension.ts | grep -cF "defaultAgentDir()"` = 1
- [ ] Entrada HELP_TOOLS: `grep -cF '"docs/tools/frida-size-app.md"' src/extension.ts` = 1 y `grep -cF '"docs/how-to-frida-size-app.md"' src/extension.ts` = 1
- [ ] Tests descongelados 7→8: `grep -cF "toBe(8)" test/environment-doctor.test.ts` = 2 y `grep -cF "totalCount).toBe(7)" test/environment-doctor.test.ts` = 0
- [ ] Fila README: `grep -cF "docs/tools/frida-size-app.md" README.md` = 1
- [ ] Roadmap M10 cerrado: `grep -cF "(pin v4.0.0 al agentDir)" docs/roadmap-extensiones.md` = 1 y `grep -cF "S–M | **P2** ✅ |" docs/roadmap-extensiones.md` = 1 (ojo: la celda real es `` `scc` ✅ (pin v4.0.0 al agentDir) `` — ancla sin backticks para no falsear el grep)
- [ ] Docs par presentes: `wc -l docs/tools/frida-size-app.md` ≥ 200 y `wc -l docs/how-to-frida-size-app.md` ≥ 200
- [ ] Baseline del repo (slice terminal): `npm test` completo en verde
- [ ] V8 motor intacto: `git diff --stat src/tools/frida-extensible-workflows/` vacío

#### Manual Verification

- [ ] Doctor UI ("Verificar entorno") lista 8 checks incluido scc con guía accionable; cero cambios webview (icono default `webview/components/EnvironmentTab.tsx:24-26`; readyCount/totalCount dinámicos línea 226 — sin "7" congelado)
- [ ] checkScc no invoca el binario (sonda síncrona marker+existsSync = misma que CAPABILITIES.scc): doctor y workflow nunca discrepan sobre scc
- [ ] Alias HELP_TOOLS sin colisión first-match: "size-app"/"size"/"tamaño"/"tamano"/"dimensionamiento"/"cocomo" no aparecen en ninguna entrada previa (verificado contra el arreglo completo en disco, exacto y por label.includes)
- [ ] Docs par coherente con el pack: cifras consistentes con las fórmulas (COCOMO Basic 81 etiquetado, spread EAF supuesto del analista, SQALE "proxy, no es SonarQube"), args/defaults exactos, árbol de entregables real
...
```text
src/tools/frida-size-app/constants.ts    # NEW — pin/assets/digests/URL builder/sonda scc
src/tools/frida-size-app/installer.ts    # NEW — ensureBinary + extractTarGz + isSccInstalledAtPin
src/tools/frida-size-app/skills.ts       # NEW — stages + preamble + prompts default
src/tools/frida-size-app/resolver.ts     # NEW — overrides 3-capas
src/tools/frida-size-app/workflow.ts     # NEW — validación eager + generador del script (5 fases)
src/tools/frida-size-app/index.ts        # NEW — patrón + factory + sonda + fire-and-forget
src/pi-session.ts                        # MODIFY — entrada frida-size-app en extensionFactories
src/environment/doctor.ts                # MODIFY — checkScc + deps.agentDir
test/environment-doctor.test.ts          # MODIFY — descongelar 7→8
src/extension.ts                         # MODIFY — HELP_TOOLS + defaultAgentDir() al doctor
README.md                                # MODIFY — fila frida-size-app en tabla Herramientas
docs/tools/frida-size-app.md             # NEW — referencia técnica
docs/how-to-frida-size-app.md            # NEW — guía de uso
docs/roadmap-extensiones.md              # MODIFY — fila M10 ✅
test/frida-size-app/installer.test.ts    # NEW — installer sin red
test/frida-size-app/resolver.test.ts     # NEW — 3 capas + veto en preamble
test/frida-size-app/pattern.test.ts      # NEW — validación/sonda/registro
test/frida-size-app/e2e.test.ts          # NEW — motor real + scc falso
test/frida-size-app/cocomo-domain.test.ts # NEW — dominio COCOMO (schema+fixtures+anti)
```

## Ordering Constraints

- Slice 1 (binario) no depende de nada; Slice 2 (prompts) tampoco — secuencial por disciplina.
- workflow.ts (Slices 3→4→5) es estrictamente incremental: cada parte usa las anteriores.
- Slice 6 (index.ts) importa `generateSizeAppWorkflow` (Slice 5) y `ensureBinary`/sonda (Slice 1) — no antes.
- Slice 7 (e2e) requiere el patrón completo registrado (Slice 6).
- Slice 8 (doctor+publicación) al final, con tests del pack en verde.
- El smoke real de scc v4.0.0 sobre OFBiz (paso del plan, Verification Note V1) debe ocurrir ANTES de congelar los mocks del e2e de Slice 7.
- Dentro de implement, cada slice es una unidad commit-eable; los hermanos se entregaron como Fase 1 pack puro → docs pipeline → fases atómicas → validación.

## Verification Notes

- **V1 — Smoke real antes de mocks** (lessons `30ef616`/`59517f7`/`9d6d8bb`): correr `scc@v4.0.0` REAL sobre un clone de OFBiz (`curl -L …/scc_Darwin_arm64.tar.gz` + `checksums.txt` manual) y capturar: salida `--by-file --format json` (shape FileJob exacta), CSVs de `--hotspots/--coupling/--by-author`, comportamiento fuera de git (subdir), `-a` DRYness. Los mocks del e2e derivan del contrato OBSERVADO, no supuesto. Verificación: transcript del smoke adjunto al plan/validación.
- **V2 — COCOMO tres números**: `grep -c "EAF" docs/dimensionamiento/dimensionamiento.md` ≥ 3 filas; el test de dominio verifica `E = 3.0·KSLOC^1.12·EAF` con fixtures precomputadas (p. ej. KSLOC=100, EAF=1.0 → E=3.0·100^1.12=**521.3** PM — cifra congelada en fixture; la "502.4" de la redacción original era un error aritmético corregido en Slice 5 tras verificación con harness ejecutable) y anti-fixtures (exponente del modo equivocado, EAF fuera de banda, wage ausente, rango no-creciente).
- **V3 — Exclusiones verificadas**: fixture de repo con `dist/` + `node_modules/` + `*.min.js` committeados (sin .gitignore para forzar el caso): la corrida los excluye y `metrics.json.exclusions` declara el volumen; `dimensionamiento.md` lo muestra. `grep "volumen excluido" docs/dimensionamiento/dimensionamiento.md` encuentra la tabla.
- **V4 — Degradación honesta**: con `lizard` ausente del PATH, `metrics.json.degradations` contiene `{familia: "ccn-funcion", …}` y el informe marca "no disponible". Test de patrón lo congela.
- **V5 — Juez mentiroso**: informe que declara familia sin sustento en `metrics.json` → juez FAIL con finding CRITICAL/HIGH; el veredicto viaja en el return sin abortar (molde M1 e2e:652-676).
- **V6 — Instalación nunca bloquea** (lessons `34d496a`/`7500370`): fire-and-forget sin await en la factory; fallo de red → log + degradación con guía; test: factory registra el patrón aunque ensureBinary rechace.
- **V7 — Integridad del binario**: sha256 del archivo comprimido verificado ANTES de extraer (orden espejo `fetch.ts:339-343`); sha incorrecto → rechazo + nada a medias (sin marker, sin binario). Test negativo con estado inmutable.
- **V8 — Motor intacto**: `git diff --stat src/tools/frida-extensible-workflows/` vacío tras implementar (señal de alerta si no).
- **V9 — Baseline del repo**: `npm run typecheck` + `npm run typecheck:test` + `npx vitest run test/frida-size-app/` + `npm test` completos en verde; `bash -n` sobre los mocks bash del e2e.
- **V10 — Piloto OFBiz**: corrida sobre clone fresco; números (KLOC efectivos, CCN p50/p90/p99, top churn/hotspots, COCOMO±rango con costo) como comentario de evidencia en el issue #139.
...
- `src/tools/frida-understand-app/index.ts` — factory + sonda + resolve 3 pasos (molde directo del index.ts del pack)
- `src/tools/frida-understand-app/workflow.ts:99-136` — validateUnderstandAppArgs (molde del error D13 de wage)
- `src/tools/frida-understand-app/workflow.ts:262-346` — writeText/shq/epochNow/slug + inventario writer único (molde de metrics.json)
- `src/tools/frida-understand-app/workflow.ts:396-505` — guards wall-clock + fan-out con gate/reintento (molde de analyze)
- `src/tools/frida-understand-app/skills.ts:38-55,151-191` — preamble no-stage + rúbrica del juez (molde)
- `src/tools/frida-traffic2api/workflow.ts:620-647,950-970` — fence T2A_EOF + abRun + helper node host-side para JSON grande (molde de la fase metrics)
- `src/tools/frida-codebase-index/constants.ts:9-71` — pin + PLATFORM_NATIVE explícita + currentPlatformNative (molde de SCC_ASSETS)
- `src/tools/frida-codebase-index/installer.ts:29-233` — InstallError con guide + isInstalledAtPin + ensureInstalled (espejo estructural de ensureBinary)
- `src/tools/frida-cc-plugins/fetch.ts:126-217,330-343` — unzipSync exportada + defaultFetchArchive https-only + verificación sha256 (patrón a copiar ~50 líneas)
...
- `src/environment/doctor.ts:428-535` — checkOllama (molde versión-de-binario + hint) + checkEnvironment (agregador a extender)
- `test/frida-traffic2api/openapi-schema.test.ts` — test de dominio: schema TypeBox + fixtures + anti-fixtures + e2e compacta (molde de cocomo-domain)
- `test/frida-cc-plugins/phase2.test.ts:403-435` — negativo de integridad sha256 con estado inmutable (molde)
- `test/frida-understand-app/e2e.test.ts:55-99,266-311,652-676` — HOME aislado + .mock-bin + FAKE_DATE + juez FAIL sin abortar (moldes e2e)

## Developer Context

**Q (D1 — directional, tamaño del pack)**: El FRD decía "4 archivos + installer"; el molde codebase-index separa `constants.ts` de `installer.ts` y el research recomendó constants propio (pin+digests+tabla explícita, lección de sufijos `constants.ts:51-57`). ¿Separar (6 archivos) o embutir en index (4)?
A: **Separar (molde codebase-index)** — 6 archivos.

**Q (D2 — directional, disparo de la descarga)**: `resolve()` es síncrona; molde hermes dispara fire-and-forget al registrar la factory (primera sesión descarga ~7 MB; luego no-op). ¿Descarga al arranque o on-demand (primera corrida degrada, lista la segunda)?
A: **Sí, al arranque** — size-app listo sin espera en su primera corrida.

**Q (D3 — directional, frontera determinista/agéntica)**: FR-9 exige `dimensionamiento.md` exclusivamente de `metrics.json`. Propuesta: analyze = 3 escritores fijos de anexos interpretativos bajo `analisis/`; informe 100% determinista citando anexos. ¿Confirmar?
A: **Sí, determinista total**.

**Q (D4 — matrix de plataformas)**: ¿Piso FRD (darwin/arm64 + linux/x64) o matrix completa de 8 assets del release v4.0.0 (digests reales ya obtenidos; `unzipSync` cubre win32; costo marginal cero)?
A: **Matrix completa (8)**.

**Q (D5 — checkScc en doctor)**: ¿Incluir el 8º check en M10 (impacto mapeado: doctor.ts + 3 congelaciones de test + `defaultAgentDir()` en extension.ts:3445; cero webview) o defer a issue propio?
A: **Incluir en M10**.

**Q (D6/D7 heredadas del checkpoint de research)**: COCOMO replicando scc exacto (constantes por modo, overhead 2.4, wage MENSUAL); spread EAF 0.85/1.00/1.15 etiquetado como supuesto del analista. Decisión heredada, registrada en el research — no se re-preguntó.

**Resumen confirmado para descomposición**: "Proceed (Recommended)" sobre el brief de diseño completo (pack 6 archivos + ensureBinary matrix 8 + script 5 fases + checkScc + trío tests + dominio COCOMO + publicación).

**Descomposición confirmada**: "Approve (Recommended)" — 8 slices (binario → prompts → workflow×3 → patrón → e2e → doctor+publicación).

**Micro-checkpoint Slice 1 (aprobado)**: "Approve (Recommended)" — constants+installer+installer.test presentados condensados (firmas, extractTarGz, orden sha→extract, fixtures en-test). Verificador: 1ª pasada 3 VIOLACIONES (claves i386 inalcanzables por `process.arch`=ia32; import `SCC_RELEASE_BASE` faltante usado 2× en manualGuide; `isSccInstalledAtPin` sin `arch` → test dependiente del host) → fix-and-re-dispatch → OK/OK/OK. Nota de persistencia: el autofix de pi-lens (markdownlint) normaliza tabs→espacios dentro de los fences de código del artefacto — sin cambio semántico; slices futuros escriben fences sabiendo que el formatter los reindentará.

**Micro-checkpoint Slice 2 (aprobado)**: "Approve (Recommended)" — skills/resolver/resolver.test presentados condensados (preamble JUEZ DE NÚMEROS, prompts analyze/judge con rúbrica FR-10, sexto consumidor del customize-layer, caso D11 de fases no overrideables). Verificador: OK/OK/OK a la primera (0 violaciones).

**Micro-checkpoint Slice 3 (aprobado)**: "Approve (Recommended)" — parte 1 de workflow.ts presentada condensada (SizeAppArgs/SizeAppCapabilities/validateSizeAppArgs espejo M1 `frida-understand-app/workflow.ts:99-136`, error D13 de wage con MXN $35,000/USD $6,000 y mapeo wage+currency embebidos, helper local `requireStringArrayAllowEmpty` que acepta []). Verificador: 1ª pasada 1 VIOLATION (criterio grep bare `Number.isInteger`=1 falso — token también en docstrings) → fix-and-re-dispatch (greps precisos + cita `adversarial-review.threshold` en vez de `.reviewers`) → 2ª pasada OK/OK/OK. La cita del molde decimal en la prosa de D7 quedó sincronizada (código = fuente de verdad).

**Micro-checkpoint Slice 4 (aprobado)**: "Approve (Recommended)" — parte 2 de workflow.ts presentada condensada (generador con bootstrap+metrics: gates scc/git/lizard deterministas, sondas scc redirect-a-disco por `shq(SCC_BIN)` ruta absoluta, 2ª pasada raw para volumen excluido, helper node host-side `metrics-agg.js` molde carve M9 `frida-traffic2api/workflow.ts:950-970`, writer único `metrics.json` con fence `SA_EOF`, degradaciones `{familia, causa, hint}` FR-7). Verificador: 1ª pasada 2 VIOLATIONES — (1) D8 escapes: el bloque lizard del template estaba un nivel corto (`/\./g` y `"\\."` se cocinan a `/./g` y `"."` → patrón -x matchea por longitud de segmento: `src/`, `lib/` sobre-excluidos; convención del molde exige doblar — mdCell `frida-understand-app/workflow.ts:296-297` escribe `\\|` para emitir `\|`); (2) criterio `SQALE_THRESHOLD>=3` falso (token en 2 líneas, grep -c cuenta líneas) → fix-and-re-dispatch (escapes doblados `replace(/\\./g, "\\\\.")` + `"|\\\\.min\\\\.js$"` → script generado `replace(/\./g, "\\.")` → runtime `(^|/)(dist|…|\.next|coverage)/|\.min\.js$` verificado nivel por nivel; criterio >=2) → 2ª pasada OK/OK/OK. Lección transversal para Slice 5: TODO regex/string escapado dentro del template del generador lleva dobles (regex) o cuádruples (string que debe contener backslash) escapes.

**Micro-checkpoint Slice 5 (aprobado)**: "Approve (Recommended)" — parte 3 de workflow.ts presentada condensada (fan-out analyze de 3 escritores fijos con gate test -s + reintento único molde M1 `frida-understand-app/workflow.ts:396-505`; synthesize determinista con `buildDerived`/`buildWaves`/`computeBusFactor`/`sqaleRating` computando EXCLUSIVAMENTE de `inv.families` con fórmulas declaradas junto al valor; informe `dimensionamiento.md` + `README.md` sin veredicto del juez; judge detached con regla corte→CONCERNS duplicada prompt+runtime y checkpoint solo `review=manual`; deadline corta solo analyze, FR-11). **Novedad del slice**: código validado con harness ejecutable (`/tmp/slice5/validate.mjs`, no parte del artefacto) que compila el template TS, verifica escapes críticos, valida el script generado como JS y ejecuta 2 casos: degradado sin abortar (kloc null, 9 degradaciones, 3 anexos, judge CONCERNS) y completo con fixture (kloc=100 → COCOMO 443.1/521.3/599.5 PM = 521.3×0.85/1.0/1.15, constantes scc exactas; SQALE B en ratio 0.05001). Verificador: 1ª pasada 1 VIOLATION (bullet de criterio "semanas=0 si TDEV central ausente" describía un camino muerto — buildWaves early-return con waves:[] y centralTdev nunca null con spread fijo) + 2 WARNING (claim pre-juez en pasado en "Cómo auditar"; línea muerta en el harness) → fix-and-re-dispatch (bullet reformulado a early-return waves:[]; redacción atemporal "incluye el veredicto… que muestrea"; harness limpiado con check V2 formula-based) → 2ª pasada OK/OK + WARNING residual sobre la cifra "502.4" de la Verification Note V2 (error aritmético: 3.0·100^1.12=521.3) corregida en este artefacto. Lecciones transversales para Slices 6-8: (a) escapes doblados aplican también a NEWLINES y COMILLAS de strings del script (`\n` del script = `\\n` en el fuente TS; comillas internas = `\\"`) — el template NO compila con backtick crudo y el script generado NO compila con `\n` simple; (b) la fixture COCOMO para cocomo-domain (Slice 7) congela 521.3 PM para KSLOC=100/EAF=1.0 semi-detached (443.1/599.5 en los bordes del spread) — no copiar cifras de prosa sin recomputar.

**Micro-checkpoint Slice 6 (aprobado)**: "Approve (Recommended)" — index/pi-session/pattern.test presentados condensados (sonda scc síncrona `isSccInstalledAtPin` + moat M1/M9, fire-and-forget con gate idempotente y log de éxito solo producción `ensureDeps === undefined`, 31 casos con warns espiados por contenido). Verificador 3 pasadas: 1ª — 4 VIOLATIONES (import `sccBinPath` desde `./installer` no exportado → `./constants`; asserts `const review = "manual"`/`const language = "es-MX"`/`const maxMinutes = 60` contradecían el canon defensivo del generador locked → `? args.review : "manual"` / `|| "es-MX"` / `? args.maxMinutes : 60`; conteos grep con tokens en docstrings → greps precisos; log `vpin` garabateado); 2ª — VIOLATION de aritmética de warns: el catch produce 2 warns por rechazo (mensaje + guía — `SccInstallError` de fetch-failure siempre lleva `guide` del `manualGuide` locked) → asserts por contenido, no por conteo total; 3ª — OK/OK/OK. Lección transversal para Slices 7-8: (c) al espiar logs multi-línea del fire-and-forget en tests, asertar por CONTENIDO del mensaje (`stringContaining` / filter de `mock.calls`), nunca por conteo total de llamadas — el número de líneas del catch no es contrato; (d) los escalares del script (maxMinutes/language/review) se asierten en su forma defensiva, las estructurales (wage/currency/cocomoType/exclude) como consts directas — espejo del criterio del molde M9 pattern.test.

**Micro-checkpoint Slice 8 (aprobado)**: "Approve (Recommended)" — doctor + publicación presentados condensados (checkScc 8º check con sonda síncrona del Slice 1 y `agentDir` inyectable, descongelación 7→8 con agentDir tmpdir para determinismo, HELP_TOOLS/README/roadmap en las posiciones exactas del wiring M9, docs par con esqueleto de los hermanos). **Novedad del slice**: slice-verifier NO despachable — cuota 429 del proveedor agotada en 2 intentos (modelo default y haiku; reset 15:02) → verificación adversarial INLINE del orquestador en 3 filas (Decisions/Cross-slice/Research, todas OK) sometida a ratificación explícita en el checkpoint — aprobada. Verificaciones clave hechas en disco: webview sin conteos congelados (`webview/components/EnvironmentTab.tsx:226` pinta `${env.readyCount} de ${env.totalCount}` dinámico; icono `default: "tools"` líneas 24-26 cubre id "scc" → claim "cero webview" verificado), 16 its actuales → 19 con 3 nuevos, alias nuevos sin colisión exacta ni label.includes contra TODO el HELP_TOOLS (incluida la entrada posterior workflows), `defaultAgentDir()` (`pi-session.ts:1210-1215`) = `path.join(os.homedir(),".frida")` idéntico al fallback del doctor, toBe(8)=2 exactas tras reemplazar las 2×toBe(7), `"S–M | **P2** ✅ |"` sin colisión (M9 es `M |…`, M2 queda sin ✅).

## Design History

- Slice 1: Fundaciones del binario — constants + installer — approved as generated (revisado por slice-verifier: 3 VIOLACIONES corregidas antes de aprobar — claves Node `ia32` en SCC_ASSETS, import `SCC_RELEASE_BASE`, parámetro `arch` en `isSccInstalledAtPin`; re-dispatch OK)
- Slice 2: Prompts y resolver 3-capas — approved as generated (verificador OK/OK/OK a la primera: D3/D11/D12/FR-10 honrados; superficie de exportación completa para Slices 3-6; atomicidad de criterios sin forward-refs)
- Slice 3: Args y validación eager (workflow.ts parte 1) — approved as generated (verificador 1ª pasada: 1 VIOLATION de atomicidad — grep bare `Number.isInteger`=1 falso, el token vive en 3 líneas (2 docstrings + 1 uso) → greps precisos `Number.isInteger(record.maxMinutes)`=1 / `(record.wage)`=0 + cita de molde corregida `adversarial-review.reviewers`→`threshold` (builtin-patterns.ts:443-450, número 0-1 sin isInteger); 2ª pasada OK/OK/OK. Prosa D7 del artefacto sincronizada con la cita corregida)
- Slice 4: Generador — bootstrap + metrics (workflow.ts parte 2) — approved as generated (verificador 1ª pasada: 2 VIOLATIONES — escapes del bloque lizard en el template literal (D8: `\.` cocinado a `.` → patrón -x de lizard matcheaba por longitud de segmento y sobre-excluía src/, lib/) y criterio grep `SQALE_THRESHOLD>=3` falso (grep -c cuenta líneas: 2) → fix-and-re-dispatch (escapes doblados convención mdCell/shq M1 + criterio >=2) → 2ª pasada OK/OK/OK)
- Slice 5: Generador — analyze + synthesize + judge (workflow.ts parte 3) — approved as generated (verificador 1ª pasada: 1 VIOLATION de atomicidad — bullet de criterio "semanas=0 si TDEV central ausente" describía un camino muerto (buildWaves early-return → waves: []; centralTdev nunca null con spread fijo) — corregido el bullet; + 2 WARNING: claim pre-juez en presente corregido ("incluye el veredicto… que muestrea") y harness de evidencia limpiado; 2ª pasada OK/OK + WARNING V2: cifra "502.4 PM" de la Verification Note era error aritmético → corregida a 521.3 en el artefacto. Código validado con harness ejecutable: template compila, script JS válido, corrida degradada no aborta, COCOMO 443.1/521.3/599.5 exacto)
- Slice 6: Patrón + factory + registro — approved as generated (verificador 3 pasadas — 1ª: 4 VIOLATIONES: import `sccBinPath` desde `./installer` (no exportado, vive en `./constants`), 3 asserts contra el canon defensivo del generador locked (escalares `? args.review : "manual"` / `|| "es-MX"` / `? args.maxMinutes : 60`, no consts directas), conteos grep corregidos (createFridaSizeApp=2, isSccInstalledAtPin=5 con docstrings → greps precisos, it()=31 no 20) y log de éxito garabateado `vpin`; 2ª: VIOLATION de aritmética — el catch loguea 2 warns por rechazo (mensaje + guía; SccInstallError de fetch-failure SIEMPRE lleva guide del manualGuide locked) vs toHaveBeenCalledTimes(1)/(1)/(2) real 2/2/4 → asserts por CONTENIDO (stringContaining("instalación de scc falló") + filter de mock.calls, inmune al número de líneas del catch); 3ª: OK/OK/OK)
- Slice 7: E2E + dominio COCOMO — approved as generated (verificador 2 pasadas — 1ª: 2 VIOLATIONES: cifras congeladas del e2e no recomputadas bajo la fórmula locked (fixture cognitive 30/40/12/20 → debt 22.5 h, no 20 — faltó lib/legacy al propagar; cascada ratio 0.013, olas 56/33/11% → 14.7/8.8/2.9 sem) y toEqual de 2 claves contra shape locked de sqale de 4 claves en el dominio → fix-and-re-dispatch (cifras verificadas con node + toMatchObject para el borde, 4 claves completas siguen congeladas en el e2e) → 2ª: OK/OK + WARNING ratificado by-design: conteos de degradaciones 9/5/4 asumen lizard real ausente del PATH del runner (documentado en beforeEach + criterio Manual; un guard dinámico reintroduciría la clase binario-externo-miente 30ef616/59517f7/9d6d8bb). Mock scc por ruta absoluta al pin — NO en .mock-bin — y corte con FAKE_DATE +90 s por solo haber 2 lecturas de epoch en el camino)
- Slice 8: Doctor + publicación — approved as generated (verificación INLINE del orquestador por cuota 429 del proveedor — 2 intentos de slice-verifier fallidos, default y haiku; 3 filas Decisions/Cross-slice/Research en OK ratificadas en el micro-checkpoint: D5 honrado — checkScc con sonda síncrona isSccInstalledAtPin y categoría "extension"; exports del Slice 1 con firmas exactas; greps de criterios recantados; alias sin colisión contra TODO el HELP_TOOLS; webview sin congelaciones — EnvironmentTab.tsx:226 pinta readyCount/totalCount dinámico e icono default "tools" líneas 24-26 cubre id "scc"; defaultAgentDir() = path.join(os.homedir(),".frida") idéntico al fallback del doctor (pi-session.ts:1210); 16 its + 3 = 19; toBe(8)=2 exactas. Byte-check post-escritura corrigió 1 criterio: el grep "scc ✅ (pin…)" no matcheaba la fila real `` `scc` ✅ (pin…) `` — ancla reescrita a "(pin v4.0.0 al agentDir)"; el CÓDIGO del bloque roadmap quedó byte-exacto al formato real del archivo)

## References

- Research: `.rpiv/artifacts/research/2026-08-27_06-28-43_m10-size-app.md` (parent — hallazgos con file:line)
- FRD: `.rpiv/artifacts/discover/2026-08-27_04-15-43_m10-size-app.md` (issue #139)
- FRDs hermanos: `.rpiv/artifacts/discover/2026-08-24_19-14-49_m1-understand-app.md`, `.rpiv/artifacts/discover/2026-08-26_12-44-58_traffic2api.md`
- Plan M9: `.rpiv/artifacts/plans/2026-08-26_17-43-36_traffic2api-m9.md` (lecciones smoke-real y fixtures)
- Validación M1: `.rpiv/artifacts/validation/2026-08-25_08-21-08_m1-skill-pack-frida-understand-app.md`
- Release scc v4.0.0: `https://github.com/boyter/scc/releases/tag/v4.0.0` (assets + `checksums.txt` — digests obtenidos en este design)
- COCOMO 81 (Boehm 1981) vía implementación scc `processor/cocomo.go`; banda EAF documentada: Intermediate COCOMO "típicamente 0.9 a 1.4" (Wikipedia EN)
