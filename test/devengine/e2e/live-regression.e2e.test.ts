...
describe("E2E live REGRESIÓN: issues del reporte fix-frida-gateway.md", () => {
	it(
		"probes P1-P4 (round-trip reasoning, content null, /models metadata) → reporte MD",
		async () => {
			...
			lines.push("");
			lines.push(`Endpoint: ${DEVENGINE_BASE_URL} · probes con fetch plano (sin workarounds de Frida)`);
			lines.push("");
