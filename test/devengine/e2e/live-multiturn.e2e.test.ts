...
describe("E2E live MULTITURN: DevEngine × conversaciones 2-3 turnos", () => {
	it(
		`matriz de casos multiturno → reporte MD`,
		async () => {
			...
			const lines: string[] = [];
			lines.push(
				`# Reporte E2E multiturn — ${DEVENGINE_MODEL} (${new Date().toISOString()})`,
			);
			lines.push("");
			lines.push(
			...
			lines.push(`## Resumen`);
			lines.push("");
			lines.push(`- **${okCount}/${CASES.length}** casos mantienen contexto`);
			lines.push("");
			...
			// Exige que al menos 1 caso funcione (para detectar regresión total)
